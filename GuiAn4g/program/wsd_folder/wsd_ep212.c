...
void  DealUartMessage_Ep212( TSerial *p )
{
	...
	if (nwy_put_msg_que(UartToEp212MsgQue, &tempTrandata, 0xffffffff) == TRUE)
	{
	...
	if (bFlowFrame)
	{
		...
		nwy_start_timer(uart_timer, 10000); //10s定时 需根据上报速度 进行调整
	}
	else
	{
...
void  Deal_UserServer_Ep212( tTranData *tempTranData ,BYTE bChannel)
{
	...
	nwy_ext_echo("\r\n ep212 recv \r\n");
	if(nwy_put_msg_que(Ep212ToUartMsgQue, tempTranData, 0xffffffff))
	{
