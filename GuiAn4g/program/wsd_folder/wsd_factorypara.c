...
WORD ReadFactoryExtPro(BYTE *pBuf)
{
	...
	case 0xF0:
		...
		else if (pBuf[0] == 0x0E) //离线跳闸最小时间
		{
			addr = GET_SAFE_SPACE_ADDR(ReportPara.OfflineOpenRelayMin);
			if (api_OperateFileSystem(READ, addr, (BYTE *)&ReportPara.OfflineOpenRelayMin, sizeof(ReportPara.OfflineOpenRelayMin)) == TRUE)
			{
				memcpy(p, (BYTE *)&ReportPara.OfflineOpenRelayMin, sizeof(ReportPara.OfflineOpenRelayMin));
				wReturnLen = sizeof(ReportPara.OfflineOpenRelayMin) + 2;
			}
			else
			{
				wReturnLen = 0;
...
WORD WriteFactoryExtPro(BYTE *pBuf)
{
	...
	switch (pBuf[1])
	{
		...
		case 0xF0:
			...
			{
				memcpy((BYTE*)&ReportPara.OfflineOpenRelayMin,(BYTE*)&pBuf[8],sizeof(ReportPara.OfflineOpenRelayMin));
				addr = GET_SAFE_SPACE_ADDR(ReportPara.OfflineOpenRelayMin);
