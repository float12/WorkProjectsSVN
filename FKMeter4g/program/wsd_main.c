...
nwy_osiTimer_t *FactoryRecv_ByteTimeout_timer = NULL;//维护端帧间超时定时器
...
nwy_osiMessageQueue_t *Ep212ToUartMsgQue = NULL;
nwy_osiMessageQueue_t *UartToEp212MsgQue = NULL;
#endif
...
void  CheckSysVolandConnectStatus( void )
{
	...
	if (HisUserTcpConFlag != UserTcpConFlag)//tcp_recv sleep60 秒 导致检测状态会等60 
	{
		...
		if (nwy_put_msg_que(UserTcpStatusChangeMsgQue,&UserTcpConFlag,0xffffffff))
		{
			nwy_ext_echo("\r\n tcp status changed");
		}
	}
...
void  CheckSysVolandControlLight( void )
{
	...
	if (nwy_ext_check_data_connect() != 1)
	{
		for ( i = 0; i < 49; i++)
		{
			nwy_gpio_set_value(REMOTE_LIGHT,(i%2));
			nwy_sleep(100); 
		}
	}
...
void  InitExternalPinsandFlags( void )
{
	...
	nwy_gpio_set_value(FLASH_PIN,1);//硬件原因
...
static void prvThreadEntry(void *param)
{
	...
	#endif
	#if(CYCLE_REPORT_PROTOCAL == PROTOCOL_212)
	Ep212ToUartMsgQue = nwy_create_msg_Que(5, sizeof(tTranData));
	UartToEp212MsgQue = nwy_create_msg_Que(5, sizeof(tTranData));
	#endif
	#if(EVENT_REPORT == YES)
	...
	#endif
	FactoryRecv_ByteTimeout_timer = nwy_timer_init(Timer_thread, factory_Recv_timer_cb, NULL);
	#if (TCP_COMM_MODULE == YES)
