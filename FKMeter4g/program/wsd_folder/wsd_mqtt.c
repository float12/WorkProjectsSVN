...
void MQTT_UserServices(void)
{
	...
	#endif
	MqttRecvfromUser();
	MqttRepytoUser(&paho_user_client);
}
#endif
#if(MQTT_USER == YES)
...
void MQTT_connetuser(void)
{
	...
	if (MQTTIsConnected(&paho_user_client))
	{
		nwy_ext_echo("\r\n user mqtt  connected!!! ");
		SubMqttTopic_user(&paho_user_client);
	}
}
			...
			if (0 != nwy_ext_check_data_connect())
			{
				if (MQTTIsConnected(&paho_user_client) != 1)
				{
					...
					if (MQTTIsConnected(&paho_user_client) != 1) //没有连接成功，再进行延时，连接成功不再进行延时
					{
						...
						{
							UserMqttReconnectTimerFlag = 0;
							nwy_start_timer(User_MQTT_Reconnect_timer, 60000 * 4); // 4分钟重试一次连接 每次连接消耗560字节
						}
					}
					else
					{
