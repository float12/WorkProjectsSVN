...
{
	MoniIapNo = 0;		 //监测单元当前升级步骤
	MoniFrameNo = 0;	 //监测单元升级数据包帧序号 //可以作为文件偏移使用
	TimeoutRetryNum = 0; // 重发次数
...
void EnterUpgrade645(void)
{
	...
	tIap_645.dwIapFlag = BASEMETER_UPGRADES; //放最后，确保置标志前清完变量
}
//-----------------------------------------------
// 函数功能: 进入升级变量处理
//
...
void ExitUpgrade645(eUpgradeResult result)
{
	...
	MeterUpgradeFD = -1;
	if(nwy_sdk_file_unlink(upgrade_file_path_645) == NWY_SUCESS)
	{
...
void handleCheckResult(BYTE *buf)
{
	...
	// 判断状态数据偏移位置
	if ((buf[OMD_OFFSET + 6]) == 0x82) // 返回块长度域
	{
		CheckFrameResultOffest = OMD_OFFSET + 6 + 3;
	}
	else if ((buf[OMD_OFFSET + 6]) == 0x81) // 返回文件长度域
	{
...
//参数:      
//         
//返回值:    TRUE: CRC校验正确，FALSE: CRC校验错误
//         
//备注:  
//--------------------------------------------------
BYTE  ComputeFileCheckSum( void )
{
	...
	for (i = 0; i < tIap_645.wAllFrameNo; i++)
	{
		...
		bLen = nwy_sdk_fread(MeterUpgradeFD, &Buf[0], LEN_OF_ONE_FRAME);
	...
	sprintf(crcStr, "%04X", tIap_645.wCrc16); // 将 CRC16 转换为字符串形式,大写形式
...
eUpgradeResult SendIapData645(void)
{
	...
	if(MoniIapNo == 0)//组报文发送升级请求，读出升级包总帧数
	{
		...
		p->ProBuf[wLen++] = 0x16;
		nwy_stop_timer(meter_upgrade_timer);
		nwy_start_timer(meter_upgrade_timer, 900000);//9600 基表升级时间实测10分钟  留1.5倍预留
	}
	else if(MoniIapNo == 1)//组报文发送升级包
	{
		...
		if(tIap_645.dwIapFlag == BASEMETER_UPGRADES)
		{
			...
			bLen = nwy_sdk_fread(MeterUpgradeFD, &p->ProBuf[wLen], LEN_OF_ONE_FRAME);
			if (bLen != LEN_OF_ONE_FRAME)
			{
	...
	// }
	nwy_uart_send_data(UART_HD,p->ProBuf,p->SendLength);
...
void SendIapData698(void)
{
	...
	if (MoniIapNo == 0) // 组报文发送升级请求，读出升级包总帧数
	{
		...
		nwy_stop_timer(meter_upgrade_timer);
		nwy_start_timer(meter_upgrade_timer, 360000);//115200 基表升级时间实测小于4分钟  留1.5倍预留
		MeterUpgradeTimeUpFlag = 0;
		nwy_ext_echo("\r\n send start frame\r\n");
	}
...
void  MeterUpgrade698_Task( void  )
{
	if ((api_GetSysStatus(eSYS_STASUS_START_COLLECT) != TRUE)&&(api_GetSysStatus(eSYS_STASUS_START_TRAN) != TRUE))
	{
		...
		if (api_GetSysStatus(eSYS_STASUS_UPGRADE_REPLY_FRAME))
		{
			...
			api_ClrSysStatus(eSYS_STASUS_UPGRADE_REPLY_FRAME);
			nwy_stop_timer(uart_timer);
			nwy_start_timer(uart_timer, 1000); // 调试重传修改状态位需要时间
		}
		else
		{
