...
#define USER_SERVER_LIGHT						TCP_USER		//远程上线灯取决于要上哪个主站
	//TCP 协议
#define MAX_PRIVATE_SERVER_NUM					1				//纯透传tcp服务器个数 （tcp可最多连接8个，默认0连接维护端主站，其他用户端主站需求若也是透传基表，对应宏定义递增）
#define MAX_USER_SERVER_NUM						0				//用户端tcp服务器个数
#define	TCP_USER								NO				//tcp 连接用户端主站
#define TCP_COMM_MODULE							NO				//tcp 与模块通信 
	//MQTT 协议
...
extern nwy_osiTimer_t *uart_send_complte_timer;
