...
void Tcp_connet(tTranData *transdata,BYTE *bNum)
{
	...
	else
	{
		...
		nwy_ext_echo("\r\nserver bNUM[%d]  ip %s",bChannel, Serverpara[bChannel-1].ServerIP);
	}
...
void TCP_PrivateTask(void *param)
{
	...
	BYTE bServerNum;
	#if(MAX_PRIVATE_SERVER_NUM != 1)
	DWORD UserConnectfre[MAX_PRIVATE_SERVER_NUM] = {0};
	#endif
	bServerNum = *(int *)param;
	nwy_ext_echo("\r\n tcp private task bServerNum is %d",bServerNum);
	while(1)
	{
...
void TCP_User_Task(void *param)
{
	...
	bServerNum = *(int *)param;
	tTranData UploadTranData;
	DWORD UserConnectfre[MAX_USER_SERVER_NUM] = {0};//待修改
	...
	while (1)
	{
		if(0 != nwy_ext_check_data_connect())
		{
			if(socket[bServerNum] == 0)
			{
				...
				if(socket[bServerNum] == 0) 	//连接失败时，每分钟执行一次
				{
					UserConnectfre[bServerNum-MAX_PRIVATE_SERVER_NUM]++;
					nwy_sleep(60000); 		//等待60s
					if (UserConnectfre[bServerNum-MAX_PRIVATE_SERVER_NUM] > USER_RESET_INTERVAL)
					{
						nwy_ext_echo("\r\n reset!!!");
						nwy_power_off(2);
					}
			...
			{
				UserConnectfre[bServerNum-MAX_PRIVATE_SERVER_NUM] = 0;
