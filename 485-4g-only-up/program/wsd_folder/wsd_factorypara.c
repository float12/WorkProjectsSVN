...
WORD ReadFactoryExtPro(BYTE *pBuf)
{
	...
	case 0xF0:
		...
		else if (pBuf[0] == 0x15) // 表波特率
		{
			lib_ExchangeData(p, (BYTE *)&MeterBaud, sizeof(MeterBaud));
			wReturnLen = sizeof(MeterBaud) + 2;
		}
		break;
	default:
		break;
...
WORD WriteFactoryExtPro(BYTE *pBuf)
{
	...
	switch (pBuf[1])
	{
		...
		case 0xF0:
			...
			{
				memcpy((BYTE *)&MeterBaud, &pBuf[8], sizeof(MeterBaud));
				wReturnLen = sizeof(MeterBaud) + 2;
				api_OperateFileSystem(WRITE, GET_SAFE_SPACE_ADDR(MeterBaud), (BYTE *)&MeterBaud, sizeof(MeterBaud));
			}
...
// 备注内容:  
//--------------------------------------------------
void api_PowerOnCreateParaFile( void )
{
	int fd;
	TSafeMem SafeMem = {0};
	...
	if(fd >= 0)
	{
		api_OperateFileSystem(WRITE,0,(BYTE*)&SafeMem,(sizeof(TSafeMem))); 
	}
	nwy_sdk_fclose(fd);
}
//--------------------------------------------------
