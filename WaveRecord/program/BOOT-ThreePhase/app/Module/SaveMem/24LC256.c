...
//-----------------------------------------------
//EEPROM的IIC定义
#define EEPROM_I2C_SDA_PORT_IN			I2cSDAIn()
#define EEPROM_I2C_SDA_PORT_DATA_IN		PORT_GetBit(PortH,Pin02)
#define EEPROM_SCL_L					PORT_ResetPortData(PortC,Pin13)
#define EEPROM_SCL_H					I2cSCLOutHigh()
#define EEPROM_SDA_L					I2cSDAOutLow()
...
void I2cSCLOutHigh(void)
{
	stc_port_init_t pInit;

	MEM_ZERO_STRUCT(pInit);			 // 全部Disable
	pInit.enPinMode = Pin_Mode_Out;	 // 输出
	pInit.enPinOType = Pin_OType_Od; // 开漏
	PORT_Init(PortC, Pin13, &pInit);
...
void I2cSDAIn(void)
{
	...
	MEM_ZERO_STRUCT(pInit); // 全部Disable  浮空输入，内部上拉无效
	pInit.enPinMode = Pin_Mode_In;// 输入
	PORT_Init(PortH, Pin02, &pInit);
}

void I2cSDAOutLow(void)
{
	stc_port_init_t pInit;
	...
	PORT_ResetPortData(PortH, Pin02);
}
//-----------------------------------------------
//函数功能: IIC的延时函数，内部函数
//
