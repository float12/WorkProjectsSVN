		...
		.NextTable[0] = &(TDataTable)
		{
			...
			.NextTable[1] = &(TDataTable)
			{
				...
				.NextTable[0] = &(TDataTable)
				{
					...
					{
						.TMqttData[0]  = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321112C00001",.ValueType = eP1, },
						.TMqttData[1]  = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321112400001",.ValueType = ePF1,},
						.TMqttData[2]  = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113900001",.ValueType = eFre,},
						.TMqttData[3]  = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321110100001",.ValueType = eUa, },
						.TMqttData[4]  = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321110200001",.ValueType = eUb, },
						.TMqttData[5]  = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321110300001",.ValueType = eUc, },
						.TMqttData[6]  = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321110900001",.ValueType = eIa, },
						.TMqttData[7]  = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321110A00001",.ValueType = eIb, },
						.TMqttData[8]  = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321110B00001",.ValueType = eIc, },
						.TMqttData[9]  = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113400001",.ValueType = ePActiveAll,},
						.TMqttData[10] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113401001",.ValueType = ePActive1,},
						.TMqttData[11] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113402001",.ValueType = ePActive2,},
						.TMqttData[12] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113403001",.ValueType = ePActive3,},
						.TMqttData[13] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113404001",.ValueType = ePActive4,},
						.TMqttData[14] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113405001",.ValueType = ePActive5,},
						.TMqttData[15] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113400002",.ValueType = eNActiveAll,},
						.TMqttData[16] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113401002",.ValueType = eNActive1,},
						.TMqttData[17] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113402002",.ValueType = eNActive2,},
						.TMqttData[18] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113403002",.ValueType = eNActive3,},
						.TMqttData[19] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113404002",.ValueType = eNActive4,},
						.TMqttData[20] = &(T_MqttData){.DataType = eCollectionToData,.DataName = "321113405002",.ValueType = eNActive5,},
					}, 
				},
			},
		},
...
const char ServerIP[] = "119.167.163.7";   //"121.41.60.84";   //"119.167.163.7";//"58.210.240.122";////
...
const uint16 ServerPort = 6161;   // 6161;//6183;
//
...
static void init_topic(void)
{
	...
	// 3数据周期
	// memcpy(&topic_pub[3][0], ReportPara.cycleDataTopic, sizeof(ReportPara.cycleDataTopic));
	// 4数据变化
...
{
	RemessageF("CMD_DEVICE_UPGRADE", &topic_pub[9][0], "ds", "code", 200.0l, "msg", "SUCCESS");
...
void MQTT_Services(void)
{
	...
	if(nwy_get_msg_que(MqttResMessageQueue, &msg, 0xffffffff))   // 等待一秒
	{
		...
		else if(strstr(pType->valuestring, "CMD_DEVICE_SOCKET"))
		{   
			char temp[10] = {0};
			sprintf(temp, "%d", socket);
			RemessageF("CMD_DEVICE_SOCKET", &topic_pub[0][0], "s", "SOCKET", temp);
			cJSON_Delete(pRoot);
		}
...
void MQTT_connet(void)
{
	...
	/*code*/
	// 连接服务器
	memcpy(paho_mqtt_at_param.host_name, Serverpara[1].ServerIP, sizeof(Serverpara[1].ServerIP));
	paho_mqtt_at_param.port = Serverpara[1].ServerPort;
	...
	if(MQTTIsConnected(&paho_mqtt_client))
	{
		...
		{
			memset(paho_mqtt_at_param.topic, 0, sizeof(paho_mqtt_at_param.topic));
			memcpy(Sub_topic, &topic_pub[8 + i][0], 64);
