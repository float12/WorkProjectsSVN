    ...
    switch (pBuf[1])
    {
		...
		case 0xF0:
			...
			else if (pBuf[0] == 0xAA)//将所设置参数全部写到文件里
			{
				if (pBuf[8] == 1)
				{
					Serverpara[0].wCrc = CalCRC16((BYTE *)&Serverpara[0].ServerPort, (sizeof(ServerParaRam_t) - sizeof(Serverpara[0].wCrc)));

					Serverpara[1].wCrc = CalCRC16((BYTE *)&Serverpara[1].ServerPort, (sizeof(ServerParaRam_t) - sizeof(Serverpara[1].wCrc)));
					...
					nwy_ext_echo("\r\nwrite para file Serverpara[0].wCrc is%04x", Serverpara[0].wCrc);
					nwy_ext_echo("\r\nwrite para file Serverpara[1].wCrc is%04x", Serverpara[1].wCrc);
					fd = nwy_sdk_fopen(filepara_name, NWY_CREAT | NWY_RDWR );
					if(nwy_sdk_fwrite(fd,(char*)&Serverpara[0].ServerPort,(2*sizeof(ServerParaRam_t))) == (2*sizeof(ServerParaRam_t)))
					{
						{
							nwy_sdk_fclose(fd);
						}					  	
