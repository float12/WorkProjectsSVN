...
#endif
#define ADDR_CONFIG_VALUE1	0X01
#define ADDR_CONFIG_VALUE2	0X0F
...
//备注:   
//-----------------------------------------------
static WORD ReadXX24XXX_For_Addr(BYTE ChipNo, WORD Len, BYTE * pBuf)
{
  	BYTE Result,i;
	WORD Addr = 0xc000;
...
//备注:   
//-----------------------------------------------
static WORD WriteXX24XXX_For_Addr(BYTE ChipNo, WORD Len, BYTE * pBuf)
{
	BYTE Result = FALSE;
	WORD Addr = 0xc000;
...
{		
	...
	if( Type == 0 )
	{
		ReadXX24XXX_For_Addr(CS_SPI_256401_CONFIG_ADDR,  1, Buf );
	}
	else
	{
		ReadXX24XXX_For_Addr(CS_SPI_256402_CONFIG_ADDR,  1, Buf );
	}
}
//-----------------------------------------------
//函数功能: 检查EEPROM地址
...
BOOL api_CheckEEPRomAddr(BYTE Type)
{
	...
	if(Type & 0x01)
	{
		Result = ReadXX24XXX_For_Addr(CS_SPI_256401_CONFIG_ADDR , 1, Buf );
		if( (Buf[0] != ADDR_CONFIG_VALUE1) || (Result != TRUE) )//未配置地址时可能通信不上
		{
			return FALSE;
		}
	}
	...
	{
		#if( SLAVE_MEM_CHIP != CHIP_NO )
		Result = ReadXX24XXX_For_Addr(CS_SPI_256402_CONFIG_ADDR , 1, Buf );
		if( (Buf[0] != ADDR_CONFIG_VALUE2) || (Result != TRUE) )//未配置地址时可能通信不上
		{
			return FALSE;
		}
...
void api_FactoryInitEEPRomAddr(void)
{
	...
	//设置主存储区地址为00 锁定地址更改
	ENABLE_WRITE_PROTECT2_FOR_ADDR; //使能e2-1写保护
	api_Delay10us(5);
	Buf[0] = ADDR_CONFIG_VALUE1;
	WriteXX24XXX_For_Addr( CS_SPI_256401_CONFIG_ADDR, 1, Buf );//写第一片E2地址
	DISABLE_WRITE_PROTECT2;
	#if( SLAVE_MEM_CHIP != CHIP_NO )
	//设置主存储区地址为0e 锁定地址更改
	ENABLE_WRITE_PROTECT1_FOR_ADDR;//使能e2-2写保护
	api_Delay10us(5);
	Buf[0] = ADDR_CONFIG_VALUE2;
	WriteXX24XXX_For_Addr( CS_SPI_256401_CONFIG_ADDR, 1, Buf );//写第二片E2地址
	DISABLE_WRITE_PROTECT1;
	#endif //#if( SLAVE_MEM_CHIP != CHIP_NO )
}
