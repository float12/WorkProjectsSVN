...
{	
	...
	else if(MeterCurrentTypesConst == CURR_60A)
	{
	...
	else if(MeterCurrentTypesConst == CURR_20A)
	{
		...
		{
			//校表基本点初始化
			AdjustDivCurrent[0] = 20000;		//0         20A         IMax      
			AdjustDivCurrent[1] = 10000;        //1         10A         0.5IMax   
			AdjustDivCurrent[2] = 5000;         //2         5A          Ib        
			AdjustDivCurrent[3] = 2500;         //3         2.5A        50%Ib     
			AdjustDivCurrent[4] = 1000;         //4         1A          20%Ib     
			AdjustDivCurrent[5] = 500;          //5         0.5A        10%Ib     
			AdjustDivCurrent[6] = 250;          //6         0.25A       5%Ib      
...
//备注: 复位的校表数据
//-----------------------------------------------
void ResetSampleReg(DWORD Reg_Addr,BYTE len)
{
    DWORD Data;
	...
	WriteSampleReg( Reg_Addr, (BYTE *)&Data ,len);
}
//-----------------------------------------------
...
//备注:   
//-----------------------------------------------
void WriteEEpromSamplePara( eSampleType Type )			
{
	api_SetSysStatus(eSYS_STATUS_EN_WRITE_SAMPLEPARA);
...
//备注:   
//-----------------------------------------------
static void InitSamplePara( void )
{	
	UseSampleDefaultPara(eALL);

	WriteEEpromSamplePara(eALL);
...
// 备注:
//--------------------------------------------------
BOOL Adjust_UIK(float Vol, float Cur, BYTE j)
{
	...
	for (i = 0; i < 12; i++)
	{
		Link_ReadSampleReg(UA_EMU + j, (BYTE *)&RegisterU[j], 4);
		Link_ReadSampleReg(IA_EMU + j, (BYTE *)&RegisterI[j], 4);
	...
	if (memcmp(&RegisterU[j], &cmpval8[0], 4) != 0)
	{
		...
		Gain = (float)((SWORD)SampleAdjustReg.GSU[j].d / 32768.0);
	...
	if (memcmp(&RegisterI[j], &cmpval8[0], 4) != 0)
	{
		...
		Gain = (float)((SWORD)SampleAdjustReg.GSI[j].d / 32768.0);
		K2 = (float)(Ku * 32 * (1 + Gain));
		UIFactor.Ik[j] = K2;
	}
	else
	...
	// 写E2P
	api_SetSysStatus(eSYS_STATUS_EN_WRITE_SAMPLEPARA);
	Result = api_VWriteSafeMem(GET_SAFE_SPACE_ADDR(SampleSafeRom.SampleUIKSafeRom.Uk[0]), sizeof(TSampleUIK), (BYTE *)&UIFactor);
	api_ClrSysStatus(eSYS_STATUS_EN_WRITE_SAMPLEPARA);
...
BOOL api_UI_Kfor8306(BYTE *Buf)
{
	...
	DWORD PowerValue = 11000;
	#elif (cCURR_TYPES == CURR_6A)
	DWORD PowerValue = 165;
	#elif (cCURR_TYPES == CURR_60A)
	DWORD PowerValue = 550;
	#endif
	#elif (HARDWARE_TYPE == SINGLE_PHASE_BOARD)//直接改MAX_PHASE会导致编译很多变量wei'ding'yi
	BYTE PhaseNum = 1;
	DWORD PowerValue = 550;
	#endif
	...
	for (i = 0; i < PhaseNum; i++)
	{
		...
		// 校准判断条件三相：220V电压，100A电流，功率0.5，单相：220V电压，1.5A电流，功率0.5
		if ((dvval / 1000) == 220 && (dpval / 1000) == PowerValue)
		{
			Result = Adjust_UIK((float)((dvval / 1000)), (float)((float)dival / 10000), i);
			if (Result == FALSE)
			{
				return FALSE;
			}
...
static void UIP_K(BYTE *Buf)
{
	...
	WriteSampleReg( PStart_EMU, td.b ,2);
	...
	WriteSampleReg( PRTH1H_EMU, SampleCtrlReg.PRTH1H.b ,2);
	WriteSampleReg( PRTH2H_EMU, SampleCtrlReg.PRTH2H.b ,2);
	WriteSampleReg( PRTH1L_EMU, SampleCtrlReg.PRTH1L.b ,2);
	WriteSampleReg( PRTH2L_EMU, SampleCtrlReg.PRTH2L.b ,2);

	WriteEEpromSamplePara(eCORR);
	WriteEEpromSamplePara(eCRTL);
}
...
static void Gain_Adjust(BYTE *Buf)
{
	...
	BYTE PhaseNum = 1;
	#elif ((cCURR_TYPES==CURR_500A) || (cCURR_TYPES==CURR_6A))
	BYTE PhaseNum = 3;
	#endif
	#elif (HARDWARE_TYPE == SINGLE_PHASE_BOARD)//直接改MAX_PHASE会导致编译很多变量wei'ding'yi
	BYTE PhaseNum = 1;
	#endif
	UIP_K(Buf);//电压电流系数校正
    SampleSpecCmdOp(0x100,CmdWriteOpen);//UIP_K函数里面有打开写保护的代码，所以这里重新关闭写保护    
	...
	for (i = 0; i < PhaseNum; i++)
	{
		...
		ResetSampleReg( GPA_EMU+i ,2);
		ResetSampleReg( GQA_EMU+i ,2);
		ResetSampleReg( GSA_EMU+i ,2);
		ResetSampleReg( GFPA_EMU+i ,2);
		ResetSampleReg( GFQA_EMU+i ,2);
		ResetSampleReg( GFSA_EMU+i ,2);
		ResetSampleReg( GSUA_EMU+i ,2);
		ResetSampleReg( GSIA_EMU+i ,2);
	}
	...
	for ( i = 0; i < 12; i++)
	{
		...
		{
			Link_ReadSampleReg(UA_EMU+j,RegisterU[j].b ,4);
			
			Link_ReadSampleReg(IA_EMU+j,RegisterI[j].b ,4);
	...
	for(i=0;i<PhaseNum;i++)
	{
		...
		SampleAdjustReg.GSU[i].d = td.w[0];
		
		WriteSampleReg(GSUA_EMU+i,td.b,2);
		if(Err_DataI >= 0) 
		{
		...
		SampleAdjustReg.GSI[i].d = td.w[0];
		
		WriteSampleReg(GSIA_EMU+i,td.b,2);
	}
	CLEAR_WATCH_DOG;
}
...
static void PH_Adjust(BYTE Type, BYTE *Buf)
{
	...
	for(i=0; i<PhaseNum; i++)
	{
		...
		Link_ReadSampleReg(PHSIA_EMU+i,Originaltd[i].b,4);
		ResetSampleReg( PHSIA_EMU+i ,4);
		ResetSampleReg( PHSUA_EMU+i ,4);
		...
		else
		{
		  	if( Type == 0 )//大电流
			{
				if( labs(RemoteValue.I[i].l/10) > (dwBigStandardCurrent*IRegionUPConst*2/100))
				{
	...
	for ( i = 0; i < 12; i++)
	{
		for ( j = 0; j < PhaseNum; j++)
		{
			Link_ReadSampleReg(PA_EMU+j,RegisterP[j].b,4 );
	...
	for(i=0;i<PhaseNum;i++)
	{
		...
		if( Type == 0 )//大电流
		{
			if( labs(RemoteValue.I[i].l/10) > (dwBigStandardCurrent*IRegionUPConst*2/100)) //最大电流相角
			{
				td.d = (Originaltd[i].d&0x000FFFFF)+((DWORD)(0x202-(Err_DataP/0.008789)*1024*1024));
				SampleAdjustReg.PHSI[i].d = td.d;
				WriteSampleReg(PHSIA_EMU+i,td.b,4);
			}
			else
			{
			...
			//----增加同步采样波形缓存通道相位校正----
			SampleSpecCmdOp(0x15B, CmdWriteOpen);
			// 扩位校正，电压通道偏移值设置为0x200,同计量通道一致
            if((td.d & 0x3FF) >= 0x200)
            {
				SampleAdjustReg.PHS_U_WAVE[i].d = 0;
				SampleAdjustReg.PHS_I_WAVE[i].d = (td.d & 0x3FF) - 0x200;
            }
			else
			{
				SampleAdjustReg.PHS_U_WAVE[i].d = 0x200-(td.d & 0x3FF);
				SampleAdjustReg.PHS_I_WAVE[i].d = 0;
			}
			// 第一级校正，填写与计量通道相同的偏移校正值
			WriteSampleReg(PHS_UA_WAVE + i, SampleAdjustReg.PHS_U_WAVE[i].b, 2);
			WriteSampleReg(PHS_IA_WAVE + i, td.b, 2);
			SampleSpecCmdOp(0x15B, CmdWriteClose);
			api_UI_Kfor8306(Buf);
		}
		else//小电流
		{
...
//备注:   
//-----------------------------------------------
static void IZero_Adjust(void)
{
	BYTE i;
	TFourByteUnion RegisterI, td, Iaverage;
	QWORD Sum=0;
    td.l = 0;


	ResetSampleReg(IAOS_EMU, 2);
	...
	for (i = 0; i < 12; i++)
	{
		Link_ReadSampleReg(IA_EMU, RegisterI.b, 4);
		Sum += ((QWORD)RegisterI.d);
		CLEAR_WATCH_DOG;
		api_Delayms(260);
	}
	RegisterI.d = (DWORD)(Sum/12);

	//参考于：锐能微RN8302、RN8302B、RN7302应用笔记v1.4-5.3 有效值 Offset 校正方法
	//计算公式：IA_OS=-Ix_average^2/2^14。
	//读电流有效值寄存器求平均值Ix_average，平方后取其补码，除以2^14取低两个字节
	//计算电流有效值offset
	RegisterI.d *= RegisterI.d;//电流寄存器平均值取平方
	if(RegisterI.d >= 0x20000000)
	{
		return;
	}
	Iaverage.d = 0xFFFFFFFF - RegisterI.d + 1;//取补码
	td.l = (SDWORD)(Iaverage.d / 0x4000);

	SampleAdjustReg.IOS[0].d = td.w[0];
	WriteSampleReg(IAOS_EMU, td.b, 2);
}
//-----------------------------------------------
...
//备注:   
//-----------------------------------------------
static void Offset_Adjust(BYTE *Buf)
{
	...
	for (j = 0; j < MAX_PHASE; j++)
	{
		...
		SampleAdjustReg.POS[j].w[0]=RegisterI[j].w[0];
		WriteSampleReg(PAOS_EMU + j, RegisterI[j].b, 2);
	}
}
...
static void EDT_Adjust( void )//!!!!!!
{
    ...
    {
        DumpData.d = 1;
        if( WriteSampleReg(EDT_CALI, DumpData.b,4) == TRUE )///长度未修改!!!!!!
        {
            break;
        }
        api_Delayms(50);	
    ...
    {
        DumpData.d = 0;
        Link_ReadSampleReg( EDT_STATUS, DumpData.b ,4);///长度未修改!!!!!!
        if( DumpData.d != 0 )
        {
            FinishFlag = TRUE;
            break;
    ...
    if( FinishFlag == TRUE )
    {
        for( i=0; i<EDT_PARAM_READ_RETRY_MAX; i++ )
        {
            for( j=0; j<(8*MAX_PHASE); j++ )
            {
                DumpData.d = 0;
                wAddr = EDT_PARAM + j;
                if( Link_ReadSampleReg(wAddr, DumpData.b,4) == FALSE )///长度未修改!!!!!!
                {
                    break;
                }
...
{	
	...
	InitSampleCaliUI();
	//关闭校表寄存器写保护
	SampleSpecCmdOp(0x100,CmdWriteOpen);
	...
	switch( AdjType )
	{
		...
		case 0x51://大电流，0.5L(L)
			PH_Adjust(0,Buf); //通道相位校正
			break;
			
		case 0x52://小电流，0.5L(L)
			PH_Adjust(1,Buf); //功率相位校正
			break;
			
		case 0x02://5%Ib，1.0(L)
			Offset_Adjust(Buf); //有功功率偏置修正
			break;
	...
	{
		WriteEEpromSamplePara(eADJUST);//校表系数备份处理
	}
	CheckSampleChipReg();//防止写入计量芯片失败影响下一步校表
	GetSampleChipCheckSum();//读校验和
	//关闭校表寄存器写保护
	SampleSpecCmdOp(0x100,CmdWriteClose);
	return TRUE;
...
BOOL api_ProcSampleCorr(BYTE Operation, WORD Addr, BYTE Len, BYTE *Buf)
{
	...
	if( Operation == WRITE )
	{
		SampleSpecCmdOp(SampleAddr,CmdWriteOpen);
		Result = WriteSampleReg((WORD)SampleAddr,(BYTE*)&tdw,Len);
		SampleSpecCmdOp(SampleAddr,CmdWriteClose );
		if( Result == FALSE )
		{
			return FALSE;
		}
		...
		{		
			for(i=0;i<((sizeof(TSampleAdjustReg)-sizeof(DWORD))/sizeof(DWORD));i++)
			{
				*Buf = SampleAdjustAddr[i][0]%0x100;
				Buf += 1;
				memcpy(Buf,(BYTE*)&SampleAdjustReg+i*sizeof(DWORD),sizeof(DWORD));
				Buf += 4;
			}
		}
		else
		{
			Link_ReadSampleReg( Addr,Buf,Len );
		}
