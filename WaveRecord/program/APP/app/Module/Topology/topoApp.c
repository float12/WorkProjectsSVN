...
//	TopoPWMEnable(FALSE);	// 调试时使用，防止打断点时一直发信号
	...
	// 增加拓扑识别的幅值判断
	if(  ((DWORD)(TopoIdentiSignalData.SignalStrength1*1000) < 150)
	  || ((DWORD)(TopoIdentiSignalData.SignalStrength2*1000) < 150)
	)
	{
		return;
	}
...
//备注: 
//-----------------------------------------------
void api_TopWaveHsdcBufTransfer(void)
{
	uint16_t ilen=0, offset=0, offset1, offset2;
	uint16_t i,j;
	int32_t tmp2=0;
	float tmp3 = 0.0f;
	
	sTopoWave.CycDot = 128;
	ilen = sTopoWave.CycDot / 2;
	static DWORD CrcWrongCnt = 0;
	static DWORD ZeroWrongCnt = 0;
	static DWORD enterCnt = 0;
	enterCnt++;
	// 半波数据CRC校验
	...
	sTopoWave.RxCnt++;
	offset2 = (sTopoWave.RxCnt - 1) * ilen;
	for (i = 0; i < 3; i++)
	{
		// 取ABC电压通道波形数据
		offset = ilen * 3 * i;
		...
		{
			offset1 = 3 * j;
			memcpy(&tmp2, &sTopoWave.RxBuf[offset1 + offset], 3);
			TopoStrSwap(&tmp2, 3);
			tmp2 = TopoTrans3bTo4b((uint8_t *)&tmp2);
			tmp3 = (float)tmp2;
			memcpy(&sTopoWave.Ubuf[i][j + offset2], &tmp3, 4);
		}
	}
	for(i = 0; i<3; i++)
	{
		...
		for(j = 0; j<ilen; j++)
		{
			...
			tmp2 = TopoTrans3bTo4b((uint8_t *)&tmp2);
			tmp3 = (float)tmp2;
			memcpy(&sTopoWave.Wbuf[i][j + offset2], &tmp3, 4);
		}
	}
	...
	{
		sTopoWave.RxCnt = 0;
		TopoFrameCheck((uint32_t *)&sTopoWave.Wbuf[0][0], (uint32_t *)&sTopoWave.Wbuf[1][0], (uint32_t *)&sTopoWave.Wbuf[2][0], 1, sTopoWave.CycDot);
	}
}
#else
...
// 备注:
//-----------------------------------------------
extern void PrepareAndSendWaveData(void);
extern BYTE FirstPhaseBuffer[SAVE_TO_SD_CYCLE_NUM * WAVE_FRAME_SIZE];
...
void TopoPowerUp(void)
{
	...
	if( ( Buf[eEVENT_TOPU_NO/8] & (0x01<<(eEVENT_TOPU_NO%8)) == 0 )
	  &&(TopoIdentiResultRAM.Num != 0)
	)
	{
		TopoIdentiResultEventDataClear();
	}
...
void TopoSecondTask(void)
{
	...
	if(TopoSignalValidFlag == 0x55AA55AA)
	{
		...
		if( (api_CheckClock(&TopoIdentiSignalData.IdentifTime) == FALSE)
		  ||(TopoIdentiSignalData.SignalPhase > 4)
		  ||(TopoIdentiSignalData.SignatureCodeLen != 2)
		)
		{
	...
	if(api_CheckClock(&ModulatSignalInfoRAM.Time) == TRUE)
	{
		...
		if( (RelativeSec1 == RelativeSec2 ) 
		 || ( (RelativeSec1 > RelativeSec2) && (RelativeSec1 - RelativeSec2 <= 3 ) )	//考虑跳秒的情况
		)
		{
...
BOOL api_TopoSignalSendStart(void)
{
	if( (api_GetSysStatus(eSYS_STATUS_POWER_ON) == FALSE)
	 || (api_GetSysStatus(eSYS_STATUS_TOPOING) == TRUE)
	 || (TopoSignalSendIntervalCnt < TOPO_SIGNAL_SEND_INTERVAL)
	)
	{
		return FALSE;
	...
	switch(ClassIndex)
	{
		case 0:
			if( (pData[0] != Structure_698) 
				||(pData[1] != 6)
				||(pData[2] != Long_unsigned_698) 
				||(pData[5] != Long_unsigned_698)
				||(pData[8] != Long_unsigned_698)
				||(pData[11] != Bit_string_698)
				)
			{
			...
			if( (TmpModulatSignalInfo.SignatureCodeLen != ModulatSignalInfoRAM.SignatureCodeLen)
			 || (memcmp(ModulatSignalInfoRAM.SignatureCode,TmpModulatSignalInfo.SignatureCode,TmpModulatSignalInfo.SignatureCodeLen) != 0)
			)
			{
				ParaIsEqual = FALSE;
			}
...
static WORD FillTopoIdentiResult645(TopoIdentiResult_t  *pTopoIdentiResult,BYTE *OutBuf,BYTE MaxSignalCnt)
{
	...
	if( (pTopoIdentiResult == NULL) 
	 || (OutBuf == NULL) 
	 || (MaxSignalCnt > TOPO_CC_MAX_NUM_IN_IDENTI_RESULT )
	)
	{
		return 0;
	}
