...
void api_RtcTask(void)
{
	...
	if( api_GetTaskFlag(eTASK_RTC_ID,eFLAG_SECOND) == TRUE )
	{
		...
		if( RunLedTimer >= 1 )
		{
			...
			if(WriteToBufSkippedHalfWave > 0)
			{
				LIGHT_RUN_STOP;
			}
			else
...
{
	DWORD		dwAddr;
	TLostPowerTime LostPowerTimeTmp;

	dwAddr = GET_SAFE_SPACE_ADDR( LostPowerTime );
	api_VReadSafeMem( dwAddr, sizeof(TLostPowerTime) , LostPowerTimeTmp.Time );	
	memcpy( (BYTE*)pPowerDownTime, LostPowerTimeTmp.Time, sizeof(TRealTimer));
}
