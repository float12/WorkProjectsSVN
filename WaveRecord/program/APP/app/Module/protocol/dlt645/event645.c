...
#if( SEL_TOPOLOGY == YES )
const DWORD TopoEventTable[3] = 
{
    //6
    0x00022220,			// 事件记录序号对应OAD
    0x00021E20,			// 事件开始时间对应OAD
    // 0x00022020,			// 事件结束时间对应OAD
    // 0x00022420,			// 事件发生源对应OAD
    // 0x00020033,			// 事件上报状态对应OAD
    0X0602833E,			// 特征电流识别结果OAD
};
const DWORD TopoEventSignalTable[3] = 
{
    //6
    0x00022220,			// 事件记录序号对应OAD
    0x00021E20,			// 事件开始时间对应OAD
...
static const TEvent645Obj_t Event645Obj[] =
{
    ...
    #if( SEL_TOPOLOGY == YES )  // 拓扑识别结果
	{0x0908,		0x3E030200,		sizeof(TopoEventTable)/sizeof(DWORD),	        TopoEventTable	},
	#endif
};								
...
//            else
//            {
//                memcpy(Buf,TopoEventTable,sizeof(TopoEventTable) );
//                DLT698RecordPara.OI = OI.w;                         //OI数据
//                DLT698RecordPara.Ch = 0;                            //不清上报 固定通道传输
//                DLT698RecordPara.pOAD = Buf;                        //pOAD指针
//                DLT698RecordPara.OADNum = sizeof(TopoEventTable)/4; //OAD个数
//                DLT698RecordPara.eTimeOrLastFlag = eNUM_FLAG;       //选择次数
//                DLT698RecordPara.TimeOrLast = pDI[0];               //绝对时间数
//
//                Len = api_ReadEvent645RecordByNo((TDLT698RecordPara *)&DLT698RecordPara, MAX_APDU_SIZE, OutBuf );
