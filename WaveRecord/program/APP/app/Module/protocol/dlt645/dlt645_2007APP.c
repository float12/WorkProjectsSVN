...
MessageTransPara_t MessageTransParaRam;
extern MessageTransPara_t MessageTransParaConst;
//-----------------------------------------------
//				内部函数声明
//-----------------------------------------------
...
//				函数定义
//-----------------------------------------------
extern WORD JudgeBroadcastMeterNo(BYTE *pBuf);
...
/*	if( ControlByte == eCONTROL_RECE_03 )
	...
	else if( ControlByte == eCONTROL_RECE_14 )
	{
		...
		{
			// 特殊扩展规约，允许非厂内设置
			if( (gPr645[PortType].dwDataID.d == 0x0400041F) || (gPr645[PortType].dwDataID.d == 0x04000420))
			{
				return DLT645_OK_00;
			}
	...
	else if( ControlByte == eCONTROL_RECE_1C )
	{
		...
		if(gPr645[PortType].bLevel == CLASS_2_PASSWORD_LEVEL)
		{
			...
			if( api_GetRunHardFlag(eRUN_HARD_MAC_ERR_FLAG) == TRUE )
			{
				return 0x8002;//关闭预付费后原有宏被屏蔽所以直接改为数值
			}
		}
...
WORD ReviseTime0X08( eSERIAL_TYPE PortType )
{
	...
	if( api_GetRunHardFlag( eRUN_HARD_ALREADY_BROADCAST_FLAG ) == FALSE )//广播校时一天一次
	{
		...
		if( (api_GetRunHardFlag(eRUN_HARD_ERR_RTC_FLAG)==FALSE) && (api_GetRunHardFlag(eRUN_HARD_CLOCK_BAT_LOW)==FALSE) )	//时钟正常进行判断
		{
			//零点前后5分钟内不允许广播校时
...
//参数  :    TSerial *p[in]
//
//返回值:     BOOL （TRUE-需要做转发或采集接收处理）
//备注内容  :  无
//-----------------------------------------------
BOOL Pre_Dlt645(TSerial *p)
{
	return FALSE; // 目前不需要预处理
	// BYTE i;
	// BOOL bRc = FALSE;
	// BYTE IsMyMeteraddr;

	// if( (JudgeBroadcastMeterNo(&(p->ProBuf[1])) || JudgeRecMeterNo_645(&(p->ProBuf[1]))) )
	// {
	// 	IsMyMeteraddr = TRUE;
	// }
	// else
	...
	// 	if( p == &Serial[i] )
	// 	{
	// 		if (IsUpCommuFrame(PRO_NO_SPECIAL,Serial[i].ProBuf))
	// 		{
	// 			if ( IsJLing(i) )
	// 			{
	// 				JLWriteRx(i,Serial[i].ProBuf,Serial[i].ProStepDlt645);
	// 				bRc = TRUE;
	// 			}
	// 			else
	// 			{
	// 				GyRxMonitor(i,PRO_NO_SPECIAL);
	// 				bRc = TRUE;
	// 			}
	// 		} 
	// 		else if( ((MessageTransParaRam.AllowUnknownAddrTrans == TRUE) && (IsMyMeteraddr == FALSE)) 
	// 			  || IsInSysMeters(&p->ProBuf[1])
	// 		)
	// 		{
	// 			CollectorWriteTx(i,Serial[i].ProBuf,&p->ProBuf[1],Serial[i].ProStepDlt645);
	// 			bRc = TRUE;
	// 		}  
	// 		break;
	// 	}
...
//备注: 
//-----------------------------------------------
void api_CheckMessageTransPara(void)
{
	// 监视当前参数
	if (lib_CheckSafeMemVerify((BYTE *)&MessageTransParaRam, sizeof(MessageTransPara_t), UN_REPAIR_CRC) == FALSE)
	{
		// 由EEProm恢复参数
		if (api_VReadSafeMem(GET_SAFE_SPACE_ADDR(ProSafeRom.MessageTransPara), sizeof(MessageTransPara_t), (BYTE *)&MessageTransParaRam) != TRUE)
		{
			memcpy((BYTE *)&MessageTransParaRam, (BYTE *)&MessageTransParaConst, sizeof(MessageTransPara_t));
			api_SetError(ERR_MESSAGE_TRANS_DATA);
		}
	}
	else
	{
		api_ClrError(ERR_MESSAGE_TRANS_DATA);
	}
}
...
//备注: 
//-----------------------------------------------
void api_InitMessageTransPara(void)
{
	memcpy((BYTE *)&MessageTransParaRam, (BYTE *)&MessageTransParaConst, sizeof(MessageTransPara_t));
	api_VWriteSafeMem( GET_SAFE_SPACE_ADDR( ProSafeRom.MessageTransPara), sizeof(MessageTransPara_t), (BYTE*)&MessageTransParaRam );
}
