...
const BYTE T_SearchMeterList[] =  //搜表结果
{
	Array_698,			 		0XFF,
	Structure_698,				7,
	TSA_698,		 			7,
...
static const BYTE T_SchedMeterResult[] = //6002 属性6 周期结果
{
	Long_unsigned_698,			2,
};

static const BYTE T_SchedMeterPara[] = //6002 属性8 搜表参数
{
	Structure_698,      4,
	Boolean_698,        1,
	Boolean_698,        1,
...
static const BYTE T_SchedMeterCycle[] = //6002 属性9 每天周期搜表参数配置
{
	Array_698,			 		0XFF,
	Structure_698,				2,
	Time_698,		 			1,
...
static const BYTE T_SchedMeterStatus[] = //6002 属性10 搜表状态
{
	Enum_698, 					1,
};

static const BYTE T_SchedMeterStrategy[] = //6002 属性11 搜表策略
{
	Structure_698,              3,
	Enum_698,                   1,
	Boolean_698,                1,
...
static const BYTE T_RogwskiSingle[]		= //单相修正系数
{
    Array_698,				   0xff,
	Structure_698,             3,
    Long_unsigned_698,         2,
...
static const BYTE T_ModuleVeriInfo[] = 	//模组版本信息 40310300
{ 
	Structure_698,				    7,
	Unsigned_698,					1,	//模组逻辑地址
	Visible_string_698,				4,	//厂商代码
...
static const BYTE T_ModuleVeriInfoLSXQ[] = 	//模组版本信息 40310301  罗氏线圈单独处理
{ 
	Structure_698,				    7,
	NULL_698,						0,	//模组逻辑地址
	Visible_string_698,				4,	//厂商代码
...
static const TCommonObj ParaMeterObj[] =
{
    ...
    #endif
	{  0x4031,  &SU_00,   	T_ModuleVeriInfo},//电能表模组列表，升级读取信息
	...
	{  0x4908,  &SU_2S,   	T_UNDoubleLong  },
#endif
    #if (SEL_TOPOLOGY == YES )
    {  0x4E06,	&SU_00, 	NULL },             //调制信号信息
	{  0x4E07,	&SU_00, 	NULL },//拓扑识别结果
    #endif
    #if (SEL_SEARCH_METER == YES )
	{  0x6002,	&SU_00, 	T_SearchMeterList },//搜表结果
    #endif
};	

//默认生产日期2022-8-15 12：00：00
 const BYTE ProduceDate[7]={230,7,8,15,12,0,0};
//-----------------------------------------------
...
static WORD AddTagPositionInfo( BYTE *pOAD, BYTE *InBuf, WORD OutBufLen, BYTE *OutBuf )
{
	...
	if(ClassIndex == 0)
	{
		if( OutBufLen < 31 )//如果缓冲大小不够，则不进行操作
		{
			return 0;
		}
		Len = 31;
		...
		*(OutBuf++) = 3;
		...
		OutBuf +=12;
		Result = GetRequestAddTag( T_PositionInfo1, InBuf+7, OutBuf );
		if( Result == 0x8000 )
		{
			return 0x8000;
		}
		OutBuf +=12;
		Result = GetRequestAddTag( T_DoubleLong, InBuf+14, OutBuf );
		if( Result == 0x8000 )
		{
			return 0x8000;
		}
	...
	{
		if( OutBufLen < 12 )//如果缓冲大小不够，则不进行操作
		{
			return 0;
		}
		Len = 12;
	...
	else
	{
		...
		Result = GetRequestAddTag( T_DoubleLong, InBuf, OutBuf );
		if( Result == 0x8000 )
		{
			return 0x8000;
		}
...
//备注内容: 罗氏线圈修正系数
//--------------------------------------------------
static WORD AddTagRogwskiTable( BYTE *pOAD, BYTE *InBuf, WORD OutBufLen, BYTE *OutBuf )
{
	BYTE ClassIndex,i;
	WORD Result,Len;
	...
	if( ClassIndex == 0 )
	{
		if( OutBufLen <  ((2+11*3)*3+2))//如果缓冲大小不够，则不进行操作
		{
			return 0;
		}
		Len = ((2+11*3)*3+2);
		...
		*(OutBuf++) = 3;
		...
		TagPara.Array_Struct_Num = 3;//加TAG按照实际个数进行添加
		
		for( i=0; i < 3; i++ )
		{
			Result = GetRequestAddTag( T_RogwskiSingle, InBuf, OutBuf );
			if( Result == 0x8000 )
			{
				return 0x8000;
			}

			InBuf += 18;//输入buf按照最大长度进行跳转 请注意
			OutBuf += Result;
		}
	}
	else
	{
		if( OutBufLen < (2+11*3))//如果缓冲大小不够，则不进行操作
		{
			return 0;
		}
		
		TagPara.Array_Struct_Num = 3;
...
//备注内容: 不含tag
//--------------------------------------------------
static WORD GetProMeterParaData(BYTE Ch, BYTE *pOAD, BYTE Sch, BYTE *pBuf ,WORD BufLen)
{
	TTwoByteUnion OI;
	...
	// else if( ((OI.w >= 0x4600)&&(OI.w <= 0x4603))||(OI.w==0x4702) )//充电桩单独模块处理
	// {
    //     return Charging698(eChargingGet,pBuf,OI.w,ClassAttribute,api_GetChargingPileCanID(LinkData[Ch].ServerAddr),ClassIndex);
	// }
	...
	switch( ClassAttribute )
	{
		case 2:
			switch( OI.w )
			{
				...
				case 0x4004://地理位置信息
					...
					{
						memcpy(pBuf,pBuf+(ClassIndex - 1)*7 ,7);
					}
					else
					{
						memcpy(pBuf,pBuf+14 ,4);
					}
					pBuf += Len;
					break;
					//case 0x4007://LCD参数
				...
				// 	// Len = api_ReadDIPara(0,pBuf);
				// 	// pBuf += Len;

				// 	break;
                #if (SEL_TOPOLOGY == YES )
                case 0x4E07:
                    Len = api_GetTopoIdentResult698(ClassIndex,BufLen,pBuf);
                    if( Len == 0 )
                    {
                        return 0x8000;
                    }
                ...
                #if (SEL_SEARCH_METER == YES )
				case 0x6002://搜表结果
                    Len = api_GetSchedMeter698(ClassIndex, pBuf , (BYTE*)&Num);
                    if( Len == 0x8000 )
                    {
                        return 0x8000;
                    }
		...
		case 3:
			if(OI.w == 0x4031)
			{
				...
				{
					return 0x8000; // 不支持扩展模组
				}
			}
			else if( OI.w == 0x4106 )//计量规格
			{
		...
		case 4:
            ...
            {
                Len += api_GetTopoModulatSignalInfo698(ClassIndex,pBuf);
                if( Len == 0 )
                {
                    return 0x8000;
                }
		...
		case 6:
			if(OI.w == 0x6002) //所有搜表结果记录数
			{
				...
				wNum = 0;
				wNum = (WORD)api_GetMeterNumInSys();
				memcpy(pBuf,&wNum,2);
				pBuf += 2;
			}
            else
		...
		#if (SEL_SEARCH_METER == YES )
        case 8:
			if(OI.w == 0x6002) // 搜表属性8
			{
                if( ClassIndex > 4 )
				{
					return 0x8000;
				}
                Len = api_GetSchedMeterPara698(ClassIndex ,pBuf );
                if( Len == 0x8000 )
                {
                    return 0x8000;
                }
			...
			break; 
        case 9:
			if(OI.w == 0x6002) // 每天周期搜表参数配置
			{
                if( ClassIndex > tCycleSInfoRom.CycleNum )
				{
					return 0x8000;
				}
                Len = api_GetSchedMeterCycle698( ClassIndex,pBuf , (BYTE*)&Num);
                if( Len == 0x8000 )
                {
                    return 0x8000;
                }
        ...
        case 11:
        	if(OI.w == 0x6002) // 搜表策略
			{
                ...
                Len = api_GetSchedMeterStrategy698(ClassIndex ,pBuf );
                if( Len == 0x8000 )
                {
                    return 0x8000;
                }
...
static WORD AddTagMeterPara(BYTE *pOAD, BYTE Sch, BYTE *InBuf, WORD OutBufLen, BYTE *OutBuf )
{
	...
	{
        return Charging698AddTag(ClassAttribute,OI.w,InBuf,OutBuf,ClassIndex);
	}
	...
	switch( ClassAttribute )
	{
		case 2://属性2
			...
			else if(OI.w == 0x4033)
			{
				...
				return GetRequestAddTag( Tag, InBuf, OutBuf );	
			}
			#if (SEL_SEARCH_METER == YES )
			else if(OI.w == 0x6002)
			{
				if( ClassIndex > api_GetMeterNumInSys() )
				{
					return 0x8000;
				}
				Tag = ParaMeterObj[Sch].Type2;
		...
		case 7:
			if(OI.w == 0x4033)
			{
				return (AddTagRogwskiTable(pOAD,InBuf, OutBufLen,OutBuf));
			}
            else
			{
				return 0x8000;
        ...
        case 8:
			if(OI.w == 0x6002)//搜表属性8
			{
				...
				Tag = &T_SchedMeterPara[2*ClassIndex];
				Tag +=GetTagOffsetAddr( ClassIndex, Tag);
				Lenth = GetTypeLen( eTagData, Tag );
				if( Lenth == 0x8000 )
                {
        ...
        case 11:
        	if(OI.w == 0x6002)//搜表策略
			{
				...
				Tag = &T_SchedMeterStrategy[2*ClassIndex];
				Tag +=GetTagOffsetAddr( ClassIndex, Tag);
				Lenth = GetTypeLen( eTagData, Tag );
				if( Lenth == 0x8000 )
                {
...
{	
	...
	lib_ExchangeData(OI.b,pOAD,2);
	if( ((OI.w >= 0x4600)&&(OI.w <= 0x4603))||(OI.w==0x4702) )
	{
	...
	{
		Lenth = GetProMeterParaData( Ch,pOAD, Sch, Buf, OutBufLen);
		if( Lenth == 0x8000 )
		{
			return Lenth;
		}
	...
	switch( ClassAttribute )
	{
		case 2://总及费率最大需量数组
			if( OI.w == 0x4004 )//地理位置信息
			{
				...
				if( ClassIndex == 0 )
				{
					Len.w = ( DataType == eData) ? 18 : 31 ;
				}
				else if( ClassIndex < 3 )
				{
					Len.w = ( DataType == eData) ? 7 :12 ;
				}
				else
				{
