...
//------以下配置可根据情况修改------
const WORD SoftVersionConst = 0xff33;   //前两位代表省份，第三位代表年，第四位代表招标或送测月份
const BYTE SelDivAdjust = YES;			//是否采用分段校准方法  必须一直为YES
const BYTE SelMChipVrefgain = YES;		//是否开启计量芯片自身片内温补
#if(BOARD_TYPE != BOARD_HC_SEPARATETYPE)//现在只有智能表有辅助电源
const BYTE SelEESoftAddrConst = NO;			//EE是否使用软件地址，YES为软件地址，NO为硬件地址
#else
const BYTE SelEESoftAddrConst = YES;		//EE是否使用软件地址，YES为软件地址，NO为硬件地址
#endif
...
const TDIPara	DIParaconst = {50,1,1};		
#endif
//-----------------------------------------------
//			protocol 配置参数
//-----------------------------------------------
...
const DWORD METER_3P3W_MAX = 130000;		//130v
const DWORD METER_3P3W_MIN = 72000;			//72v
const DWORD METER_3P3W_UB_LIMIT = 3000;		//3v
...
const WORD Def_MaxError = 30;
			...
			const DWORD dwMeterBasicCurrentConst = 1500; //基本电流，3位小数
			const DWORD dwMeterMaxCurrentConst = 6000; //最大电流，3位小数
			const WORD IRegionUPConst = 73;				//相位分段区间上限73%
			const WORD IRegionDownConst = 67;			//相位分段区间下限67%
	...
	#elif( cCURR_TYPES == CURR_20A )	
			const DWORD dwMeterBasicCurrentConst = 5000;//基本电流，3位小数
			const DWORD dwMeterMaxCurrentConst = 20000; //最大电流，3位小数
			const WORD IRegionUPConst = 73;				//相位分段区间上限73%
			const WORD IRegionDownConst = 67;			//相位分段区间下限67%
			//const WORD SimpleCurrConst = 20;
			//const WORD IRegionConst = 500;		//500%Ib
			...
			//const WORD SimpleVolConst = 721;		
			//const WORD wSampleVolGainConst = 2; //采样芯片电压增益倍数		
	#elif( cCURR_TYPES == CURR_500A )	
			//const WORD wMeterBasicCurrentConst = 5000;//基本电流，3位小数 !!!!!!
			//const DWORD dwMeterBasicCurrentConst = 100000;//基本电流，3位小数
			//const DWORD dwMeterMaxCurrentConst = 500000; //最大电流，3位小数
			const WORD IRegionUPConst = 73;				//相位分段区间上限73%
			const WORD IRegionDownConst = 67;			//相位分段区间下限67%
			const DWORD dwMeterBasicCurrentConst = 100000; //基本电流，3位小数
			const DWORD dwMeterMaxCurrentConst = 500000; //最大电流，3位小数
			const WORD SimpleCurrConst = 80;	//80mv
			#if( cMETER_TYPES == METER_3P3W )	
			const WORD SimpleVolConst = 1835;	//68K * 8 + 1K
			#else
			//const WORD SimpleVolConst = 3774;	//33K * 8 + 1K
			#endif
...
const DWORD SampleUIPGain[11] = //照顾小信号 功率扩大100倍
{
	0x0001D656,	//UA	带积分器
	0x0001D656,	//UB
	0x0001D656,	//UC
...
const DWORD SampleCtrlDefData[18] = 
{	
	0x00043210,//CFCFG			带积分器
	0x00004000,//EMMIE
	0x00408000,//EMUCFG 
	0x00007777,//EMUCON
	0x00000010,//ZXOTCFG,电压夹角及测频计算阈值选择为 ZXOTU
	0x00000080,//AUTODC_EN
	0x00002A95,//ADCCTL 电压2倍增益、电流4倍
	0x00004DD2,//HFCONST
	0x0000002E,//PSTART
	0X00000000,//MODSEL
	0x00000000,//PRTH1L_EMU
	0x00000000,//PRTH1H_EMU
	0x00000000,//PRTH2L_EMU
	0x00000000,//PRTH2H_EMU
	0x00000000,//SAGCFG_EMU
	0x00200300,//EMUCFG3 采用功率启动方式2
	0x00000005,//ZXOT_EMU,  0.1%
	0x00000642,//ZXOTU_EMU  30%
};
...
{	
	...
	0x00000000,//PRTH2H_EMU
	0x00000000,//SAGCFG_EMU
	0x00200300,//EMUCFG3 采用功率启动方式2,一周波更新频率寄存器
	0x00000005,//ZXOT_EMU,  0.1%
	0x00000642,//ZXOTU_EMU  30%
};
#elif (cCURR_TYPES==CURR_60A)
...
#endif//#if (cCURR_TYPES==CURR_500A)
#endif//#if (HARDWARE_TYPE == SINGLE_PHASE_BOARD)
//采样芯片控制类寄存器地址
const DWORD SampleCtrlAddr[18][2]= 
{
	{CFCFG_EMU,3},
	{EMMIE_EMU,2},
	{EMUCFG_EMU,3},
...
const DWORD SampleAdjustAddr[33][2] = 
{
	...
	{GQA_EMU,2},	{GQB_EMU,2},	{GQC_EMU,2},//2字节
	{GSA_EMU,2},	{GSB_EMU,2},	{GSC_EMU,2},//2字节
	{GSUA_EMU,2},	{GSUB_EMU,2},	{GSUC_EMU,2},//2字节
	{GSIA_EMU,2},	{GSIB_EMU,2},	{GSIC_EMU,2},//2字节
	{PAPHSL_EMU,2},	{PBPHSL_EMU,2},	{PCPHSL_EMU,2},//2字节
	{PHSIA_EMU,4},	{PHSIB_EMU,4},	{PHSIC_EMU,4},//3字节
	{PAOS_EMU,4},	{PBOS_EMU,4},	{PCOS_EMU,4},//4字节
...
const DWORD SampleAdjustDefData[33] = 
{
	...
	0x00000000, 0x00000000, 0x00000000,	//GS
	0x00000000, 0x00000000, 0x00000000,	//GSU
	0x00000000, 0x00000000, 0x00000000,	//GSI
	0x00000000, 0x00000000, 0x00000000,	//PHSP
	0x20080200,	0x20080200, 0x20080200,	//PHSI
	0x00000000, 0x00000000, 0x00000000,	//POS	
	0x00000000, 0x00000000, 0x00000000,	//IOS
	0x00000000, 0x00000000, 0x00000000,	//PHS_U_WAVE
...
const DWORD HT7627SUConst = 0x8500;
const DWORD HT7627SIConst = 0x24C800;
#endif
...
const TCycleSInfoRom CycleSInfoRomConst =
{
	.CycleSearchFlag = 1,       // 周期搜表默认关闭（安徽打开）
    .AutoUpdateArchives = 1,    // 自动更新采集档案,默认关闭（安徽打开）
    .SearchMeterEventFlag = 0,  // 搜表事件生成标志 
    .ClearResultOption = 1,     // 清空搜表结果选项，默认不清空（安徽每天搜表前清空）
    .CycleNum = 1, //搜表周期数
	.Cycle = {                  //时，分，秒 ，搜表时长（min）
		{{8, 0, 0},60},
	},
    .SearchMode = 0,            // 搜表模式{周期搜表（0），即装即采（1）}
	.IsPowerUpSearch = 0x00,    // 上电是否启动搜表
    .PowerUpSearchTime = 0,     // 上电搜表时长（min）
};
#else
const TCycleSInfoRom CycleSInfoRomConst =
{
	.CycleSearchFlag = 0,       // 周期搜表默认关闭
    .AutoUpdateArchives = 0,    // 自动更新采集档案,默认关闭
    .SearchMeterEventFlag = 0,  // 搜表事件生成标志 
    .ClearResultOption = 0,     // 清空搜表结果选项，默认不清空
    .CycleNum = 1, //搜表周期数
	.Cycle = {                  //时，分，秒 ，搜表时长
		{{21, 30, 0},0},
	},
...
#endif //#if (cSPECIAL_CLIENTS == SPECIAL_GW_AN_HUI)

#endif //#if (SEL_SEARCH_METER == YES )

#if (SEL_TOPOLOGY == YES )
const ModulatSignalInfo_t ModulatSignalInfoConst = 
{
    .BitWideTime = 600,
    .HighLevelPulseWidth = 400,
    .LowLevelPulseWidth = 800,
    .SignatureCodeLen = 2,
    .SignatureCode = {0xE9,0xAA},
    .ModulatFreq   = 83330,
    .ExtInfoLen = 0,
    .ExtInfo = {0}
};
...
MessageTransPara_t MessageTransParaConst = 
{
	.AllowUnknownAddrTrans = TRUE,
	.Baud = 6,
	.Timeout = 30,
};
...
const TFreezePara InstantFreeze[MAX_INSTANT_FREEZE_ATTRIBUTE+1]=
{
	...
	{0, 0x20010200, 3 },		// 电流
	{0, 0x20020200, 3 },		// 电压相角
	{0, 0x20030200, 3 },		// 电压电流相角
	{0, 0x20040200, 3 },		// 有功功率
	{0, 0x20050200, 3 },		// 无功功率
	{0, 0x20060200, 3 },		// 视在功率
	{0, 0x200A0200, 3 },		// 功率因素
	{0, 0x200F0200, 3 },		// 电网频率
	{0, 0x00000400, 3 },		// 组合有功电能
	{0, 0x00100400, 3 },		// 正向有功电能
	{0, 0x00200400, 3 },		// 反向有功电能
...
{
	{12,0x0000000C, 12},		//冻结属性个数
	{5, 0x20000200, MAX_FREEZE_MIN_DEPTH },		// 电压
	{5, 0x20010200, MAX_FREEZE_MIN_DEPTH },		// 电流
	{5, 0x20020200, MAX_FREEZE_MIN_DEPTH },		// 电压相角
	{5, 0x20030200, MAX_FREEZE_MIN_DEPTH },		// 电压电流相角
	{5, 0x20040200, MAX_FREEZE_MIN_DEPTH },		// 有功功率
	{5, 0x20050200, MAX_FREEZE_MIN_DEPTH },		// 无功功率
	{5, 0x20060200, MAX_FREEZE_MIN_DEPTH },		// 视在功率
	{5, 0x200A0200, MAX_FREEZE_MIN_DEPTH },		// 功率因素
	{5, 0x200F0200, MAX_FREEZE_MIN_DEPTH },		// 电网频率
	{5, 0x00000401, MAX_FREEZE_MIN_DEPTH },		// 组合有功总电能
	{5, 0x00100401, MAX_FREEZE_MIN_DEPTH },		// 正向有功总电能
	{5, 0x00200401, MAX_FREEZE_MIN_DEPTH },		// 反向有功总电能
...
//	{1, 0x20000200, 365 },		// 电压
//	{1, 0x20010200, 365 },		// 电流
//	{1, 0x20020200, 365 },		// 电压相角
//	{1, 0x20030200, 365 },		// 电压电流相角
//	{1, 0x20040200, 365 },		// 有功功率
//	{1, 0x20050200, 365 },		// 无功功率
//	{1, 0x20060200, 365 },		// 视在功率
//	{1, 0x200A0200, 365 },		// 功率因素
//	{1, 0x200F0200, 365 },		// 电网频率
//	{1, 0x00000400, 365 },		// 组合有功电能
...
const TFreezePara ClosingFreeze[MAX_CLOSING_FREEZE_ATTRIBUTE+1]=
{
	...
	{1, 0x20000200, 12 },		// 电压
	{1, 0x20010200, 12 },		// 电流
	{1, 0x20020200, 12 },		// 电压相角
	{1, 0x20030200, 12 },		// 电压电流相角
	{1, 0x20040200, 12 },		// 有功功率
	{1, 0x20050200, 12 },		// 无功功率
	{1, 0x20060200, 12 },		// 视在功率
	{1, 0x200A0200, 12 },		// 功率因素
	{1, 0x200F0200, 12 },		// 电网频率
...
const TFreezePara MonFreeze[MAX_MON_FREEZE_ATTRIBUTE+1]=
{
	...
	{1, 0x20000200, 24 },		// 电压
	{1, 0x20010200, 24 },		// 电流
	{1, 0x20020200, 24 },		// 电压相角
	{1, 0x20030200, 24 },		// 电压电流相角
	{1, 0x20040200, 24 },		// 有功功率
	{1, 0x20050200, 24 },		// 无功功率
	{1, 0x20060200, 24 },		// 视在功率
	{1, 0x200A0200, 24 },		// 功率因素
	{1, 0x200F0200, 24 },		// 电网频率
	{1, 0x00000400, 24 },		// 组合有功电能
	{1, 0x00100400, 24 },		// 正向有功电能
	{1, 0x00200400, 24 },		// 反向有功电能
// 	{17, 0x00000011,17},	//冻结属性个数
...
// 时区表切换冻结
// 默认周期无效，2个点
const TFreezePara TZChgFreeze[MAX_TIME_ZONE_FREEZE_ATTRIBUTE]=
{
	{12, 0x0000000c, 12},		//冻结属性个数 
...
//-----------------------------------------------
	...
	const BYTE RateVoltageConst[6] = {"220V"};		//额定电压 ascii码     4
		...
		const BYTE MaxCurrentConst[6] = {"20A"};	//最大电流 ascii码     6
		const BYTE MinCurrentConst[10] = {4,'0','.','2','A'};	//最小电流 ascii码     7
		const BYTE TurnCurrentConst[10] = {4,'0','.','5','A'};	//转折电流 ascii码     8
		const DWORD ActiveConstantConst = 1000;		//有功脉冲常数    	11
	#elif( cCURR_TYPES == CURR_60A )
 	...
 	#elif( cCURR_TYPES == CURR_500A )
		const BYTE RateCurrentConst[6] = {"100A"};	//额定电流 ascii码     5
		const BYTE MaxCurrentConst[6] = {"500A"};	//最大电流 ascii码     6
		const BYTE MinCurrentConst[10] = {2,'4','A'};//最小电流 ascii码  7
		const BYTE TurnCurrentConst[10] = {3,'1','0','A'};//转折电流 ascii码 8
		//57v对应100000,220v对应30000
		#if(cMETER_VOLTAGE==METER_220V)
		const DWORD ActiveConstantConst = 200L;    //有功脉冲常数         11
		#else
	...
	const BYTE RateVoltageConst[6] = { "57.7V" };     //额定电压 ascii码     4
		...
		const WORD wMeterBasicCurrentConst = 100; //基本电流，3位小数
		const DWORD dwMeterMaxCurrentConst = 500; //最大电流，3位小数
		const WORD SimpleCurrConst = 80;	//80mv
		#if( cMETER_TYPES == METER_3P3W )	
		const WORD SimpleVolConst = 1835;	//68K * 8 + 1K
		#else
...
const BYTE QPrecisionConst[4] = {"2"};
	...
	#if(cRELAY_TYPE == RELAY_NO )
	//电表型号 ascii码     11
	const BYTE MeterModelConst[32] = {"WE6531"};
	#else
		#if( PREPAY_MODE == PREPAY_LOCAL )
		//电表型号 ascii码     11
		const BYTE MeterModelConst[32] = {"DTSUXXX-D"};
		#else
		//电表型号 ascii码     11
		const BYTE MeterModelConst[32] = {"DTSUXXX-D"};
		#endif
...
#endif
//电能表位置信息 初始化为0	
const BYTE MeterPositionConst[18] = {0};
//698.45协议版本号(数据类型:WORD)    13
const WORD ProtocolVersionConst = 0x1713;
//645协议版本号(数据类型:ASCII)   
...
const BYTE ListenType = 0;

// 4G模块默认参数
// ip地址 端口号 APN 192.168.1.1:5678 CMNET {0XC0,0XA8,0X00,0X01,0X2E,0X16,'C','M','N','E','T',0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X24};
const BYTE TCPIP_IP_Port_APNConst[23] @"PRG_CONST_C" = {0x00};
// 行政区码 终端地址
const BYTE TCPIP_AreaCode_TerConst[5] @"PRG_CONST_C" = {0};
// APN用户名 大端模式"WSD123" {"WSD123"};
const BYTE TCPIP_APN_UseNameConst[32] @"PRG_CONST_C" = {0};
// APN密码 大端模式"ABCDefg"  {'A','B','C','D','e','f','g',0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xC0};
const BYTE TCPIP_APN_PassWordConst[33] @"PRG_CONST_C" = {0x00};

// ip地址 端口号 APN 192.168.1.2:1234 无APN {0XC0,0XA8,0X01,0X02,0XD2,0X04,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X41};
const BYTE MQTT_IP_Port_APNConst[23] @"PRG_CONST_C" = {0x00};
// 行政区码 终端地址
const BYTE MQTT_AreaCode_TerConst[5] @"PRG_CONST_C" = {0};
// APN用户名 小端模式"DYTQ123" {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,'3','2','1','Q','T','Y','D'};
const BYTE MQTT_APN_UseNameConst[32] @"PRG_CONST_C" = {0x00};
// APN密码 小端模式"DYTQ" {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,'Q','T','Y','D',0x1A};
const BYTE MQTT_APN_PassWordConst[33] @"PRG_CONST_C" = {0x00};
...
const WORD  EventRogowskiADelayConst = 500; //接入判定延时，500ms
const WORD  EventRogowskiBDelayConst = 500; //断开判定延时,500ms
#endif

//终端抄表失败事件
#if( SEL_EVENT_ROGOWSKI_COMM_FAIL == YES )
const BYTE  EventRetryNoConst = 0; //终端抄表失败，重试轮次
const BYTE  EventCollectNoConst = 0; //终端抄表失败,关联采集任务号
#endif
