...
//主要是事件记录部分
	...
	#define  SEL_DLT698_2016_FUNC            	NO
    //是否支持 1376.2
    #define  SEL_1376_2_FUNC                    NO 
    ...
    //搜表功能
    #define SEL_SEARCH_METER                    NO

    //拓扑功能
	#define SEL_TOPOLOGY                        NO

	// 是否判断安全模式
	#define SEL_JUDGE_SAFE_MODE					NO

	#define SEL_ESAM_TYPE						0	//0：无ESAM,1:20规范ESAM,2:导轨表ESAM
	...
	//电能表零线电流异常
	#define SEL_EVENT_NEUTRAL_CURRENT_ERR	NO
	//终端状态量变位记录(遥信变位)
	#define SEL_EVENT_DI_CHANGE				NO
	//罗氏线圈变更事件
	#define SEL_EVENT_ROGOWSKI_CHANGE		NO
    //发现未知电能表事件
	#if (SEL_SEARCH_METER == YES)
    #define SEL_EVENT_FIND_UNKNOWN_METER    YES
	#else
	#define SEL_EVENT_FIND_UNKNOWN_METER    NO
	#endif
	//终端抄表失败事件
	#define SEL_EVENT_ROGOWSKI_COMM_FAIL	NO
	//------------------------------------------------------------------------------------
	// 编程记录选择  共12项
	//--------------------------------------
	// 电表清零记录
	...
	// 广播校时记录
	#define SEL_PRG_INFO_BROADJUST_TIME			NO
	// 额外模块记录
	#define SEL_PRG_MODULE						NO
	//------------------------------------------------------------------------------------------------------------
	// 扩展事件类型选择  共6项
	//------------------------------------------------------------------------------------------------------------
	// 购电记录
