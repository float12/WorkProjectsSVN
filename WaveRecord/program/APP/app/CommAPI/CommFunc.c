...
//-----------------------------------------------
//函数功能: 一个QWORD类型（8字节无符号）的BCD码数据转换为HEX码数据
//
//参数: 
//			In[in]					要转换的数据
//                    
...
//备注:8字节无符号数转换
//-----------------------------------------------
QWORD lib_UQWBCDToBin(QWORD In)
{
	//这个地方不再判断是否为BCD码，BBCDToBina这个函数判了
	return ((QWORD)lib_DWBCDToBin( (In>>32)&0xffffffff )) * 100000000 + (QWORD)lib_DWBCDToBin(In&0xffffffff);
}
...
//备注:   	
//-----------------------------------------------
void lib_MemSwap(BYTE *BufA,BYTE *BufB,WORD Length)
{
    BYTE tmp;
    WORD i;
...
// 备注内容:  长度为WORD类型
//--------------------------------------------------
void lib_ExchangeBitWordLen(BYTE *BufA, BYTE *BufB, WORD Len)
{
	WORD i;
...
//备注内容:  无
//--------------------------------------------------
void lib_OrData( BYTE *BufA, BYTE *BufB, WORD Num )
{
	WORD i;
...
//备注内容:  无
//--------------------------------------------------
void lib_AndData( BYTE *BufA, BYTE *BufB, WORD Num )
{
	WORD i;
