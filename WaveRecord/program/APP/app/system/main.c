...
void CheckImportantPara( void )
{
	...
	if( lib_CheckSafeMemVerify( (BYTE *)(FPara2), sizeof(TFPara2), UN_REPAIR_CRC ) == TRUE )
	{
		if( api_GetEnergyConst() == 1 )
		{
			...
			{
//				api_SetError(ERR_PULSCONST_TOO_BIG);//录波不用电能
			}
		}
	}
}
...
void TransmitDataInAsciiFormat(DWORD Data)
{
  ...
  while(Num > 1)
  {
    tempNum1 = Data/Num;//计算最高位
    Data = Data%Num;//计算剩下的数字
    Num /= 10;//
  ...
  {
  		USART_SendData(M4_USART3, AsciiString[i]);	//写入数据
    api_Delayms(3);
  }
}
...
void api_USART_SEND(DWORD dwData)
{
	...
	USART_FuncCmd(M4_USART3, UsartTxEmptyInt, Disable);					//发送中断使能
