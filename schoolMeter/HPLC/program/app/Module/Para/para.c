	...
	switch( Type )
	{
		case eMeterCommAddr://通讯地址
			...
			if( Operation == WRITE )
			{
				...
				Result = api_WritePara( 0, GET_STRUCT_ADDR( TFPara1, MeterSnPara.CommAddr ), ParaLength, pBuf );
				strcat((char*)AT_buf, "AT+NAME=");
				for(i = 0; i < 6; i++)
				{
					sprintf((char*)&AT_buf[strlen("AT+NAME=") + i*2],"%02x", pBuf[i]);
				}
				strcat((char*)AT_buf, "\r\n");
				memcpy( Serial[eBlueTooth].ProBuf, AT_buf, strlen((char*)AT_buf));     
				Serial[eBlueTooth].SendLength = strlen((char*)AT_buf);    
			}
			else
			{
...
//备注:   
//-----------------------------------------------
BYTE api_CheckResetDelay(void)
{
	if(lib_CheckCRC32((BYTE *)&ResetDelayPara.ResetDelay,sizeof(ResetDelayPara.ResetDelay)) != ResetDelayPara.CRC32)
	{
		ResetDelayPara.ResetDelay = 0xFF;
...
{	
	...
	if(api_CheckSysVol(eOnRunSpeedDetectUnsetFlag) == TRUE)
	{
