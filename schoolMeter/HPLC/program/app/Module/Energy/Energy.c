...
BOOL RepairEnergyUseFlash( void );
//-----------------------------------------------
//				函数定义
//-----------------------------------------------
...
//则需要进行转存 直通表按220v 100安考虑 非直通表按220v 10安考虑
//如果内存中的电能大于额定电压，最大电流，功率因数等于1时走8小时（单相是24个小时）的电能
//则需要进行清零 直通表按220v 100安考虑 非直通表按220v 10安考虑
//-----------------------------------------------
static BOOL WatchEnergy( DWORD *pDW, BYTE Type )
{
	...
	if( Type == 0 )
	{
		if( MeterTypesConst == METER_ZT )
		{
			dwEnergy = (DWORD)264*dwActiveConstant;//三相：220 * 100 * 4 * 3 = 264000 
		}
		else
		{
	...
	else if( Type == 1 )
	{
		if( MeterTypesConst == METER_ZT )
		{
			dwEnergy = (DWORD)(264*2)*dwActiveConstant;
		}
		else
		{
...
static BOOL EnergyCheck( BYTE Type )
{
	...
	for( i = 0; i < (1+NUM_OF_SEPARATE_ENERGY); i++ )
	{
		if( lib_CheckSafeMemVerify( (BYTE *)&EnergyRam.Energy[i], sizeof(TEnergyDBase), UN_REPAIR_CRC ) == FALSE )
		{
			...
			{
				ClearTDBase(&EnergyRam.Energy[i]);
				api_WriteSysUNMsg( ENERGY_CHECK_RAM_ERR );
			}
			else
			{
				// 从EEP恢复电能尾数
...
//		 pAddB[in]
//
// 返回值:pAddA = pAddA + pAddB
//
// 备注:
//-----------------------------------------------
void AddEnergy(TEnergyDBase *pAddA, TEnergyDBase *pAddB)
{
	BYTE i;
	DWORD *pdwA, *pdwB;
	...
	pAddA->CRC32 = lib_CheckCRC32((BYTE *)pAddA, (sizeof(TEnergyDBase) - sizeof(DWORD)));
}
...
// 备注:
//-----------------------------------------------
BOOL api_GetEnergyFlashMemData(TEnergyRom *pBuf)
{
	BYTE i,j,Ratio, Phase;
	WORD wAddr;
	TEnergyDBase RamEnergy[1 + NUM_OF_SEPARATE_ENERGY];

	// 低度
	memcpy((BYTE *)pBuf, (BYTE *)&EnergyRomBackup, sizeof(EnergyRomBackup));
	// 增量
	memcpy((BYTE *)&RamEnergy[0], (BYTE *)&EnergyRam, sizeof(EnergyRam));

	// 检查数据合法性
	for (i = 0; i < (1 + NUM_OF_SEPARATE_ENERGY); i++)
	{
		if (pBuf->Energy[i].CRC32 != lib_CheckCRC32((BYTE *)&(pBuf->Energy[i]), (sizeof(TEnergyDBase) - sizeof(DWORD))))
		{
			wAddr = GET_SAFE_SPACE_ADDR( EnergySafeRom.Energy.Energy[i] ); 
			
			if( api_VReadSafeMem( wAddr, sizeof(TEnergyDBase), (BYTE *)&(pBuf->Energy[i]) ) != TRUE )
			{
				return FALSE;
			}
		}

		if (RamEnergy[i].CRC32 != lib_CheckCRC32((BYTE *)&(RamEnergy[i]), (sizeof(TEnergyDBase) - sizeof(DWORD))))
		{
			memset( (BYTE *)&(RamEnergy[i]), 0x00, sizeof(TEnergyDBase) );
		}
	}
	...
	for (i = 0; i < (MAX_RATIO); i++)
	{
		for (j = 0; j < (1 + NUM_OF_SEPARATE_ENERGY_RATIO); j++)
		{
			if (pBuf->RatioEnergy[i][j].CRC32 != lib_CheckCRC32((BYTE *)&(pBuf->RatioEnergy[i][j]), (sizeof(TEnergyDBase) - sizeof(DWORD))))
			{
				wAddr = GET_SAFE_SPACE_ADDR( EnergySafeRom.Energy.RatioEnergy[i][j] );	

				if( api_VReadSafeMem( wAddr, sizeof(TEnergyDBase), (BYTE *)&(pBuf->RatioEnergy[i][j]) ) != TRUE )
				{
					return FALSE;
				}
			}
	...
	// 总及分项电能
	for (Phase = 0; Phase < (1 + NUM_OF_SEPARATE_ENERGY); Phase++)
	{
		AddEnergy(&(pBuf->Energy[Phase]), &RamEnergy[Phase]);
	}

	// 费率电能
	Ratio = api_GetCurRatio();

	// 一定要判断费率
	if ((Ratio == 0) || (Ratio > MAX_RATIO))
	{
		Ratio = DefCurrRatioConst; // 缺省认为是平时段
	}

	// 总及分项费率电能
	for (Phase = 0; Phase < (1 + NUM_OF_SEPARATE_ENERGY_RATIO); Phase++)
	{
		AddEnergy(&(pBuf->RatioEnergy[Ratio - 1][Phase]), &RamEnergy[Phase]);
	}
...
BOOL RepairEnergyUseFlash(void)
{
	...
	WORD wAddr;
	DWORD RecordNo;
	TEnergyRom TempFlash = {0};

	// 获取数据指针
	if (api_FindFlashMemDataNo(&RecordNo) != TRUE)
	{
		return FALSE;
	}

	// 读取数据
	if (api_GetFlashMemData(RecordNo, GET_STRUCT_ADDR(TFlashMem, Energy), sizeof(TempFlash), (BYTE *)&TempFlash) != TRUE)
	{
		return FALSE;
	}
	...
	for (i = 0; i < (1 + NUM_OF_SEPARATE_ENERGY); i++)
	{
		if (TempFlash.Energy[i].CRC32 != lib_CheckCRC32((BYTE *)&(TempFlash.Energy[i]), (sizeof(TEnergyDBase) - sizeof(DWORD))))
		{
			return FALSE;
		}
	}
	...
	for (i = 0; i < (MAX_RATIO); i++)
	{
		for (j = 0; j < (1 + NUM_OF_SEPARATE_ENERGY_RATIO); j++)
		{
			if (TempFlash.RatioEnergy[i][j].CRC32 != lib_CheckCRC32((BYTE *)&(TempFlash.RatioEnergy[i][j]), (sizeof(TEnergyDBase) - sizeof(DWORD))))
			{
				return FALSE;
			}
		}
	...
	// 刷新RAM低度
	memcpy((BYTE *)&EnergyRomBackup, (BYTE *)&TempFlash, sizeof(EnergyRomBackup));
	...
	for (i = 0; i < (1 + NUM_OF_SEPARATE_ENERGY); i++)
	{
		...
		// 写EE低度
		wAddr = GET_SAFE_SPACE_ADDR(EnergySafeRom.Energy.Energy[i]);
		api_VWriteSafeMem(wAddr, sizeof(TEnergyDBase), (BYTE *)&EnergyRomBackup.Energy[i]);
	}
	...
	for (i = 0; i < MAX_RATIO; i++)
	{
		for (j = 0; j < (1 + NUM_OF_SEPARATE_ENERGY_RATIO); j++)
		{
			...
			// 写EE费率电能
			wAddr = GET_SAFE_SPACE_ADDR(EnergySafeRom.Energy.RatioEnergy[i][j]);
			api_VWriteSafeMem(wAddr, sizeof(TEnergyDBase), (BYTE *)&EnergyRomBackup.RatioEnergy[i][j]);
		}
	}

	api_ClrSysStatus(eSYS_STATUS_EN_WRITE_ENERGY);

	api_WriteSysUNMsg(SYSUN_WRITE_ENERGY_EE);
