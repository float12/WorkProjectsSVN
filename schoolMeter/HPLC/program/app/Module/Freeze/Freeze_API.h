...
//-----------------------------------------------
//配置第三片存储器时，冻结数据的基地址
#define FLASH_FREEZE_BASE					(FLASH_EVENT_BASE+SUB_EVENT_INDEX_MAX_NUM*EVENT_RECORD_LEN)
...
#define FLASH_SECTOR_NUM					(6)		//除日、月、分钟外冻结扇区数
#define MIN_FLASH_SECTOR_NUM				(1500)	//分钟冻结扇区数
#define MINBAK_FLASH_SECTOR_NUM				(100)	//分钟冻结扇区备份
...
#else
#define MAX_INSTANT_FREEZE_ATTRIBUTE		(12+4)// 默认 电能*8个，最大需量及发生时间*2，有功功率无功功率	一共12个
#endif																		
...
#else
	#define MAX_DAY_FREEZE_ATTRIBUTE		(23+8)	// 默认 正反向有功,组合无功1,2,第1,2,3,4象限无功总，尖峰平谷电能,正反向有功最大需量及发生时间；有功无功功率,ABC电压合格率、余金额还没加
	//时间4字节、总电能4字节、12费率电能52字节、12费率143字节、功率16字节、电压合格率16字节、剩余金额8字节、透支金额4字节，预留240字节，CRC校验4字节
	#define MAX_DAY_FREEZE_E2_ONCE_DATA_LEN	1300//(4L+4*6+52*8+2*143+16*2+16*3+8+4+4)现有长度936，E2空间按照1300个字节开
	//日冻结空间不足扇区更改为23个----李丕凯
	#define MAX_DAY_FREEZE_DATA_LEN			(DAY_FLASH_SECTOR_NUM*SECTOR_SIZE)// (4字节Time+20字节数据)*62*8+       (4+11*5)*62*2+      (4+4*4)*2*62+     (4+16)*3*62
#endif
#define MAX_DAY_FREEZE_E2_DEPTH				7													//E2备份的日冻结数据深度
...
#else
#define MAX_TIME_ZONE_FREEZE_ATTRIBUTE		(12+4)// 默认 电能8个	最大需量以及发生时间2个	有功/无功功率2个	   一共12个	
#endif
...
#else
#define MAX_DAY_TIMETABLE_FREEZE_ATTRIBUTE	(12+4)	// 默认 电能8个	最大需量以及发生时间2个	有功/无功功率2个	   一共12个
#endif
...
#define MAX_CLOSING_FREEZE_E2_ONCE_DATA_LEN	1300 //(4L+4*3+52*9+143*2+4+240+4)现有长度898，E2空间按照1300个字节开

#endif
#define MAX_CLOSING_FREEZE_DATA_LEN			(FLASH_SECTOR_NUM*SECTOR_SIZE)						// (4字节Time+20字节数据)*12*2= 576 字节
#define MAX_CLOSING_FREEZE_E2_DEPTH			12												//E2备份的结算日冻结数据深度
...
#define NILM_EQUIPMENT_MAX_NO				10			// 一次最多识别，存电器的个数,注意模組是30个。
#define NILM_EQUIPMENT_TYPE_NO				1			// 可识别的电器类型数量
//-----------------------------------------------
//				结构体，共用体，枚举
//-----------------------------------------------
//分钟冻结占用空间最多，放在最后面
...
extern const TFreezePara InstantFreeze[MAX_INSTANT_FREEZE_ATTRIBUTE+1];
extern const TFreezePara MinFreeze[MAX_MINUTE_FREEZE_ATTRIBUTE+1];
extern const TFreezePara HourFreeze[MAX_HOUR_FREEZE_ATTRIBUTE+1];
extern const TFreezePara DayFreeze[MAX_DAY_FREEZE_ATTRIBUTE+1];
extern const TFreezePara ClosingFreeze[MAX_CLOSING_FREEZE_ATTRIBUTE+1];
extern const TFreezePara MonFreeze[MAX_MON_FREEZE_ATTRIBUTE+1];
extern const TFreezePara TZChgFreeze[MAX_TIME_ZONE_FREEZE_ATTRIBUTE+1];
extern const TFreezePara DTTChgFreeze[MAX_DAY_TIMETABLE_FREEZE_ATTRIBUTE+1];
#if( PREPAY_MODE == PREPAY_LOCAL )
extern const TFreezePara TariffRateChgFreeze[MAX_TARIFF_RATE_FREEZE_ATTRIBUTE+1];
extern const TFreezePara LadderChgFreeze[MAX_LADDER_FREEZE_ATTRIBUTE+1];
extern const TFreezePara LadderClosingFreeze[MAX_LADDER_CLOSING_FREEZE_ATTRIBUTE+1];
#endif
