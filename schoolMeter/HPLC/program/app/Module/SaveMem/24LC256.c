...
//-----------------------------------------------
#define I2CTIMES		6
#if(( MASTER_MEMORY_CHIP == CHIP_24LC128 )||( MASTER_MEMORY_CHIP == CHIP_24LC256 ))
	#define PAGE_SIZE		64
#elif( MASTER_MEMORY_CHIP == CHIP_24LC512 )
	#define PAGE_SIZE		128
#endif
#define ADDR_CONFIG_VALUE1_ST	0X01
#define ADDR_CONFIG_VALUE2_ST	0X0F
#define ADDR_CONFIG_VALUE1_FM   0x00
#define ADDR_CONFIG_VALUE2_FM	0x0E
...
static BYTE Get24LC256DevAddr( BYTE ChipNo )
{
	...
	else if( ChipNo == CS_SPI_256401_CONFIG_ADDR )
	{
		return MEM_24LC25601_CONFIG_DEVADDR;
	}
	else if( ChipNo == CS_SPI_256402_CONFIG_ADDR )
	{
		return MEM_24LC25602_CONFIG_DEVADDR;
	}
	else
	{
		return 0xff;
...
static WORD WriteMemWithPages(BYTE ChipNo, WORD Addr, WORD Len, BYTE * pBuf)
{
	...
	/*if( api_GetSysStatus(eSYS_STATUS_EN_WRITE_MONEY) == FALSE )
	{
		if( lib_CheckProtectArea(GET_SAFE_SPACE_ADDR( MoneySafe ), GET_SAFE_SPACE_ADDR( MoneySafe ) + sizeof(TMoney), Addr, Addr+Len) != TRUE )
		{
			api_WriteSysUNMsg(SYSUN_EEPROM_PROTECT_44);
			return FALSE;
		}
	}*/
	...
	for(;;)
	{
		...
		for(i=0; i<3; i++)
		{
			...
			//掉电写重试改为2次 防止一片EE损坏写超时影响第二片EE写入
			if( (api_GetSysStatus( eSYS_STATUS_POWER_ON ) == FALSE) && (i >= 1) )
			{
				return FALSE;
			}
			#endif
		...
		// 重试次数是否已到？ 掉电减少了次数 不进入错误处理 评估无影响
		if( i >= 3 )
		{
			//置错误标志
			if( ChipNo == CS_SPI_256401 )
			{
				...
				if( g_byEEErrTimes[0] < 120 )
				{
					...
					if( SelEEDoubleErrReportConst == YES )
					{
						if(( g_byEEErrTimes[0] == 120 )&&( g_byEEErrTimes[1] == 120 ))
						{
...
static WORD WriteXX24XXXPage(BYTE ChipNo, WORD Addr, WORD Len, BYTE * pBuf)
{
	...
	{
		#if( SLE_THIRD_MEM_BACKUP == YES )
		//掉电读重试改为2次 防止一片EE损坏写超时影响第二片EE写入
		//必须放在最前面 减少延时并且不受后面的continue影响
		if( (api_GetSysStatus( eSYS_STATUS_POWER_ON ) == FALSE) && (i >= 2) )
		{
			break;
		}
		#endif
...
//备注:   
//-----------------------------------------------
static WORD ReadE2DeviceAddr_ST(BYTE ChipNo, WORD Len, BYTE * pBuf)
{
  	BYTE Result,i;
	WORD Addr = 0xc000;

	Result = TRUE;
	if( ( ChipNo != CS_SPI_256401_CONFIG_ADDR)
	#if( SLAVE_MEM_CHIP != CHIP_NO )
	&& (ChipNo != CS_SPI_256402_CONFIG_ADDR)
	#endif
	)
	{
		return FALSE;
...
//返回值: 	TRUE:读成功		FALSE:读失败
//
//备注:   复旦微的E2在没有写地址时，返回10，地址写过之后bit4 置为0
//-----------------------------------------------
static WORD ReadE2DeviceAddr_FM(BYTE ChipNo, WORD Len, BYTE * pBuf)
{
	BYTE Result = TRUE, i;
	WORD Addr = 0x06CA;
	...
	// 读数据时不再打开写允许功能，确保在读数据的时候不会产生误写操作
	for( i=0; i<3; i++ )
	{
		CLEAR_WATCH_DOG;
		if( I2CReadDev(ChipNo, Addr, Len, pBuf) == TRUE )
	...
	//XD代表没写保护，XF代表写保护，可以把WP管脚拉高再读一下应该就是XF
	pBuf[0] = ((pBuf[0] >> 4) & 0x0F);
...
//备注:   
//-----------------------------------------------
static WORD WriteE2DeviceAddr_ST(BYTE ChipNo, WORD Len, BYTE * pBuf)
{
	BYTE Result = FALSE;
	WORD Addr = 0xc000;
...
//返回值: 	TRUE:写成功		FALSE:写失败
//
//备注:   一下流程 参考复旦微手册以及 复旦微技术人员要求
//-----------------------------------------------
static WORD WriteE2DeviceAddr_FM(BYTE ChipNo, WORD Len, BYTE * pBuf)
{
	WORD i, CDAEnAddr = 0x3F35;
	BYTE Ret, DevAddr;
	...
	//1、CDA写使能 写3F35地址，不用发送数据
	// 得到设备地址
	DevAddr = Get24LC256DevAddr(ChipNo);
	if( DevAddr == 0xff )
	{
		return FALSE;
	}
	...
	for(i = 0; i < I2CTIMES; i++)
	{
		...
		// 操作地址
		Send_Byte24LC256IIC(DevAddr);
		if(Ack24LC256IIC() == FALSE)	//无应答重试
		{
			continue;
		}

		Send_Byte24LC256IIC( (BYTE)((CDAEnAddr>>8)&0xff) );
		if(Ack24LC256IIC() == FALSE)
		{
			continue;
		}

		Send_Byte24LC256IIC( (BYTE)(CDAEnAddr&0xff) );
		if(Ack24LC256IIC() == FALSE)
		{
			continue;
		}
	...
	//2、写入CDA 器件地址，06CA 处写入器件地址
	Ret = I2CWriteDev(ChipNo, 0x06CA, 1, pBuf);
...
BOOL api_GetEEPRomAddr( BYTE Type, BYTE* Buf )
{
	...
	//选择第一片或者第二片E2
	Chip = (Type == 0) ? SelEESoftAddrConst[0] : SelEESoftAddrConst[1];
	Addr = (Type == 0) ? CS_SPI_256401_CONFIG_ADDR : CS_SPI_256402_CONFIG_ADDR;

	if( CHIP_IS_SOFT_ADDR(Chip) )
	{
		if(Chip == CHIP_E2_ST_SOFT)
		{
			Ret = ReadE2DeviceAddr_ST(Addr , 1, Buf );
		}
		else if(Chip == CHIP_E2_FM_SOFT)
		{
			Ret = ReadE2DeviceAddr_FM(Addr , 1, Buf );
		}
	}
...
BOOL api_CheckEEPRomAddr(BYTE Type)
{
	...
	if(Type & 0x01)
	{
		if(CHIP_IS_SOFT_ADDR(SelEESoftAddrConst[0]))
		{
			if(SelEESoftAddrConst[0] == CHIP_E2_ST_SOFT)
			{
				Addr = ADDR_CONFIG_VALUE1_ST;
			}
			else if(SelEESoftAddrConst[0] == CHIP_E2_FM_SOFT)
			{
				Addr = ADDR_CONFIG_VALUE1_FM;
			}
			else
			{
				return FALSE;
			...
			Result = api_GetEEPRomAddr(0, Buf);
			if( (Buf[0] != Addr) || (Result != TRUE) )
			{
				return FALSE;
			}
		}
	...
	{
		#if( SLAVE_MEM_CHIP != CHIP_NO )
		if(CHIP_IS_SOFT_ADDR(SelEESoftAddrConst[1]))
		{
			if(SelEESoftAddrConst[1] == CHIP_E2_ST_SOFT)
			{
				Addr = ADDR_CONFIG_VALUE2_ST;
			}
			else if(SelEESoftAddrConst[1] == CHIP_E2_FM_SOFT)
			{
				Addr = ADDR_CONFIG_VALUE2_FM;
			}
			else
			{
				return FALSE;
			...
			Result = api_GetEEPRomAddr(1, Buf);
			if( (Buf[0] != Addr) || (Result != TRUE) )
			{
				return FALSE;
			}
...
void api_FactoryInitEEPRomAddr(void)
{
	...
	//1、写保护使能，禁止写数据
	ENABLE_WRITE_PROTECT1_FOR_ADDR;
	ENABLE_WRITE_PROTECT2_FOR_ADDR;
	api_Delay10us(5);
	...
	if(CHIP_IS_SOFT_ADDR(SelEESoftAddrConst[0]))
	{
		...
		{
			Addr = ADDR_CONFIG_VALUE1_ST;
			WriteE2DeviceAddr_ST( CS_SPI_256401_CONFIG_ADDR, 1, &Addr );
		}
		else if(SelEESoftAddrConst[0] == CHIP_E2_FM_SOFT)
		{
			Addr = ADDR_CONFIG_VALUE1_FM << 4;
			WriteE2DeviceAddr_FM( CS_SPI_256401_CONFIG_ADDR, 1, &Addr );
		}
	...
	if(CHIP_IS_SOFT_ADDR(SelEESoftAddrConst[1]))
	{
		...
		//2、由于ST的在没有写地址的时候，只回复0地址，因此第一片与第二片都写0地址
		//   FM的在没有写地址的时候，什么数据都恢复，由于第一片地址已经写了地址，不在用器件地址0, 用器件地址F
		//   但是由于第一片E2使能了写保护，理论还是可以用器件0地址写第二片E2
		if(SelEESoftAddrConst[1] == CHIP_E2_ST_SOFT)
		{
			Addr = ADDR_CONFIG_VALUE2_ST;
			WriteE2DeviceAddr_ST( CS_SPI_256401_CONFIG_ADDR, 1, &Addr );
		...
		{
			Addr = ADDR_CONFIG_VALUE2_FM << 4;	//CDA寄存器 高4位表示地址
			WriteE2DeviceAddr_FM( CS_SPI_256402_CONFIG_ADDR, 1, &Addr );
		}
