...
static BOOL JudgeIRAuthority( BYTE Ch,eRequestMode eType,BYTE* pOMD )
{
	...
	//新增
	if((OMD.w[1] == 0x5002)||(OMD.w[1] == 0x4800)||(OMD.w[1] == 0x4000))
 	{
		return TRUE;
 	}
...
static WORD JudgeTaskAuthority( BYTE Ch, eRequestMode eType, BYTE *pOI )
{
	...
	if( (eType==eACTION_MODE) && (LinkData[Ch].AddressType != eWILDCARD_ADDRESS_TYPE) )
	{
			...
			|| 	( (OI.w==0xF301) && ((ClassAttribute==5)||(ClassAttribute==6)) ) )//显示查看与全显命令
		{
			...
			if( (api_GetRunHardFlag(eRUN_HARD_MAC_ERR_FLAG) == TRUE)//挂起不允许方法操作
			&& (APPData[Ch].eSafeMode != eNO_SECURITY)//安全传输
			&&(OI.w == 0X4000))
			{
				return FALSE;
			}
...
static eAPPBufResultChoice GetRequestNormal( BYTE Ch,BYTE *pOAD )
{
	...
	else
	{
		...
		{
            Len = GetProOadData( eGetNormalData, Ch, 1,0XFF,pOAD, (BYTE *)NULL,(APPData[Ch].APPMaxBufLen.w- APPData[Ch].APPCurrentBufLen.w-1) , &APPData[Ch].pAPPBuf[APPData[Ch].APPCurrentBufLen.w+1]);//添加数据buf
		}
...
{	
	...
	if( Tag == 2 )//显示读取 
	{
		if( (((pDLT698RecordPara->OI) >= 0x3000) && ((pDLT698RecordPara->OI) < 0x3030) && ((pDLT698RecordPara->OI) != 0x300C))
		  ||((pDLT698RecordPara->OI) == 0x303B)||((pDLT698RecordPara->OI) == 0x303C)  ||((pDLT698RecordPara->OI) == 0x3040)) //事件RCSD 
		{
...
eAPPBufResultChoice DealRCSDIsZeroData( BYTE Ch, WORD OI, BYTE *DAR)
{
	...
	|| (OI == 0x303B) || (OI == 0x303C) || ( OI == 0x3040))//事件RCSD 
	{
		if(OI == 0x303B)
		{
			pEvent = pEventUnit[0x30];		//获取对应的事件记录表
...
eAPPBufResultChoice DealGetRequestRecordResult( BYTE Ch,WORD Len)
{
	...
	else
	{
		...
		{
			eResultChoice = DLT698AddBufDAR( Ch,DAR_OverRegion );// M条记录，M=0
			if( eResultChoice != eADD_OK )//
			{
				return eResultChoice;
			}
...
BYTE SetPsitionInfo( BYTE ClassAttribute,BYTE ClassIndex,BYTE *pData )
{
				...
				if( pData[2] == Structure_698 )//经度结构体
				{
					if( pData[3] == 4 )//个数
					{
						if(pData[4] == Enum_698 )//方位
						{
							...
							if( pData[6] == Unsigned_698 )//度
							{
								Buf[1] = pData[7]; 
								if( pData[8] == Unsigned_698 )//分
								{
									Buf[2] = pData[9]; 
									if( pData[10] == Unsigned_698 )//秒
									{
										Buf[3] = pData[11]; 
										if( pData[12] == Structure_698 )//纬度结构体
										{
											if( pData[13] == 4 )//个数
											{
												if(pData[14] == Enum_698 )//方位
												{
													Buf[4] = pData[15]; 
													if( Buf[4] > 1 )//纬度方位   enum{S（0），N（1）}，
													{
														return DAR_OverRegion;
													}
													if( pData[16] == Unsigned_698 )//度
													{
														Buf[5] = pData[17]; 
														if( pData[18] == Unsigned_698 )//分
														{
															Buf[6] = pData[19]; 
															if( pData[20] == Unsigned_698 )//秒
															{
																Buf[7] = pData[21]; 
																if( pData[22] == Double_long_unsigned_698)
																{	
																	lib_ExchangeData(&Buf[8], &pData[23], 4);
																	DAR = DAR_Success;
																}
															}
														}
	...
	{
		OffestLen = (ClassIndex == 2)? 4:0;//确定偏移地址
		...
		if( pData[0] == Structure_698 )//经度结构体
		{
			if( pData[1] == 4 )//个数
			{
				if(pData[2] == Enum_698 )//方位
				{
					...
					if( pData[4] == Unsigned_698 )//度
					{
						Buf[OffestLen+1] = pData[5]; 
						if( pData[6] == Unsigned_698 )//分
						{
							Buf[OffestLen+2] = pData[7]; 
							if( pData[8] == Unsigned_698 )//秒
							{
								Buf[OffestLen+3] = pData[9]; 
								DAR = DAR_Success;
							}
						}
					}
	...
	else if( ClassIndex == 3)
	{
		...
		{	
			lib_ExchangeData(&Buf[8], &pData[1], 4);
			DAR = DAR_Success;
		} 
	}
...
//备注内容:    无
//--------------------------------------------------
BYTE SetCommAddrOrMeterNoOrCustomCode( WORD OI,BYTE ClassAttribute,BYTE ClassIndexes,BYTE *pData )//暂未考虑ClassIndexes不为零的情况
{
	WORD Result;
	BYTE Buf[10],DAR;
...
eAPPBufResultChoice SetRequestParameter( BYTE Ch, BYTE *pOAD )
{
	...
	case 0x4002://表号
	case 0x4003://客户编号
		DAR = SetCommAddrOrMeterNoOrCustomCode( OI.w, ClassAttribute,ClassIndex, pData);
		break;
	case 0x4004://设备地理位置	
...
eAPPBufResultChoice SetRequestNormal( BYTE Ch,BYTE *pOAD)
{
	...
	else if( (OI.w >= 0x3000) && (OI.w <= 0x330e) )//设置事件类对象
	{
		eResultChoice = SetRequestEvent( Ch, pOAD );
	}
	else if( (OI.w >= 0x4000) && (OI.w <= 0x4517))//设置参变量 
	{
			...
			if( pData[2] == OAD_698 )
			{
				...
				if( Mode == 0 )
				{
					if( (OAD.d == 0xf2010200) || (OAD.d == 0xf2010201) || (OAD.d == 0xf2010202))
					{
						...
						{
							if( (pData[8] < 11)&&(pData[8] != 5) )//bit0~bit2 表示波特率只能到7
							{
				...
				{
					//修改缺省值波特率需要打开低级厂内，波特率明文可以设置，防止现场意外被更改。
					if(( OAD.d == 0xf20902FD ) || (OAD.d == 0xf2090201) || ( (OAD.d == 0xf20902ee) && (api_GetSysStatus(eSYS_STATUS_LOW_INSIDE_FACTORY) == TRUE) ))
					{
						if( pData[7] == COMDCB_698 )
						{
							if( (pData[8] < 11)&&(pData[8] != 5) )//bit0~bit2 表示波特率只能到10
							{
								TmpCommPara0 = (FPara2->CommPara.ComModule & 0xF0);//不赋值波特率
								//1200bps（2），2400bps（3），4800bps（4），7200bps（5），9600bps（6），19200bps（7），
								TmpCommPara0 |= (pData[8]&0x0F);//表示波特率

								CommPara = FPara2->CommPara;
								if((OAD.d == 0xf20902FD)|| (OAD.d == 0xf2090201))	//载波协商 波特率
								{
									CommPara.ComModule = TmpCommPara0;
									DAR = (CommPara.ComModule == FPara2->CommPara.ComModule) ? DAR_Success : DAR_HardWare;
									CROverTime = CR_OVER_TIME ;
								}
								else if(OAD.d == 0xf20902ee)//设置载波波特率缺省值
								{
									CommPara.ComModule = TmpCommPara0;
									CommPara.ComModuleDeflt = TmpCommPara0;
									DAR = (CommPara.ComModuleDeflt == FPara2->CommPara.ComModuleDeflt) ? DAR_Success : DAR_HardWare;
									CROverTime = CR_OVER_TIME ;
								}

								if( DAR != DAR_Success )
								{
									if( api_WritePara(1, GET_STRUCT_ADDR(TFPara2, CommPara), sizeof(CommPara), (BYTE *)&CommPara) == TRUE )
									{
...
eAPPBufResultChoice  ActionRequest( BYTE Ch,BYTE *pOMD)
{
	...
	else if( (OI.w >= 0xf200) && (OI.w <= 0xf20b ))//输入输出设备类
	{
	...
	if(OI.w != 0x4000)//为避免现场重复广播校时，广播校时不记录自由事件
	{
