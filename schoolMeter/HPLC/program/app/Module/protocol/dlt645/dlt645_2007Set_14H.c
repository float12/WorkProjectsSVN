...
WORD SetPara04000FXX( BYTE SubID0, BYTE Len, BYTE *pBuf )
{
	...
	if( SubID0 == 0x00 )
	{
		...
		{
			api_SetSysStatus(eSYS_STATUS_EN_WRITE_SAMPLEPARA);//有这个防护就不会出现第一片没有写只写第二片EEP的问题，这样写成功就是True，写不成功就是False
			...
			{
				memcpy((BYTE*)&TF415UIK.Uk,(BYTE*)&Tsetbuf.Uk,sizeof(TSampleUIK));//写eep会自动更新Tsetbuf的crc
	...
	{
		//04000fB3----是否允许因为通讯异常对415进行cpu复位，0：默认允许；1：不允许415复位
		Result = pBuf[0];
		if(( Result == 0) ||( Result == 1))
		{
...
WORD ProIap( eSERIAL_TYPE PortType )
{
	...
	else if ((gPr645[PortType].dwDataID.d == 0x0f0f0f02) || (gPr645[PortType].dwDataID.d == 0x0f0f0f05) || (gPr645[PortType].dwDataID.d == 0x0f0f0f0b) )
	{
		...
		if(LastFrameNo != wFrameNo)//重试不计算校验
		{
	...
	{
		// 主站发送完成帧，DATA = 55H，表示确认升级；DATA = AAH，表示放弃升级；
		if( ProBuf[0] == 0x55 )
		{
			...
			//计算整个程序文件的CRC16校验码，并与下发的进行比较 !!!!!!
			tIap.dwIapFlag = IAP_FLAG;
			tIap.dwCrc32 = lib_CheckCRC32((BYTE *)&tIap.dwFileSize, sizeof(tIap) - sizeof(DWORD)); // 计算校验

			api_WriteMemRecordData(IAP_INFO_ADDR, sizeof(tIap), (BYTE *)&tIap.dwFileSize);

			ResetDelayPara.ResetDelay = 3;//延时3s开始升级,等待回复升级完成帧
