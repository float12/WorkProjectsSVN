	...
	else
	{
		...
		for( n = 0; n < 15; n++ )
		{
			if( ReceNum != Serial[i].RXRPoint )
			{
				...
				Result = DoReceProc_Dlt645( Serial[i].ProBuf[Serial[i].RXRPoint], &Serial[i] ,i);
				#else
				Result = FALSE;
				#endif
...
//备注: 安全空间中的数据不对，不恢复。
//--------------------------------------------------
void RecoverModuleDefauleBaudRate(WORD Timer)
{
	BYTE TmpCommPara0 = FPara2->CommPara.ComModuleDeflt;
...
void api_ProtocolTask(void)
{
	...
	if( api_GetTaskFlag(eTASK_PROTOCOL_ID,eFLAG_SECOND) == TRUE )
	{
		...
		if(ResetDelayPara.ResetDelay != 0xFF)
		{
			if(api_CheckResetDelay() != FALSE)//校验成功
			{
				if(ResetDelayPara.ResetDelay != 0)
				{
					ResetDelayPara.ResetDelay--;
...
/*		#if(SEL_POWER_LINE_CARRIER_OR_GPRS == YES)
	...
	if( api_GetTaskFlag(eTASK_PROTOCOL_ID,eFLAG_MINUTE) == TRUE )
	{
		...
		if( CROverTime != 0 )//倒计时
		{
			...
			if( CROverTime == 0 )
			{
				RecoverModuleDefauleBaudRate(0);
			}
		}
	}
...
void DoReceMoveData(WORD ProtocolType, TSerial * p)
{
	...
	else
	{
		...
		for( i=0; i<ProStep; i++ )
		{
			 Buf[i] = p->ProBuf[(BeginPos+i)%MAX_PRO_BUF];
		}
...
{    
	...
	RecoverModuleDefauleBaudRate(CR_OVER_TIME);		//载波波特率变化后上电恢复默认值
}
