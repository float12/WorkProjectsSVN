...
#ifdef ZX_USING_SFUD
    /* ZX-Thread RTOS platform */
    sfud_dev = zx_sfud_flash_find_by_dev_name(FAL_USING_NOR_FLASH_DEV_NAME);
#else
    /* bare metal platform */
...
int sfud_nor_flash_init(void)
{
	...
	if (cs_pin != ZX_NULL)
	{
		zx_hw_spi_device_attach("spi1", NOR_FLASH_SPI_DEV_NAME, cs_pin->gpio, cs_pin->pin);
		if (zx_sfud_flash_probe(FAL_USING_NOR_FLASH_DEV_NAME, NOR_FLASH_SPI_DEV_NAME) == ZX_NULL)
		{
			return -ZX_ERROR;
		}
		return ZX_EOK;
