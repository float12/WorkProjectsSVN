...
WORD api_ReadMeterVersion(BYTE *Buf)
{
	...
	//00101 010    版本（2字节）
	Buf[i++] = 0x2A;
	Buf[i++] = (BYTE)MODULE_VERSION;
	Buf[i++] = (WORD)MODULE_VERSION>>8;

	//00110 010    用户（2字节）
	Buf[i++] = 0x32;
	Buf[i++] = api_GetCodeBufferFlag(0);
	Buf[i++] = api_GetCodeBufferFlag(1);

	//00111 010    FLASH效验码（2字节）
	Buf[i++] = 0x3a;
	tw = api_CheckCpuFlashSum(0x01);
	...
	//10011 010    参数效验码（2字节）
	Buf[i++] = 0x9a;
	tw = api_CheckCpuFlashSum(0x02);
	lib_ExchangeData(Buf+i,(BYTE*)&tw,2);
	i += 2;
	//Buf[i++] = 0x00;
    //Buf[i++] = 0x00;

    //10100 010    BOOT版本号（2字节）
	Buf[i++] = 0xa2;
	Buf[i++] = *((BYTE *)FLASH_ADDR_BOOT_VERSION + 1);
    Buf[i++] = *((BYTE *)FLASH_ADDR_BOOT_VERSION);

    //10101 010    BOOT校验码（2字节）
	Buf[i++] = 0xaa;
    tw = api_CheckCpuFlashSum(0x00);
	lib_ExchangeData(Buf+i,(BYTE*)&tw,2);
	i += 2;

    //10110 010    DSP版本号（2字节）
	Buf[i++] = 0xb2;
	Buf[i++] = 0x00;
    Buf[i++] = 0x00;

    //10111 010    DSP校验码（2字节）
	Buf[i++] = 0xba;
	Buf[i++] = 0x00;
    Buf[i++] = 0x00;
...
WORD ReadFactoryExtPro(BYTE *pBuf)
{
	...
	switch(pBuf[1])
	{
		...
		case 0x08://读电表版本
			if(pBuf[0] == 0xdb)
			{
				wReturnLen = api_ReadMeterVersion(&pBuf[2]);
                wReturnLen += 2;//数据标识
		...
		case 0x0A://读取模组license状态
			if(pBuf[0] == 0xdb)
			{
				licenseres = get_license_status();
				memcpy(&pBuf[2],(BYTE*)&licenseres,sizeof(licenseres));
				wReturnLen += sizeof(licenseres);
				wReturnLen += 2;//数据标识
			}
			break;	
...
{
	DWORD tdw;
	WORD wReturnLen,OI,OI1;
	BYTE  OutputMessage[128];
	...
	if( api_GetSysStatus( eSYS_STATUS_INSIDE_FACTORY ) == FALSE )//未打开厂内模式
	{
		...
		// else
		{
			if( (OI != 0x0201 ) && (OI != 0x0206 )&&(OI != 0x3002))//只允许打开厂内模式和低级厂内模式,30002是参数设置开关
			{
				return 0x8001;
			}
		}
	...
	wReturnLen = 0;
	memcpy( (BYTE*)&OI1, pBuf, sizeof(WORD) );
	switch(pBuf[1])
	{
		case 0x02:
			if(pBuf[0] == 0x01)
		...
		case 0x0a://程序升级
			...
			if(pBuf[0] == 0x02)//启动电表程序升级
			{
				...
				api_WriteFreeEvent(RT_FREE_UPDATA,645);
				
				wReturnLen = 1;
			}
			break;
		case 0x30:
			if (pBuf[0] == 0x01)//存芯片参数
			{
				if((pBuf[4] == 0x55) && (pBuf[5] == 0xAA) && (pBuf[6] == 0xAA) && (pBuf[7] == 0x55))
				{
					lib_ExchangeData(&OutputMessage[0],&pBuf[8],16);
					if(api_VWriteSafeMem(GET_SAFE_SPACE_ADDR(ParaInfoSafeRom.Para), sizeof(TParaInfoSafeRom), OutputMessage))//测试用：正常程序需要打开
					{
						wReturnLen = 1;
					}
				}
            ...
            if(pBuf[0] == 0x03)//存储算法参数
			{
                if((pBuf[4] == 0x55) && (pBuf[5] == 0xAA) && (pBuf[6] == 0xAA) && (pBuf[7] == 0x55))
				{
					lib_ExchangeData(&OutputMessage[0],&pBuf[8],128);
					save_license(&OutputMessage[0]);
					wReturnLen = 1;
				}
			}
			...
			api_WriteFreeEvent( EVENT_OPEN_FAC_MODE, OI1);
			break;
