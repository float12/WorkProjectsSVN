...
//特征消息大小256字节字节
#define FEATRUE_MSG_SIZE			(256)
//特定设备信息中识别到设备的最大值
#define MAX_SPECIFIC_DEV_NUM		(10)
//消息队列的消息框个数
#define MSG_NUMBER					(3)
#endif
//-----------------------------------------------
...
{	
	eGET_ALL  = 0,
	eGET_ADDR,// = 0,
	eGET_SAMPLENUM,//计量元件数,几相
	eGET_SAMPLEINFO,//电压电流系数等信息
	eGET_V,
	eGET_I,
	eGET_RTC,
	eMAXCOMNO
...
#pragma pack(1)
typedef struct TNAddr_t
{
	BYTE	NAddressType;
	BYTE	NAddress[6];
	DWORD   dwCRC;
}TNAddr_t;
typedef struct TErrPassNotAut_t
{
	WORD OI;
	BYTE Attr;
	BYTE RecErrPassFlag; 	//接收错误标志
	BYTE RecErrPassCount;	//接收错误次数
	BYTE SendCount;			//发送次数
}TErrPassNotAut;
...
extern TErrPassNotAut ErrpassNoAut[6];
...
#if(PROTOCOL_VERSION == 25)
#pragma pack(1)
typedef struct TDeviceInfo_t
{
	WORD wDevID;  								//上传设备类别2Bytes
	BYTE bDevStatus;  							//上传设备状态1Byte
	DWORD dwDevAvgPow;  						//上传设备平均功率4Bytes
	WORD tDevTime;  							//上传设备事件发生时间,相对整15分钟的秒数
}TDeviceInfo;

typedef struct TDeviceInfoPool_t
{
	BYTE bDevCount;  							//上传的识别到的设备个数，最大是10个
	TDeviceInfo Message[MAX_SPECIFIC_DEV_NUM];	//一次可以传递最多传10个设备数据,消息队列消息框是3个，最多一次性容纳30个识别到的设备。例如15个设备，需要分两次发送，第一次发送10个，第二次发送5个
}TDeviceInfoPool;
#pragma pack()
...
#if(PROTOCOL_VERSION == 25)
extern BYTE g_UnDevFeature[FEATRUE_MSG_SIZE]; //此全局变量用于存放未知设备事件的特征数据
extern TDeviceInfoPool g_DevInof;			   //此全局变量用于存放特定设备信息
extern const BYTE g_bDevice_Code_List[25];
#endif
//-----------------------------------------------
// 				函数声明
//-----------------------------------------------
...
// 备注内容:  无
//--------------------------------------------------
void api_SetTagArrayNum(BYTE Num);//zh
