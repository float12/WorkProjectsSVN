...
extern BYTE UseProxy;//针对20EB的报文
...
//备注内容:  无
//--------------------------------------------------
WORD GetAndCheck645Pare( eSERIAL_TYPE PortType, BYTE *pBuf)
{
...
void SendDlt645(TSerial * p)
{
	...
	TTwoByteUnion Len;
/*  //魏玲坤提供 代理方式
	BYTE HeadFrame[] = {0x68,0x3A,0x00,0x43,0x05,0x72,0x73,0x62,0x60,0x03,0x00,0x00,0xA6,0x02,
						0x09,0x07,0x00,0xF2,0x09,0x02,0x01,0x03,0x00,0x08,0x01,0x00,0x00,0x0F,0x00,0x0F };
	BYTE TailFrame[] = {0x00,0x00,0x72,0xB3,0x16};
*/
/* */ //车间调试用 OAD方式
	// BYTE HeadFrame[] = {0x68,0x3A,0x00,0x43,0x05,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xA0,0x44,0xDA,
	// 					0x07,0x01,0x00,0x20,0xEB,0x03,0x00,0x09,0x21};
					//	0x68,0x00,0x00,0x00,0x00,0x00,0x00,0x68,0x14,0x15,0x0E,0x12,0x12,0x37,0x35,0x33,0x33,0x33,0x37,0x36,0x35,0x34,0x34,0x35,0x12,0x0E,0xBB,0xBB,0xBB,0xBB,0x34,0xAF,0x16,
	BYTE TailFrame[] = {0x00,0x00,0xF3,0x2B,0x16};
...
/*	p->ProBuf[0]=0x0fe;
	...
	p->ProBuf[3]=0x0fe;
*/
	p->SendLength = (TDataLength + 12);//如有FE FE FE FE 则加16

	//执行延时
	p->ReceToSendDelay = (DWORD)RECE_TO_SEND_DELAY;

	if(p->pBufEx[0] == 0x68 && p->wExLen != 0) //前面有标准698帧
	{
		if(p->pBufEx[14] == 0x07 && p->pBufEx[15] == 0x01)//if(p->pBufEx[14] == PROXY_REQUEST && p->pBufEx[15] == GET_PROXY_TransCommandRequest)	//是代理方式的透明传输
		{
			wExLen = 0;

			p->pBufEx[3] |= 0x80;	//改为上行帧标识
			p->pBufEx[14] |= 0x80;

			/*p->pBufEx[21] = 0x01;  //有数据返回 这是原来透传的

			p->wExLen = 22;
				*/
			p->pBufEx[21] = 0x0;  //有数据返回 改成 20 EB 的
			p->pBufEx[22] = 0x01;  //有数据返回 这
			p->wExLen = 23;

			memcpy(&pBufEx[wExLen], &p->pBufEx[0], p->wExLen);
			wExLen += p->wExLen;

			//改成 20 EB 的加个数据类型 0x09 透传的不加
			pBufEx[wExLen++] = 0x09;

			pBufEx[wExLen++] = p->SendLength;
			memcpy(&pBufEx[wExLen], &p->ProBuf[0], p->SendLength);
			wExLen += p->SendLength;

			memcpy(&pBufEx[wExLen], TailFrame, sizeof(TailFrame));
			wExLen += sizeof(TailFrame);

			Len.w = (wExLen - 2);//长度 不包括起始字符+结束字符
			pBufEx[1] = Len.b[0]; //数据长度
			pBufEx[2] = Len.b[1];

			Len.w = fcs16(&pBufEx[1],11);	//不包括起始地址和校验码
			pBufEx[12] = Len.b[0];  //添加hrc
			pBufEx[13] = Len.b[1];


			Len.w = fcs16(&pBufEx[1], (wExLen - 4));	//不包括起始地址和校验码
			pBufEx[wExLen-3] = Len.b[0];  //添加hrc
			pBufEx[wExLen-2] = Len.b[1];


			memcpy(&p->ProBuf[0], &pBufEx[0], wExLen);
			p->SendLength = wExLen;
		}
...
//return TRUE;	//wxy add
	...
	if(j!=TRUE)
	{
        j = JudgeRecMeterNoOnlyOneData(0xAA,pBuf);
	// 	j=TRUE;
	...
	else if( p->ProBuf[p->BeginPosDlt645] == 0x68 )
	{
		...
		if( p->ProStepDlt645 < sizeof(TDlt645Head) )
		{
			...
			if( p->ProStepDlt645 == sizeof(TDlt645Head) )
			{
				...
				{
					pBuf[i] = p->ProBuf[(p->BeginPosDlt645+i)%MAX_PRO_BUF];
				}
				//映射结构
				...
				if( Dlt645Head.Sync2 != 0x68 )//如果没有第2个帧起始符68，则前面收到的第一68不对，从它后面搜索第一个68
				{
					...
					{
						if( p->ProBuf[(p->BeginPosDlt645+i)%MAX_PRO_BUF] == 0x68 )
						{
							p->BeginPosDlt645 = (p->BeginPosDlt645+i)%MAX_PRO_BUF;
							p->ProStepDlt645 -= i;
							break;
						}
		...
		else if( p->ProStepDlt645 < (sizeof(TDlt645Head) + p->Dlt645DataLen + 2 ) )
		{
			...
			else if( p->ProStepDlt645 == (sizeof(TDlt645Head) + p->Dlt645DataLen + 2) )
			{
				...
				if(((UsePrintfFlag >> 0) & 0x0001) == 0x0001)
				{
					rt_kprintf("\nRX: ");
					j = 0;
					for(i = 0;i<Serial->ProStepDlt645;i++)
					{
...
static void FillDlt645FrameAddr( eSERIAL_TYPE PortType )
{
	...
	for(i=0; i<6; i++)
	{
        ...
        gPr645[PortType].pMessageAddr[DLT645_ADDR_STARTONFRAME+i] = 0x00;
	}
}
			...
			case eCONTROL_RECE_11://读命令
				if( DLT645APPFollow[PortType].FollowFlag != eLOAD_FOLLOW )
				{
					if( gPr645[PortType].wDataLen <= DLT645_FOLLOW_LEN )
					{
                        ...
                        {
                            memcpy( gPr645[PortType].pMessageAddr+DLT645_DATA_STARTONFRAME+DLT645_07_DATAID_LEN, DLT645Sendbuf[PortType].DataBuf+8, Lenth );
                        }
