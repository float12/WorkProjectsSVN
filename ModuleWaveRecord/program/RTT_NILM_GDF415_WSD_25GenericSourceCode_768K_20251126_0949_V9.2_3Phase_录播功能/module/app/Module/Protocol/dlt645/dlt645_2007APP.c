...
//备注内容:  无
//--------------------------------------------------
WORD DealDLT645_Factory( eSERIAL_TYPE PortType )
{
	WORD wResult;
	...
	switch( gPr645[PortType].byReceiveControl )
	{
		case eCONTROL_RECE_11:			
			...
			if( wResult != 0 )
			{
				...
				{
					memcpy( DLT645Sendbuf[PortType].DataBuf+8, gPr645[PortType].pMessageAddr + 14, wResult );
				}
		...
		case eCONTROL_RECE_14:
			wResult = ReadADData(gPr645[PortType].pMessageAddr + 22,0,0,NULL);//zh
			if( wResult == 0 )
			{
				wResult = DLT645_ERR_DATA_01;
			}
