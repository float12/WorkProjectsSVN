...
#define UNDEV_EVENT_SECTOR_NUM		(25)	// 309A未知设备事件每一个事件所需的扇区数
#define UNDEV_EVENT_DEEP			(288)	// 309A未知设备事件一个事件最大深度
#define UNDEV_EVENTRECORD_NUMBER	(12)	// 309A未知设备事件一个扇区存储12条
#endif
...
{	
	...
	#if(PROTOCOL_VERSION == 25)	
	//特定设备事件
	eSUB_EVENT_SPECIFIC_DEV,
	
	//未知设备事件
	eSUB_EVENT_UNKNOWN_DEV,
	#endif
...
typedef enum
{
	...
	#if(PROTOCOL_VERSION == 25)	
	eSUB_EVENT_SPECIFIC_DEV_NO,				//特定设备事件
	
	eSUB_EVENT_UNKNOWN_DEV_NO,				//未知设备事件
	#endif
...
typedef struct TEventOADCommonData_t
{
	...
	#if(PROTOCOL_VERSION == 25)	
	WORD			EventSource;				//事件发生源：由于之前不是pack（1）结构，因此加入此字段，整体结构体大小保持不变，事件发生源在309B事件中有应用，其余事件无应用！！！
	#endif
}TEventOADCommonData;
