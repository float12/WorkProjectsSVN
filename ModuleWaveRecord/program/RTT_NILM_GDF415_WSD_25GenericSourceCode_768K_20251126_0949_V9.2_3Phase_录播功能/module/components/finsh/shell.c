...
void finsh_thread_entry(void *parameter)
{
	...
	while (1)
	{
		...
		{
			JlinkChval = (unsigned char)ch;
			JlinkRecTime = 1000;

			//判定开始
			if((JlinkChval == 0x68) && (JlinkWaveState == 0))
			{
				JlinkWaveState = 1; //累计先导字节
				JlinkRecBuf[JlinkRecCount] = JlinkChval; //0x68
//				rt_kprintf("strat\n");
			}

			if(((JlinkChval == 0x31) || (JlinkChval == 0x33) || (JlinkChval == 0x34)) && (JlinkWaveState == 1))
			{
				JlinkWaveState = 2;
				JlinkRecBuf[JlinkRecCount] = JlinkChval; //0x31
			}

			if(JlinkWaveState == 2)
			{
				JlinkRecBuf[JlinkRecCount] = JlinkChval;
			}
			JlinkRecCount++;
			
			//判定结束
			#if(MAX_PHASE == 1)
			if((JlinkChval == 0x16) && (JlinkWaveState == 2) && (JlinkRecCount == 778))
			#elif(MAX_PHASE == 3)
			if((JlinkChval == 0x16) && (JlinkWaveState == 2) && (JlinkRecCount == 2314))
			#endif
			{
				JlinkRecCount = 0;
				JlinkWaveState = 0;
//				rt_kprintf("end\n");
				DoSPIReceProc(&JlinkRecBuf[0]);
			}
			else if((JlinkChval != 0x16) && (JlinkWaveState == 2) && (JlinkRecCount == 778))
			{
				JlinkRecCount = 0;
				JlinkWaveState = 0;
				rt_kprintf("error\n");
