...
long version_rtos(void)
{
    rt_show_version();

    return 0;
}
MSH_CMD_EXPORT(version_rtos, show RT-Thread version information);
...
long list_thread(void)
{
    ...
    rt_kprintf(" --- ---- ---  ------- ---------- ----------  ------  ---------- ---\n");
#else
    rt_kprintf("%-*.*s pri  status  stack addr     sp     stack size max used left tick  error\n", maxlen, maxlen, item_title);
    object_split(maxlen);
    rt_kprintf(" ---  ------- ---------- ---------- ----------  ------  ---------- ---\n");
#endif /*RT_USING_SMP*/
        ...
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++)
            {
                ...
                thread = (struct rt_thread *)obj;
                {
                    ...
                    while (*ptr == '#') ptr ++;
                    rt_kprintf(" 0x%08x 0x%08x 0x%08x    %02d%%   0x%08x %s\n",
...
long list_msgqueue(void)
{
    ...
    rt_kprintf("%-*.*s msg_size max_msgs entry suspend thread\n", maxlen, maxlen, item_title);
    object_split(maxlen);
    rt_kprintf(" -------- -------- ----- --------------\n");
    do
    {
