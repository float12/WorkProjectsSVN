 ...
 */
rt_device_t rt_console_set(rt_device_t new_device)
{
...
rt_weak int rt_kprintf(const char *fmt, ...)
{
     ...
     * length. */
    // length = rt_vsnprintf(rt_log_buf, sizeof(rt_log_buf) - 1, fmt, args); //不支持浮点数
	length = vsnprintf(rt_log_buf, sizeof(rt_log_buf) - 1, fmt, args); //支持浮点数
