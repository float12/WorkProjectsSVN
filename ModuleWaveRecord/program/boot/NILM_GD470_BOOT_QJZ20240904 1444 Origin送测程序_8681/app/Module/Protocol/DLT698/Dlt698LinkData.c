...
{	
	...
	else
	{
		...
		if(( LinkData[Ch].AddressType == eSINGLE_ADDRESS_TYPE )&&( ManageComInfo.AddressType == 0x00 ))
		{
			//@@@@未获取地址时，模块回复的地址？？
...
{
	BYTE Buf[MAX_PRO_BUF+EXTRA_BUF],i;
	...
	if( ManageComInfo.AddressType == 0x00 )//未获取电表地址时，使用通配地址
	{
		...
		for( i=0; i<6; i++ )
		{
			Buf[5+i] = 0xAA;//服务器地址，即通配地址
		}
	}
	else//使用电表地址
	{
		Buf[4] = 0x05; //地址长度
		if(ManageComInfo.Status == eGET_SAMPLEINFO)
		{
		    Buf[4] |= 0x10;//读计量芯配置参数，逻辑地址为1
		}
		for( i=0; i<6; i++ )
		{
			Buf[5+i] = ManageComInfo.Address[5-i];//服务器地址，即电表地址
		}
	}
