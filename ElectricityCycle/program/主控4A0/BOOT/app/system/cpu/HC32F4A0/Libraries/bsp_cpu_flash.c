...
void FLASH_Unlock()
{
	...
	/* Wait flash ready. */
    while(SET != EFM_GetAnyStatus(EFM_FLAG_RDY))
    {
        ;
    }
	while(SET != EFM_GetAnyStatus(EFM_FLAG_RDY1))
    {
        ;
    }
}
...
//	FLASH_ClearFlag(FLASH_FLAG_BSY | FLASH_FLAG_EOP | FLASH_FLAG_PGERR | FLASH_FLAG_WRPERR);	
	...
	status = (FLASH_Status)EFM_Program(_ulFlashAddr, _ucpSrc, _ulSize);
	if (status != FLASH_OK)
	{
		FLASH_Lock();
		return FALSE;
...
BOOL bsp_EraseAppFlash( void )
{
	...
	dwAddr = FLASH_APP_START_ADDR;

	while (dwAddr <= FLASH_APP_END_ADDR)
	{
		...
		dwAddr = dwAddr + FLASH_SECTOR_SIZE;
