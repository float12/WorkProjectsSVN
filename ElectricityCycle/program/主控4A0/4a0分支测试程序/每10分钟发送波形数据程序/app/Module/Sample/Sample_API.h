...
typedef struct TSamplePara_t
{
	...
	#if (SAMPLE_CHIP == CHIP_RN8302B)
	TSampleAdjustReg SampleAdjustReg[SAMPLE_CHIP_NUM];//校表、上电
	TSampleCtrlReg SampleCtrlReg;
	TSampleCorr SampleCorr;//校表、上电、读瞬时量
	TEDTDataPara EDTDataPara;
	TSampleChipPara SampleChipPara;
	//四计量芯片
    TSampleUIK	SampleUIKArr[SAMPLE_CHIP_NUM];
	TChargePara ChargePara;//充电记录数据最大时间
