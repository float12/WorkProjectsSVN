...
BOOL api_ConnectMechanismInfo( eConnectMode ConnectModeMode, BYTE *Buf )
{
	...
	#if(SEL_ESAM_TYPE == 1)
	CmdBuf[1] = 0x02;
	#elif(SEL_ESAM_TYPE == 2)
	CmdBuf[1] = 0x4E;
	#elif(SEL_ESAM_TYPE != 0)
	"ESAM类型配置错误"
	#endif
	CmdBuf[2] = 0x00;
	CmdBuf[3] = 0x00;
...
{
	BYTE CmdBuf[50], OutBuf[128], CmdLen = 6;
	BYTE Ret = FALSE;
	...
	if(Type == READ_METER_RN)
	{
		...
		Ret = Link_ESAM_TxRx(CmdLen, CmdBuf, 0x00, (void *)NULL, OutBuf);
		if(Ret == TRUE)
		{
			rand[0] = (OutBuf[0] << 8) + OutBuf[1];
			memcpy(&rand[1], &OutBuf[2], rand[0]);
		}
	}
...
{
	BYTE CmdBuf[50], OutBuf[512], CmdLen = 6;
	BOOL Result = FALSE;
	...
	if(Type == READ_METER_RN)
	{
		//8字节表号 + 随机数 + (数据 +MAC)--终端那边用的6字节表号
		CmdBuf[4] = (8 + rand[0] + Len) >> 8;	
		CmdBuf[5] = 8 + rand[0] + Len;

        //添加8字节电能表表号
		memcpy(&CmdBuf[CmdLen], MeterNo, 8);
		CmdLen += 8;
		...
		if(rand[0] <= 0x20)
		{
			memcpy(&CmdBuf[CmdLen], &rand[1], rand[0]);
			CmdLen += rand[0];

			Result = Link_ESAM_TxRx(CmdLen, CmdBuf, Len, Dat, OutBuf);
		}		
	}
...
BOOL api_processMacRN(eProcessMacRN type,verifyMacInfo_t *verifyMacInfo)
{
	...
	if(type == GENERATE_RN)
	{
		if( verifyMacInfo->rand == NULL)
		{
			return FALSE;
		}
		if(api_GetEsamSafeRN(READ_METER_RN,verifyMacInfo->rand))
		{
			return TRUE;
		}
		else
		{
			//获取随机数失败，清零随机数并生成异常事件
			verifyMacInfo->rand[0] = 0;
			memset(&verifyMacInfo->rand[1],0,16);
			api_WriteSysUNMsg(ESAM_GET_RN_ERR);
			return FALSE;
		}
	}
	else if(type == VERIFY_MAC)
	{
		if( (verifyMacInfo->apduMacBuf == NULL) || ( verifyMacInfo->rand == NULL))
		{
			return FALSE;
		}
		//如果随机数无效
		if(verifyMacInfo->rand[0] == 0x00)
		{
			//不验证MAC
			return TRUE;
		}
		...
		{
			//获取表号
			if(api_GetMeterNo(verifyMacInfo->mailAddr,meterNo) == FALSE )
			{
				return FALSE;
			}
			//验证MAC
			if( api_VerifyEsamSafeMAC( READ_METER_RN,meterNo,verifyMacInfo->rand,verifyMacInfo->apduMacBufLen,verifyMacInfo->apduMacBuf) == TRUE )
			{
				//验证MAC成功
				return TRUE;
			}
			...
			{
				//验证MAC失败，产生异常事件
				api_WriteSysUNMsg(ESAM_VERIFY_MAC_ERR);
				return FALSE;
			}
		}
	}
