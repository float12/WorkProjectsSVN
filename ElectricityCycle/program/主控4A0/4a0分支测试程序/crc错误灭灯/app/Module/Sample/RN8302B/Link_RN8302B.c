...
BOOL Link_ReadSampleReg( WORD Addr, BYTE *pBuf ,BYTE bLen,BYTE chipNo)
{
	...
	DoSPICS( chipNo + CS_SPI_SAMPLE1, TRUE );
	...
	tmp = api_UWriteSpi( eCOMPONENT_SPI_SAMPLE, 0xff );
        
	if( tmp != bSum)
	{
		DoSPICS(chipNo + CS_SPI_SAMPLE1, FALSE );
...
//备注:     写寄存器固定为1字节  
//-----------------------------------------------
BOOL Link_WriteSampleReg( WORD Addr, BYTE *pBuf ,BYTE bLen, BYTE chipNo)
{
	BYTE bSum = 0,bData,i;
	
	api_InitSPI( eCOMPONENT_SPI_SAMPLE, eSPI_MODE_1 );
	
	DoSPICS(chipNo + CS_SPI_SAMPLE1, TRUE );//片选
...
BOOL WriteSampleReg(WORD Addr, BYTE *pBuf, BYTE bLen, BYTE chipNo)
{
	...
	{
		Link_WriteSampleReg(Addr, pBuf, bLen, chipNo);
		Link_ReadSampleReg(0x18D, Datat1, 4, chipNo); 
...
void SampleSpecCmdOp(WORD Addr,eRnAddrCmd Cmd, BYTE chipNo)
{
	 ...
	 || ((Addr >= 0x01f3) && (Addr <= 0x01fa)) )
	{
		...
		{
			WriteSampleReg(0x0180,(BYTE*)&bLock[1],1, chipNo);
		}
		else if( Cmd == CmdWriteOpen )
		{
			WriteSampleReg(0x0180,(BYTE*)&bLock[0],1, chipNo);
		}
		else
		{
			;
		  ...
		  || (Addr == 0x00f8) )
		{
			if( Cmd == CmdWriteClose )
			{
				WriteSampleReg(0x017f,(BYTE*)&bLock2[1],4, chipNo);
			}
			else if( Cmd == CmdWriteOpen )
			{
				WriteSampleReg(0x017f,(BYTE*)&bLock2[0],4, chipNo);
			}
			else
			{
				;
