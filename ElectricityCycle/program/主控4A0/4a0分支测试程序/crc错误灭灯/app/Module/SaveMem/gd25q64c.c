...
BOOL  api_UpdateEraseFlash( DWORD Addr )
{
	...
	//写使能
	DoSPICS(CS_SPI_FLASH, TRUE);
	api_UWriteSpi( eCOMPONENT_SPI_FLASH, GD25Q64C_CMD_WREN );
	DoSPICS(CS_SPI_FLASH, FALSE);	//写完命令必须把CS拉高、
	...
	DoSPICS(CS_SPI_FLASH, FALSE);//写完地址必须把片选拉高 否则不进行擦除操作
	
	if( WaitFlashChipDoOver( CS_SPI_FLASH ) != TRUE )
	{
		return FALSE;
	}
...
//备注:
//-----------------------------------------------
static BOOL EraseExtFlashSector( WORD No, DWORD Addr )
{
	ASSERT( (Addr % SECTOR_SIZE) == 0, 1 );
...
static BOOL WriteFlashPage( BYTE No, DWORD Addr, WORD Len, BYTE *pBuf )
{
	...
	{
		if( EraseExtFlashSector( No, Addr ) != TRUE )//擦除失败由重试机制进行重试
		{
			return FALSE;//返回失败
		}
	}
...
//备注:
//-----------------------------------------------
static BOOL WriteFlashPageUpDate( BYTE No, DWORD Addr, WORD Len, BYTE *pBuf )
{
	BYTE CmpData[GD25Q64C_PAGE_SIZE];
	DWORD i;
...
//备注:
//-----------------------------------------------
static BOOL WriteFlashPageLogicUpDate( BYTE No, DWORD Addr, WORD Len, BYTE *pBuf )
{
	DWORD i;
	
	for(i=0; i<FLASH_MAX_WRITE_COUNT; i++)
	{
		if( WriteFlashPageUpDate(No, Addr, Len, pBuf) == TRUE )
		{
			WriteFlashErrorTimes = 0;
			g_byFlashErrTimes = 0;
		...
		else
		{
			...
			//记录异常事件记录
			api_WriteSysUNMsg(SYSUN_FALSE_WRITE_ERR|((Addr/SECTOR_SIZE)&0x1FFF));
			//复位芯片
			ResetFlash();
		}
	}
...
BOOL WriteExtFlashUpDate( BYTE No, DWORD Addr, WORD Len, BYTE *pBuf )
{
	...
	{
		// 写入一个页
		if( WriteFlashPageLogicUpDate(No, TrueAddr, PageLen, pBuf) != TRUE )
		{
			return FALSE;
		}
