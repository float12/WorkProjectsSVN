...
{
	return WriteExtFlashUpDate( CS_SPI_FLASH, Addr, Length, pBuf );
}
...
//备注:
//-----------------------------------------------
BOOL api_TestFlashComm(void)
{
	BYTE res = 0;
	BYTE TestBuf[20] = {0};

	ResetSPIDevice( eCOMPONENT_SPI_FLASH, 5 );
	memset( (BYTE *)&TestBuf[0], 0xaa, 10 );
	res = api_WriteMemRecordData(3L*4*1024, 10, (BYTE *)&TestBuf[0]);
	if(res == FALSE)
	{
		return FALSE;
	}
	memset( &TestBuf[10], 0x00, 10 );
	res = api_ReadMemRecordData(3L*4*1024, 10, (BYTE *)&TestBuf[10]);
	if(res == FALSE)
	{
		return FALSE;
	}
	if(memcmp((BYTE *)&TestBuf[0], (BYTE *)&TestBuf[10], 10) != 0)
	{
		return FALSE;
	}
	return TRUE;
