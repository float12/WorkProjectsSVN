...
WORD GetPro645RequestMeterPara( eSERIAL_TYPE PortType,BYTE *DI, WORD OutBufLen, BYTE *OutBuf )
{
	...
	BYTE TaggedByteBuf[1];
 	BYTE i,Status,SubmergedStartState;
	BYTE DataBuf[20];
	WORD wID1_2, Lenth;
	...
	switch( wID1_2 )
	{
		...
		case 0x0004:
			...
			else if(DI[0] == 0x1F)  // 扩展规约，读取地理位置，4G模块使用
			{
				Lenth = api_ProcMeterTypePara( READ, eMeterMeterPosition, Buf );
				Lenth = 18;
				Sel_HEX2BCD = NO;
                Sel_EXData = YES;
			}
			else if(DI[0] == 0x20)  // 扩展规约，读取透传报文参数
			{
				Buf[0] = MessageTransParaRam.AllowUnknownAddrTrans;
				Buf[1] = MessageTransParaRam.Baud;
				Buf[2] = MessageTransParaRam.Timeout;
				Lenth = 3;
				Sel_HEX2BCD = NO;
                Sel_EXData = NO;
			}
		...
		case 0x0005:	//电表运行状态字 密钥状态字
			...
			{
				Lenth = api_GetMeterStatus(ePROTOCO_645, 1, 2, Buf);
				if (Lenth != 2)
				{
					Lenth = 0;
				}
...
//				}
		...
		case 0x0013:
			...
			{
				case 0xD1:
					Lenth = api_ProcGprsTypePara(READ, eTCPIP_IP_Port_APN, Buf);
					break;
				case 0xD5:
					Lenth = api_ProcGprsTypePara(READ, eTCPIP_AreaCode_Ter, Buf);
					break;
				case 0XD6:
					Lenth = api_ProcGprsTypePara(READ, eTCPIP_APN_UseName, Buf) + api_ProcGprsTypePara(READ, eTCPIP_APN_PassWord, Buf+32);
					break;
				case 0xE1:
					Lenth = api_ProcGprsTypePara(READ, eMQTT_IP_Port_APN, Buf);
					break;
				case 0xE5:
					Lenth = api_ProcGprsTypePara(READ, eMQTT_AreaCode_Ter, Buf);
					break;
				case 0XE6:
					Lenth = api_ProcGprsTypePara(READ, eMQTT_APN_UseName, Buf) + api_ProcGprsTypePara(READ, eMQTT_APN_PassWord, Buf+32);
					break;
				case 0xF1:
					SubmergedStartState = api_GetSampleStatus( 0x20, 0 );
					Buf[0] = 0x0f;
					if( ( SubmergedStartState & 0x01 ) == 0x00 )//判断A相是否为起动状态
					{
						Buf[0] = Buf[0] & 0x03;
					}
					if( ( SubmergedStartState & 0x02 ) == 0x00 )//判断B相是否为起动状态
					{
						Buf[0] = Buf[0] & 0x05;
					}
					if( ( SubmergedStartState & 0x04 ) == 0x00 )//判断C相是否为起动状态
					{
						Buf[0] = Buf[0] & 0x06;
					}
					Lenth = 1;
