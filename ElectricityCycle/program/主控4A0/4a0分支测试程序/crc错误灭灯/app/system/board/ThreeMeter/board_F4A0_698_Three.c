...
void DmaIrqInit(void)
{
	...
	stcIrqSignConfig.enIRQn = UART_DMA_TC_INT_IRQn;
...
void InitBoard(void)
{
	...
	PowerUpStatus = RMU_GetStatus(RMU_FLAG_PWR_ON);
...
static void ExtIntConfig(TExtIntCof *pExtIntCof)
{
	...
	/* IRQ sign-in */
	stcIrqSignConfig.enIntSrc = pExtIntCof->IntSrc;
	stcIrqSignConfig.enIRQn = pExtIntCof->IntIRQn;
...
void api_TopWaveSpiInit(BYTE SampleChipNo)
{
	...
	api_InitSPI(WaveDataConf[SampleChipNo].SPIComponent, eSPI_MODE_1);
...
static void LoopChargeCallback(BYTE LoopNum)
{
	...
	NVIC_DisableIRQ(Edge1_Test_IrqNo + LoopNum - 1);
...
{
	NVIC_ClearPendingIRQ(Edge1_Test_IrqNo + loop);
	NVIC_EnableIRQ(Edge1_Test_IrqNo + loop);
}
