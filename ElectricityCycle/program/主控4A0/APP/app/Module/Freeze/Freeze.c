...
extern const TFreezePara TZChgFreeze[MAX_TIME_ZONE_FREEZE_ATTRIBUTE];
...
//备注:
//-----------------------------------------------
static void CheckMinFreezeInfo( void )
{
	BYTE i;
	WORD MinUpTimeFlag;
...
BOOL WriteFreezeRecordData_EraseFlash(DWORD Addr, WORD Length, BYTE * pBuf)
{
	...
	else
	{
		...
		if( api_CheckSysVol( eOnRunSpeedDetectPowerMode ) == FALSE )
		{
			return FALSE;
		}
		dwSectorAddr = (Addr / SECTOR_SIZE) * SECTOR_SIZE;//写的本扇区基地址
...
void api_FreezeTask( void )
{
	...
	if( FreezeFlag != 0 )
	{
		...
		for(i=0; i<=eFREEZE_MAX_NUM; i++)
		{
			...
			{
				if( api_CheckSysVol( eOnRunDetectPowerMode ) == FALSE )//掉电检测
				{
					break;
				}
...
{	
	...
	{		
		...
		for( i=0;  i<pData->pAllInfo->NumofOad; i++ )
		{
			...
			if (IsModuleOI((pData->pAttOad->Oad & 0xff) * 256 + (pData->pAttOad->Oad & 0xff00) / 256,eModuleAllOI))//拓展程序冻结已加tag
			{
				pData->Tag = 0;
			}
			//若读出的数据是结算需量数据，且数据为ff，则置为null
