...
#define EXT_MODULE_START_OAD			0X0602E033			//拓展模块发生数据
#define EXT_MODULE_END_OAD				0X07021133			//拓展模块接受数据
#define CHARGING_WARNING_OAD			0X06022A33			//告警状态
#define CHARGING_WARNING_CHANGE_OAD		0X07022A33			//告警变位
#define CHARGING_ERR_OAD				0X06022B33			//故障状态
#define CHARGING_ERR_CHANGE_OAD			0X07022B33			//故障变位
#define CHARGING_COM_EXCTION_OAD		0X06022C33			//通信异常状态
#define CHARGING_COM_EXCTION_CHANGE_OAD	0X07022C33			//通信异常变位
#define DI_STATE_OAD					0x06024033			//遥信状态
#define ROGOWSKI_START_OAD				0X00223340			//变更前的罗氏线圈识别结果
#define ROGOWSKI_END_OAD				0X00823340			//变更后的罗氏线圈识别结果
#define FIND_UNKNOWN_METER_OAD          0x00020260          //搜表结果
#define TOPO_EVENT_OAD					0X0602833E			//识别结果信息
#define TOPO_SIGNAL_OAD				    0X0002084E			//一个特征电流信号识别信息
...
BYTE eEventTypehch[]=
{
	...
	eEVENT_DI_CHANGE_NO,					//终端状态量变位事件29
	#endif
	
	eNUM_OF_EVENT,							// 工况类的事件数量 29
...
{	//失压
	...
	// 终端状态量变位
	#if( SEL_EVENT_DI_CHANGE == YES )//51
	eSUB_EVENT_DI_CHANGE,		//单相7
	#endif
...
const TSubEventInfoTab	SubEventInfoTab[] = 
{
	...
	//终端状态量变位事件
	#if( SEL_EVENT_DI_CHANGE == YES )
	{eSUB_EVENT_DI_CHANGE,			eEVENT_DI_CHANGE_NO,		ePHASE_ALL,	api_GetDIEventStatus },
	#endif
	
	{eSUB_EVENT_CHARGING_WARNING,	eEVENT_CHARGING_WARNING_NO,			ePHASE_ALL, GetPrgStatus },
	{eSUB_EVENT_CHARGING_ERR	,	eEVENT_CHARGING_ERR_NO,				ePHASE_ALL, GetPrgStatus },
	{eSUB_EVENT_CHARGING_COM_EXCTION,eEVENT_CHARGING_COM_EXCTION_NO,	ePHASE_ALL, GetPrgStatus },

	// 罗氏线圈变更事件
	#if( SEL_EVENT_ROGOWSKI_CHANGE == YES )             
	{eSUB_ROGOWSKI_CHANGE,		eEVENT_ROGOWSKI_CHANGE_NO, 		ePHASE_ALL, api_GetRogowskiChangEventStatus },	
	#endif

	//发现未知电能表事件
	#if(SEL_EVENT_FIND_UNKNOWN_METER == YES)
    {eSUB_FIND_UNKNOWN_METER,		eEVENT_FIND_UNKNOWN_METER_NO, 		ePHASE_ALL, api_GetFindUnknownMeterEventStatus },	
    #endif

	// 终端抄表失败事件
	#if( SEL_EVENT_ROGOWSKI_COMM_FAIL == YES )             
	{eSUB_ROGOWSKI_COMM_FAIL,	eSUB_ROGOWSKI_COMM_FAIL_NO, 	ePHASE_ALL, api_GetRogowskiFailEventStatus },	
	#endif

	#if( SEL_TOPOLOGY == YES )
	{eSUB_EVENT_TOPU,	           eEVENT_TOPU_NO, 	    ePHASE_ALL, api_GetTopuEventStatus },
    {eSUB_EVENT_TOPU_SIGNAL,eEVENT_TOPU_SIGNAL_NO, 	    ePHASE_ALL, GetPrgStatus },
	#endif

	#if( SEL_PRG_MODULE == YES )
	{eSUB_EVENT_MODULE_0, 	eSUB_EVENT_MODULE_0_NO, 	ePHASE_ALL, GetPrgStatus },	
	{eSUB_EVENT_MODULE_1, 	eSUB_EVENT_MODULE_1_NO, 	ePHASE_ALL, GetPrgStatus },	
	{eSUB_EVENT_MODULE_2, 	eSUB_EVENT_MODULE_2_NO, 	ePHASE_ALL, GetPrgStatus },	
	{eSUB_EVENT_MODULE_3, 	eSUB_EVENT_MODULE_3_NO, 	ePHASE_ALL, GetPrgStatus },	
	{eSUB_EVENT_MODULE_4, 	eSUB_EVENT_MODULE_4_NO, 	ePHASE_ALL, GetPrgStatus },	
	{eSUB_EVENT_MODULE_5, 	eSUB_EVENT_MODULE_5_NO, 	ePHASE_ALL, GetPrgStatus },	
	{eSUB_EVENT_MODULE_6, 	eSUB_EVENT_MODULE_6_NO, 	ePHASE_ALL, GetPrgStatus },	
	{eSUB_EVENT_MODULE_7, 	eSUB_EVENT_MODULE_7_NO, 	ePHASE_ALL, GetPrgStatus },	
	{eSUB_EVENT_MODULE_8, 	eSUB_EVENT_MODULE_8_NO, 	ePHASE_ALL, GetPrgStatus },	
	{eSUB_EVENT_MODULE_9, 	eSUB_EVENT_MODULE_9_NO, 	ePHASE_ALL, GetPrgStatus },	
	#endif
	//电能表编程事件
...
const TEventInfoTab EventInfoTab[] =
{
	...
	#if( SEL_EVENT_DI_CHANGE == YES )//终端状态量变位事件
	{0x3104,eEVENT_DI_CHANGE_NO,		1,					NO,		0,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	#endif	
	
//	{0x30E0,eEVENT_CHARGING_WARNING_NO,			1,			NO,	MAX_CHARGING_W_BYTENUM*2,0,eTYPE_NULL,eTYPE_NULL,eTYPE_NULL,eTYPE_NULL},
	
//	{0x30E1,eEVENT_CHARGING_ERR_NO,				1,			NO,	MAX_CHARGING_E_BYTENUM*2,0,eTYPE_NULL,eTYPE_NULL,eTYPE_NULL,eTYPE_NULL},
	
//	{0x30E2,eEVENT_CHARGING_COM_EXCTION_NO,		1,			NO,	MAX_CHARGING_PE_BYTENUM*2,0,eTYPE_NULL,eTYPE_NULL,eTYPE_NULL,eTYPE_NULL},

	#if( SEL_EVENT_ROGOWSKI_CHANGE == YES )		//罗氏线圈变更事件
	//OI	事件枚举号					最大相		掉电特殊处理 扩展长度		参数个数  
	{0x3053,eEVENT_ROGOWSKI_CHANGE_NO,	1,					NO,		0,		    	2,	eTYPE_SPEC1,eTYPE_SPEC2,eTYPE_NULL,	eTYPE_NULL,},
	#endif
	//发现未知电能表事件
	#if(SEL_EVENT_FIND_UNKNOWN_METER == YES)
    {0x3111,eEVENT_FIND_UNKNOWN_METER_NO, 1,                NO, sizeof(FindUnknownMeterEventData_t), 0,	eTYPE_NULL,eTYPE_NULL,eTYPE_NULL, eTYPE_NULL,},
    #endif
	#if( SEL_EVENT_ROGOWSKI_COMM_FAIL == YES )    // 终端抄表失败事件         
	{0x310F,eSUB_ROGOWSKI_COMM_FAIL_NO,	1,					NO,		0,		    	2,	eTYPE_SPEC1,eTYPE_SPEC2,eTYPE_NULL,	eTYPE_NULL,},
	#endif
	#if( SEL_TOPOLOGY == YES )
	{0x3E03,eEVENT_TOPU_NO,	            1,					NO,sizeof(TopoIdentiResult_t),		0,	eTYPE_NULL,eTYPE_NULL,eTYPE_NULL,	eTYPE_NULL,},
    {0x4E07,eEVENT_TOPU_SIGNAL_NO,      1,					NO,sizeof(TopoCharactCurIdentiInfo_t),0,	eTYPE_NULL,eTYPE_NULL,eTYPE_NULL,	eTYPE_NULL,}, 
	#endif
	#if( SEL_PRG_MODULE == YES )//拓展事件		
	{0xBD00,eSUB_EVENT_MODULE_0_NO,		1,					NO,		0x304,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	{0xBD01,eSUB_EVENT_MODULE_1_NO,		1,					NO,		0x304,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	{0xBD02,eSUB_EVENT_MODULE_2_NO,		1,					NO,		0x304,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	{0xBD03,eSUB_EVENT_MODULE_3_NO,		1,					NO,		0x304,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	{0xBD04,eSUB_EVENT_MODULE_4_NO,		1,					NO,		0x304,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	{0xBD05,eSUB_EVENT_MODULE_5_NO,		1,					NO,		0x304,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	{0xBD06,eSUB_EVENT_MODULE_6_NO,		1,					NO,		0x304,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	{0xBD07,eSUB_EVENT_MODULE_7_NO,		1,					NO,		0x304,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	{0xBD08,eSUB_EVENT_MODULE_8_NO,		1,					NO,		0x304,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	{0xBD09,eSUB_EVENT_MODULE_9_NO,		1,					NO,		0x304,				0,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,	eTYPE_NULL,},
	#endif
...
const DWORD FixedColumnOadTab[] = {
    ...
    AHOUR_OAD,                         // 安时值OAD
    V_UNBALANCE_OAD,                   // 电压不平衡OAD
    I_UNBALANCE_OAD,                   // 电流不平衡OAD
    PRG_SEGTABLE_OAD,                  // 时段表编程OAD
    PRG_SEGTABLE_VALUE,                // 时段表编程前时段表数据
    PRG_HOLIDAY_OAD,                   // 节假日编程OAD
    PRG_HOLIDAY_VALUE,                 // 节假日编程前节假日数据
	#if(PREPAY_MODE == PREPAY_LOCAL)
    CARD_SN_OAD,                       // 异常插卡事件记录单元0x3310--卡序列号	octet-string，
    CARD_ERR_INFO_OAD,                 // 异常插卡事件记录单元0x3310--插卡错误信息字	unsigned，
    CARD_ORDER_HEAD_OAD,               // 异常插卡事件记录单元0x3310--插卡操作命令头	octet-string，
    CARD_ERR_RES_OAD,                  // 异常插卡事件记录单元0x3310--插卡错误响应状态  long-unsigned，
    RETURN_MONEY_OAD,                  // 退费事件记录单元0x3311--退费金额      double-long-unsigned（单位
    #endif
	EXT_MODULE_START_OAD,              // 拓展模块发生数据
    EXT_MODULE_END_OAD,                // 拓展模块接受数据
    CHARGING_WARNING_OAD,              // 告警状态
    CHARGING_WARNING_CHANGE_OAD,       // 告警变位
    CHARGING_ERR_OAD,                  // 故障状态
    CHARGING_ERR_CHANGE_OAD,           // 故障变位
    CHARGING_COM_EXCTION_OAD,          // 通信异常状态
    CHARGING_COM_EXCTION_CHANGE_OAD,   // 通信异常变位
    DI_STATE_OAD,                      // 遥信状态
    ROGOWSKI_START_OAD,                // 变更前的罗氏线圈识别结果
    ROGOWSKI_END_OAD,                  // 变更后的罗氏线圈识别结果
	#if(SEL_EVENT_FIND_UNKNOWN_METER == YES)
    FIND_UNKNOWN_METER_OAD,            // 搜表结果
	#endif
	#if( SEL_TOPOLOGY == YES )
    TOPO_EVENT_OAD,                          // 特征电流识别结果
    TOPO_SIGNAL_OAD,
	#endif
};
...
static const TEventOadTab	EventOadTab[] = 
{
	...
	{0x30E0,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,CHARGING_WARNING_OAD,CHARGING_WARNING_CHANGE_OAD},	
	{0x30E1,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,CHARGING_ERR_OAD,CHARGING_ERR_CHANGE_OAD},	
	{0x30E2,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,CHARGING_COM_EXCTION_OAD,CHARGING_COM_EXCTION_CHANGE_OAD},	
	{0xBD00,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,EXT_MODULE_START_OAD,EXT_MODULE_END_OAD }, // 拓展事件0
	{0xBD01,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,EXT_MODULE_START_OAD,EXT_MODULE_END_OAD }, // 拓展事件0
	{0xBD02,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,EXT_MODULE_START_OAD,EXT_MODULE_END_OAD }, // 拓展事件0
	{0xBD03,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,EXT_MODULE_START_OAD,EXT_MODULE_END_OAD }, // 拓展事件0
	{0xBD04,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,EXT_MODULE_START_OAD,EXT_MODULE_END_OAD }, // 拓展事件0
	{0xBD05,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,EXT_MODULE_START_OAD,EXT_MODULE_END_OAD }, // 拓展事件0
	{0xBD06,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,EXT_MODULE_START_OAD,EXT_MODULE_END_OAD }, // 拓展事件0
	{0xBD07,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,EXT_MODULE_START_OAD,EXT_MODULE_END_OAD }, // 拓展事件0
	{0xBD08,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,EXT_MODULE_START_OAD,EXT_MODULE_END_OAD }, // 拓展事件0
	{0xBD09,7,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,EXT_MODULE_START_OAD,EXT_MODULE_END_OAD }, // 拓展事件0
    #if(SEL_EVENT_FIND_UNKNOWN_METER == YES)
	{0x3111,6,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,FIND_UNKNOWN_METER_OAD }, // 发现未知电能表事件
	#endif
	#if( SEL_TOPOLOGY == YES )
	{0x3E03,6,RECORDNO_OAD,STARTTIME_OAD,ENDTIME_OAD,GEN_SOURCE_OAD,NOTIFICATION_OAD,TOPO_EVENT_OAD}, // topu
    {0x4E07,3,RECORDNO_OAD,STARTTIME_OAD,TOPO_SIGNAL_OAD}, // topu
	#endif
	#if(PREPAY_MODE == PREPAY_LOCAL)
...
BOOL GetEventIndex( WORD OI, BYTE *pEventIndex )
{
	...
	FindThisEventSaveOI(&OI,0,eEventMap);
...
BOOL GetEventRecordInfo(BYTE i, TEventAddrLen *pTEventAddrLen )
{
    ...
    #if( SEL_TOPOLOGY == YES )
    if(pTEventAddrLen->EventIndex == eEVENT_TOPU_NO)
    {
        pTEventAddrLen->dwRecordAddr = FLASH_EVENT_BASE+SUB_EVENT_INDEX_MAX_NUM*EVENT_RECORD_LEN;   //放到扩展区域
    }
    else if(pTEventAddrLen->EventIndex == eEVENT_TOPU_SIGNAL_NO)
    {
        pTEventAddrLen->dwRecordAddr = FLASH_EVENT_BASE+SUB_EVENT_INDEX_MAX_NUM*EVENT_RECORD_LEN + EVENT_RECORD_TOPO_LEN;   //放到扩展区域
    }
    else
    #endif
    {
...
{	
	...
	for( i = 0; i < sizeof(SubEventInfoTab)/sizeof(TSubEventInfoTab); i++ )
	{
		if( inEventIndex == SubEventInfoTab[i].EventIndex )
		{
            ...
            #endif
			#if( SEL_EVENT_ROGOWSKI_CHANGE == YES )
			if (inEventIndex == eEVENT_ROGOWSKI_CHANGE_NO)
			{
				api_FactoryInitRogwskiSignChange();
			}
			else
			#endif
			#if( SEL_EVENT_ROGOWSKI_COMM_FAIL == YES )
			if (inEventIndex == eSUB_ROGOWSKI_COMM_FAIL_NO)
			{
				api_FactoryInitRogwskiSignFail();
			}
			else
			#endif
			if((inEventIndex == eEVENT_CHARGING_WARNING_NO) || (inEventIndex == eEVENT_CHARGING_ERR_NO) || (inEventIndex == eEVENT_CHARGING_COM_EXCTION_NO))
			{
//				api_FactoryInitChargingEvent(inEventIndex-eEVENT_CHARGING_WARNING_NO);
			}

            #if(SEL_EVENT_FIND_UNKNOWN_METER == YES)
            if( inEventIndex == eEVENT_FIND_UNKNOWN_METER_NO)
            {
            ...
            #if(SEL_TOPOLOGY == YES)
            if( inEventIndex == eEVENT_TOPU_NO)
            {
                TopoIdentiResultEventDataClear();
            }
            else if(inEventIndex == eEVENT_TOPU_SIGNAL_NO)
            {
...
BOOL api_AddEventAttribute( WORD OI, BYTE *pOad )
{
    ...
    #if( SEL_EVENT_ROGOWSKI_CHANGE == YES )
	else if(tEventIndex == eEVENT_ROGOWSKI_CHANGE_NO )//互感器线圈变更深度20
	{
		RecordDepth = EVENT_ROGOWSKI_CHANGE_NUM;
	}
    #endif
	#if( SEL_EVENT_ROGOWSKI_COMM_FAIL == YES )
	else if (tEventIndex == eSUB_ROGOWSKI_COMM_FAIL_NO)
	{
		RecordDepth = EVENT_ROGOWSKI_ERR_NUM;
	}
    #endif
    #if (SEL_EVENT_FIND_UNKNOWN_METER == YES )
    else if(tEventIndex == eEVENT_FIND_UNKNOWN_METER_NO)
    {
        RecordDepth = EVENT_FIND_UNKNOWN_METER_NUM;
    }
    #endif
	#if( SEL_TOPOLOGY == YES )
	else if (tEventIndex == eEVENT_TOPU_NO)
	{
		RecordDepth = EVENT_TOPU_NUM;
	}
    else if (tEventIndex == eEVENT_TOPU_SIGNAL_NO)
	{
		return FALSE;
	}
	#endif
	...
	if(GetEventSectorInfo(EventAttInfo.AllDataLen, RecordDepth, RecordDepth, &tEventSectorInfo) != FALSE)
	{
        ...
        #if (SEL_TOPOLOGY == YES)
        if( (OI == 0x3E03) && (tEventSectorInfo.dwSaveSpace <= (EVENT_RECORD_TOPO_LEN)) )
        {
...
BOOL api_WriteEventAttribute( WORD OI, BYTE OadType, BYTE *pOad, WORD OadNum )
{
	...
	if( (OadType==0) || (OadType==0xFF) )
	{
        ...
        {
            RecordDepth = EVENT_TOPU_SIGNAL_NUM;
        }
		#endif
		else
		{
		...
		if(GetEventSectorInfo(EventAttInfo.AllDataLen, RecordDepth, RecordDepth, &tEventSectorInfo) != FALSE)
		{
            ...
            else if( (OI == 0x4E07) && (tEventSectorInfo.dwSaveSpace <= (EVENT_RECORD_TOPO_SIGNAL_LEN)) )
            {
                EventAttInfo.MaxRecordNo = RecordDepth;//赋值最大记录数
            }
            else 
...
BOOL SaveInstantEventRecord( BYTE inEventIndex, BYTE Phase, BYTE BeforeAfter, BYTE EndTimeType )
{
	...
	if( BeforeAfter == EVENT_START )
	{
        ...
        if(inEventIndex == eEVENT_TOPU_NO)
        {
            if(WriteEventRecordData( dwAddr, sizeof(TEventOADCommonData), dwBaseAddr, EVENT_RECORD_TOPO_LEN, (BYTE *)&EventOADCommonData ) == FALSE)
            {
        ...
        {
            if(WriteEventRecordData( dwAddr, sizeof(TEventOADCommonData), dwBaseAddr, EVENT_RECORD_TOPO_SIGNAL_LEN, (BYTE *)&EventOADCommonData ) == FALSE)
            {
		...
		if( EventAttInfo.NumofOad != 0 )
		{
			...
			for( i = 0; i < EventAttInfo.NumofOad; i++ )
			{
				...
				if( (Temp != 3) && (Temp != 4) )//如果属性特征不为3,4，默认按照1,2,进行处理！！！！！！
				{
					...
					{
                        #if( SEL_EVENT_ROGOWSKI_CHANGE == YES )
						if ((inEventIndex == eEVENT_ROGOWSKI_CHANGE_NO) && (EventAttInfo.Oad[i] == ROGOWSKI_START_OAD))
						{
							TmpBuf[Offset] = RogwskiEventBuf[0];
						}
						else
						#endif
						{
            ...
            if(inEventIndex == eEVENT_TOPU_NO)
            {
                if(WriteEventRecordData( dwAddr, EventAttInfo.OadValidDataLen, dwBaseAddr, EVENT_RECORD_TOPO_LEN, TmpBuf ) == FALSE)// 存开始OAD对应的数据
                {
		...
		if((inEventIndex >= eEVENT_CHARGING_WARNING_NO) && (inEventIndex <= eEVENT_CHARGING_COM_EXCTION_NO))
		{
			dwAddr = EventAddrLen.dwRecordAddr + sizeof(TEventOADCommonData) + EventSectorInfo.dwSectorAddr + EventAttInfo.OadValidDataLen;
			if(inEventIndex == eEVENT_CHARGING_WARNING_NO)
			{
				WriteEventRecordData(dwAddr, MAX_CHARGING_W_BYTENUM * 2, dwBaseAddr, EVENT_RECORD_LEN, ChargingSaveEventBuf);
			}
			else if(inEventIndex == eEVENT_CHARGING_ERR_NO)
			{
				WriteEventRecordData(dwAddr, MAX_CHARGING_E_BYTENUM * 2, dwBaseAddr, EVENT_RECORD_LEN, ChargingSaveEventBuf);
			}
			else
			{
				WriteEventRecordData(dwAddr, MAX_CHARGING_PE_BYTENUM * 2, dwBaseAddr, EVENT_RECORD_LEN, ChargingSaveEventBuf);
			}
		}
		// 拓展事件不存在OAD  直接记录额外属性
		else if((inEventIndex >= eSUB_EVENT_MODULE_0_NO) && (inEventIndex <= eSUB_EVENT_MODULE_9_NO))
		{
			dwAddr = EventAddrLen.dwRecordAddr + sizeof(TEventOADCommonData) + EventSectorInfo.dwSectorAddr + EventAttInfo.OadValidDataLen;
			if(WriteEventRecordData(dwAddr, *(WORD *)ModuleEventBuf, dwBaseAddr, EVENT_RECORD_LEN, ModuleEventBuf) == FALSE)
			{
			}
		}
        #if(SEL_TOPOLOGY == YES)
        ...
        {
            dwAddr = EventAddrLen.dwRecordAddr + sizeof(TEventOADCommonData) + EventSectorInfo.dwSectorAddr + EventAttInfo.OadValidDataLen;
            if(WriteEventRecordData( dwAddr, sizeof(TopoCharactCurIdentiInfo_t), dwBaseAddr, EVENT_RECORD_TOPO_SIGNAL_LEN, (BYTE*)&TopoIdentiSignalData ) == FALSE)
            {
	...
	{						
		...
		if( EventAttInfo.NumofOad != 0 )
		{
			...
			for( i = 0; i < EventAttInfo.NumofOad; i++ )
			{
				...
				{                    
					...
					#endif
                    #if( SEL_EVENT_ROGOWSKI_CHANGE == YES )
					else if ((inEventIndex == eEVENT_ROGOWSKI_CHANGE_NO) && (EventAttInfo.Oad[i] == ROGOWSKI_END_OAD))
					{
						TmpBuf[Offset] = RogwskiEventBuf[1];
					}
                    #endif
					else
					{
            ...
            if(inEventIndex == eEVENT_TOPU_NO)
            {
                if(WriteEventRecordData( dwAddr, EventAttInfo.OadValidDataLen, dwBaseAddr, EVENT_RECORD_TOPO_LEN, TmpBuf ) == FALSE)
                {
        ...
        #endif
		{
            ...
            if(inEventIndex == eEVENT_TOPU_NO)
            {
                if(WriteEventRecordData( dwAddr, sizeof(TRealTimer), dwBaseAddr, EVENT_RECORD_TOPO_LEN, (BYTE *)&EventOADCommonData.EventTime.EndTime ) == FALSE)
                {
		...
		#endif
		#if (SEL_PRG_MODULE == YES)
		if ((inEventIndex >= eSUB_EVENT_MODULE_0_NO) && (inEventIndex <= eSUB_EVENT_MODULE_9_NO))
		{
			dwAddr = EventAddrLen.dwRecordAddr+sizeof(TEventOADCommonData) + EventSectorInfo.dwSectorAddr;
			if(api_ReadMemRecordData(dwAddr,2,ModuleEventbuf))
			{
				
			}
			dwAddr += *(WORD*)ModuleEventbuf;
			if(WriteEventRecordData( dwAddr, *(WORD*)ModuleEventBuf, dwBaseAddr, EVENT_RECORD_LEN,  ModuleEventBuf ) == FALSE)
			{
		...
		#endif
        #if(SEL_EVENT_FIND_UNKNOWN_METER == YES)
		else if(inEventIndex == eEVENT_FIND_UNKNOWN_METER_NO)
		{
			dwAddr = EventAddrLen.dwRecordAddr + sizeof(TEventOADCommonData) + EventSectorInfo.dwSectorAddr + EventAttInfo.OadValidDataLen;
			if(WriteEventRecordData(dwAddr, sizeof(FindUnknownMeterEventData_t), dwBaseAddr, EVENT_RECORD_LEN, (BYTE*)&gFindUnknownMeterEventDataRAM) == FALSE)
			{
		...
		#endif
        #if(SEL_TOPOLOGY == YES)
		else if(inEventIndex == eEVENT_TOPU_NO)
		{   
			dwAddr = EventAddrLen.dwRecordAddr + sizeof(TEventOADCommonData) + EventSectorInfo.dwSectorAddr + EventAttInfo.OadValidDataLen;
			if(WriteEventRecordData(dwAddr, sizeof(TopoIdentiResultRAM), dwBaseAddr, EVENT_RECORD_TOPO_LEN, (BYTE*)&TopoIdentiResultRAM) == FALSE)
			{
...
static WORD GetDataByOad( TEventData *pData )
{
	...
	BYTE Temp,i,OadIndex,tmpBuf[EVENT_ALONE_OADLEN+30],ExtModtmpbuf[0x304+30];
	BYTE *pTemp;
	BYTE Buf[30],Buf1[30],Buf2[30];
	...
	else if (pData->dwOad == CHARGING_WARNING_OAD)
	{
		wLen=2+MAX_CHARGING_W_BYTENUM;
		if( (pData->off+wLen) > pData->Len )
		{
			return 0xFFFF;// 已满
		}
		dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr+pData->pEventAttInfo->OadValidDataLen;
		api_ReadMemRecordData( dwTemp, MAX_CHARGING_W_BYTENUM, Buf2 );
		(pData->pBuf+pData->off)[0]=0x04;
		(pData->pBuf+pData->off)[1]=MAX_CHARGING_W_BYTENUM*8;
		memcpy((pData->pBuf+pData->off+2),Buf2,MAX_CHARGING_W_BYTENUM);
	}
	else if (pData->dwOad == CHARGING_ERR_OAD)
	{
		wLen=2+MAX_CHARGING_E_BYTENUM;
		if( (pData->off+wLen) > pData->Len )
		{
			return 0xFFFF;// 已满
		}
		dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr+pData->pEventAttInfo->OadValidDataLen;
		api_ReadMemRecordData( dwTemp, MAX_CHARGING_E_BYTENUM, Buf2 );
		(pData->pBuf+pData->off)[0]=0x04;
		(pData->pBuf+pData->off)[1]=MAX_CHARGING_E_BYTENUM*8;
		memcpy((pData->pBuf+pData->off+2),Buf2,MAX_CHARGING_E_BYTENUM);

	}
	else if (pData->dwOad == CHARGING_COM_EXCTION_OAD)
	{
		wLen=2+MAX_CHARGING_PE_BYTENUM;
		if( (pData->off+wLen) > pData->Len )
		{
			return 0xFFFF;// 已满
		}
		dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr+pData->pEventAttInfo->OadValidDataLen;
		api_ReadMemRecordData( dwTemp, MAX_CHARGING_PE_BYTENUM, Buf2 );
		(pData->pBuf+pData->off)[0]=0x04;
		(pData->pBuf+pData->off)[1]=MAX_CHARGING_PE_BYTENUM*8;
		memcpy((pData->pBuf+pData->off+2),Buf2,MAX_CHARGING_PE_BYTENUM);
	}
	else if (pData->dwOad == CHARGING_WARNING_CHANGE_OAD)
	{
		wLen=2+MAX_CHARGING_W_BYTENUM;
		if( (pData->off+wLen) > pData->Len )
		{
		...
		dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr+pData->pEventAttInfo->OadValidDataLen+MAX_CHARGING_W_BYTENUM;
		api_ReadMemRecordData( dwTemp, MAX_CHARGING_W_BYTENUM, Buf2 );
		(pData->pBuf+pData->off)[0]=0x04;
		(pData->pBuf+pData->off)[1]=MAX_CHARGING_W_BYTENUM*8;
		memcpy((pData->pBuf+pData->off+2),Buf2,MAX_CHARGING_W_BYTENUM);
	}
	else if(pData->dwOad == CHARGING_ERR_CHANGE_OAD)
	{
		wLen=2+MAX_CHARGING_E_BYTENUM;
		if( (pData->off+wLen) > pData->Len )
		{
		...
		dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr+pData->pEventAttInfo->OadValidDataLen+MAX_CHARGING_E_BYTENUM;
		api_ReadMemRecordData( dwTemp, MAX_CHARGING_E_BYTENUM, Buf2 );
		(pData->pBuf+pData->off)[0]=0x04;
		(pData->pBuf+pData->off)[1]=MAX_CHARGING_E_BYTENUM*8;
		memcpy((pData->pBuf+pData->off+2),Buf2,MAX_CHARGING_E_BYTENUM);
	}
	else if(pData->dwOad == CHARGING_COM_EXCTION_CHANGE_OAD)
	{
		wLen=2+MAX_CHARGING_PE_BYTENUM;
		if( (pData->off+wLen) > pData->Len )
		{
		...
		dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr+pData->pEventAttInfo->OadValidDataLen+MAX_CHARGING_PE_BYTENUM;
		api_ReadMemRecordData( dwTemp, MAX_CHARGING_PE_BYTENUM, Buf2 );
		(pData->pBuf+pData->off)[0]=0x04;
		(pData->pBuf+pData->off)[1]=MAX_CHARGING_PE_BYTENUM*8;
		memcpy((pData->pBuf+pData->off+2),Buf2,MAX_CHARGING_PE_BYTENUM);
	}
	else if (pData->dwOad == EXT_MODULE_START_OAD )
	{
		dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr;
		api_ReadMemRecordData( dwTemp, 0x304, ExtModtmpbuf );
		if (*(WORD*)ExtModtmpbuf==2)//没发生单元  程序有问题
		{
			*(pData->pBuf+pData->off)=0x00;
			*(pData->pBuf+pData->off+1)=0x00;
			return 2;
		}
		wLen=*(WORD*)ExtModtmpbuf-1;
		if( (pData->off+wLen) > pData->Len )
		{
			return 0xFFFF;// 已满
		}
		memcpy(pData->pBuf+pData->off,ExtModtmpbuf+2,(*(WORD*)ExtModtmpbuf)-2);
		if (*(WORD*)(ExtModtmpbuf+*(WORD*)ExtModtmpbuf)>=0x300)
		{
			*(pData->pBuf+pData->off-2+*(WORD*)ExtModtmpbuf)=0x00;
		}
		else
		{
			wLen=*(WORD*)ExtModtmpbuf+*(WORD*)(ExtModtmpbuf+*(WORD*)ExtModtmpbuf)-4;
			if( (pData->off+wLen) > pData->Len )
			{
				return 0xFFFF;// 已满
			}
			memcpy(pData->pBuf+pData->off-2+*(WORD*)ExtModtmpbuf,ExtModtmpbuf+(*(WORD*)ExtModtmpbuf)+2,(WORD)*(ExtModtmpbuf+(WORD)*ExtModtmpbuf)-2);
		}
	}
    #if(SEL_EVENT_FIND_UNKNOWN_METER == YES)
	else if ( pData->dwOad == FIND_UNKNOWN_METER_OAD )
	{
        if( (pData->Last==1) && (pData->status==1) )
        {
            //事件未结束
            wLen = api_GetFindUnknownMeter(&gFindUnknownMeterEventDataRAM,pData->pBuf+pData->off);
        }
        else
        {
            dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr+pData->pEventAttInfo->OadValidDataLen;
		    api_ReadMemRecordData( dwTemp, sizeof(FindUnknownMeterEventData_t), ExtModtmpbuf );
            wLen = api_GetFindUnknownMeter((FindUnknownMeterEventData_t*)ExtModtmpbuf,pData->pBuf+pData->off);
        }

        if( (wLen == 0) || ( (pData->off+wLen) > pData->Len) )
		{
			return 0xFFFF;// 已满
		}
	}
	#endif
    #if(SEL_TOPOLOGY == YES)
	else if ( pData->dwOad == TOPO_EVENT_OAD )
	{
        if( (pData->Last==1) && (pData->status==1) )
        {
            //事件未结束
            wLen = api_GetTopoIdentEvent698(&TopoIdentiResultRAM,pData->pBuf+pData->off);
        }
        else
        {
            dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr+pData->pEventAttInfo->OadValidDataLen;
		    api_ReadMemRecordData( dwTemp, sizeof(TopoIdentiResult_t), ExtModtmpbuf );
            wLen = api_GetTopoIdentEvent698((TopoIdentiResult_t*)ExtModtmpbuf,pData->pBuf+pData->off);
        }
    ...
    else if( pData->dwOad == TOPO_SIGNAL_OAD )
    {
        dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr+pData->pEventAttInfo->OadValidDataLen;
        api_ReadMemRecordData( dwTemp, sizeof(TopoCharactCurIdentiInfo_t), pData->pBuf+pData->off );
        wLen = sizeof(TopoCharactCurIdentiInfo_t);
	...
	#endif
	else if (pData->dwOad == EXT_MODULE_END_OAD )//上面已全部处理完
	{
		return 0;
	}
	#if( SEL_PRG_INFO_CLEAR_EVENT == YES )
...
static WORD GetDataByOad645( BYTE NoDataFlag, TEventData *pData )
{
	...
	else if ( pData->dwOad == TOPO_EVENT_OAD )
	{
        ...
        {
            //事件未结束
            memcpy(pData->pBuf+pData->off,&TopoIdentiResultRAM,sizeof(TopoIdentiResult_t));
            wLen = sizeof(TopoIdentiResult_t);
        }
        else
        {
            dwTemp = pData->pEventAddrLen->dwRecordAddr+sizeof(TEventOADCommonData)+pData->pEventSectorInfo->dwSectorAddr+pData->pEventAttInfo->OadValidDataLen;
		    api_ReadMemRecordData( dwTemp, sizeof(TopoIdentiResult_t), pData->pBuf+pData->off);
            wLen = sizeof(TopoIdentiResult_t);
        }

        if(NoDataFlag == 0x55) //事件未发生
        {
            return 0x8000;
        }
...
//备注: 只支持上Last条记录
//-----------------------------------------------
WORD ReadEvent645RecordByTime( TDLT698RecordPara *pDLT698RecordPara, WORD Len, BYTE *pBuf )
{
	TEventOADCommonData EventOADCommonData;
	TEventOADCommonData EventOADCommonDataBak;
	TEventAttInfo		EventAttInfo;
	...
	return ReadEvent645RecordSub( 0x00,&Data );
}
...
BYTE GetEventDelayTime(BYTE EventOrder)
{
	BYTE DelayTime;
	#if( SEL_PRG_MODULE == YES )
	if((EventOrder >= eSUB_EVENT_MODULE_0_NO)&&(EventOrder <= eSUB_EVENT_MODULE_9_NO))
	{
		return 1;
	}
	#endif
	#if( SEL_EVENT_STEAL_ELECTRICITY == YES )
	if(EventOrder == eEVENT_STEAL_ELECTRICITY_NO)
	{
		return 1;
	}
	#endif
	...
	#endif
	#if( SEL_EVENT_DI_CHANGE == YES )
	else if(EventOrder == eEVENT_DI_CHANGE_NO)
	{
		//遥信变位取1秒
		DelayTime = 1;
	}
	#endif
	#if( SEL_EVENT_ROGOWSKI_CHANGE == YES )
	else if(EventOrder == eEVENT_ROGOWSKI_CHANGE_NO)
	{
		//取1秒
		DelayTime = 1;
	}
	#endif
	#if( SEL_EVENT_ROGOWSKI_COMM_FAIL == YES )
	else if(EventOrder == eSUB_ROGOWSKI_COMM_FAIL_NO)
	{
		//取1秒
		DelayTime = 1;
	}
	#endif
    #if(SEL_EVENT_FIND_UNKNOWN_METER == YES)
	else if(EventOrder == eEVENT_FIND_UNKNOWN_METER_NO)
	{
		//取1秒
		DelayTime = 1;
	}
    #endif
    #if(SEL_TOPOLOGY == YES)
	else if(EventOrder == eEVENT_TOPU_NO)
	{
		//取1秒
		DelayTime = 1;
	}
...
//备注:   
//-----------------------------------------------
static void SetEventSysStatus(BYTE inSubEventIndex,BYTE *pStatus)
{
	BYTE i, j;
...
//备注:   
//-----------------------------------------------
static void ClrEventSysStatus(BYTE inSubEventIndex,BYTE *pStatus)
{
	BYTE i, j;
...
//备注:   
//-----------------------------------------------
static BOOL GetEventSysStatus(BYTE inSubEventIndex,BYTE *pStatus)
{
	BYTE i, j;
...
//								eSET_EVENT_STATUS
//								eGET_EVENT_STATUS    
// 								eCLEAR_EVENT_STATUS_CURR
// 								eSET_EVENT_STATUS_CURR  
// 								eGET_EVENT_STATUS_CURR                  
//			inSubEventIndex[in]	eSUB_EVENT_INDEX
//                    
...
{	
	...
	{
		Type-=0x80;
		pStatus = EventStatus.CurrentStatus;
	}
	else
	{
		pStatus = EventStatus.DelayedStatus;
	}
	
	if( Type == eGET_EVENT_STATUS )
	{
		if( GetEventSysStatus(inSubEventIndex,pStatus) == TRUE )
		{
			Status = 1;
		}
	}
	...
	{
		SetEventSysStatus(inSubEventIndex,pStatus);
	}
	else if( Type == eCLEAR_EVENT_STATUS )
	{
		ClrEventSysStatus(inSubEventIndex,pStatus);
	}
	else
	{
		ASSERT(0, 0);	
...
void api_PowerUpEvent(void)
{
	...
	#if( (SEL_EVENT_ROGOWSKI_CHANGE == YES) || (SEL_EVENT_ROGOWSKI_COMM_FAIL == YES) )
	api_SampleRogwskPowerUp();//上电罗氏线圈相关事件处理
	#endif
...
WORD DealEventParaByEventOrder( BYTE Operation, BYTE inEventIndex, BYTE ParaType, BYTE *pBuf )
{
	...
	else if( ParaType == eTYPE_SPEC1 )
	{
		...
		if( inEventIndex == eEVENT_ROGOWSKI_CHANGE_NO )	
		{
			dwAddr = GET_CONTINUE_ADDR( EventConRom.EventPara.wRogowskiADelay );	
			pTemp = (BYTE *)&gEventPara.wRogowskiADelay;
			wLen = 2;
		}
		#endif
		...
		if( inEventIndex == eSUB_ROGOWSKI_COMM_FAIL_NO )	
		{
			dwAddr = GET_CONTINUE_ADDR( EventConRom.EventPara.bRetryNo );	
			pTemp = (BYTE *)&gEventPara.bRetryNo;
			wLen = 1;
		}
		#endif
	...
	else if( ParaType == eTYPE_SPEC2 )
	{
		...
		if( inEventIndex == eEVENT_ROGOWSKI_CHANGE_NO )	
		{
			dwAddr = GET_CONTINUE_ADDR( EventConRom.EventPara.wRogowskiBDelay );	
			pTemp = (BYTE *)&gEventPara.wRogowskiBDelay;
			wLen = 2;
		}
		#endif
		...
		if( inEventIndex == eSUB_ROGOWSKI_COMM_FAIL_NO )	
		{
			dwAddr = GET_CONTINUE_ADDR( EventConRom.EventPara.bCollectNo );	
			pTemp = (BYTE *)&gEventPara.bCollectNo;
			wLen = 1;
		}
		#endif
...
void FactoryInitParaEventLimit( void )
{
	...
	gEventPara.dwLimit[eEVENT_LOST_V_NO] = (DWORD)dwMeterBasicCurrentConst * EventLostVIRateConst / 100;//0.05Ib
	...
	gEventPara.dwLimit[eEVENT_BREAK_NO] = (DWORD)dwMeterBasicCurrentConst * EventBreakIRateConst / 100; //0.005Ib
	#endif
	...
	gEventPara.dwLimit[eEVENT_LOST_I_NO] = (DWORD)dwMeterBasicCurrentConst * EventLostIIRateConst / 100;	//0.005Ib
	gEventPara.LostILimitLI = (DWORD)dwMeterBasicCurrentConst * EventLostILimitLIConst / 10; //5%额定电流 四位小数
	#endif
	...
	gEventPara.dwLimit[eEVENT_BREAK_I_NO] = (DWORD)dwMeterBasicCurrentConst * EventBreakIIRateConst / 100; //0.005Ib
	#endif

	//功率反向 0x3007
	#if( SEL_EVENT_BACKPROP == YES )
	gEventPara.byLimit[eEVENT_BACKPROP_NO] = EventBackpTimeConst;
	gEventPara.dwLimit[eEVENT_BACKPROP_NO] = (DWORD)wStandardVoltageConst * dwMeterBasicCurrentConst * EventBackpPRateConst / 1000000L; //0.5%基本功率
	#endif

	//过载 0x3008
	#if( SEL_EVENT_OVERLOAD == YES )
	gEventPara.byLimit[eEVENT_OVERLOAD_NO] = EventOverLoadTimeConst;
	#if( cCURR_TYPES == CURR_500A )
	gEventPara.dwLimit[eEVENT_OVERLOAD_NO] = (DWORD)wStandardVoltageConst * ( dwMeterMaxCurrentConst /1000 ) * EventOverLoadPRateConst / 10L; //1.2最大功率
	#else
	...
	#if( cCURR_TYPES == CURR_500A )
	dwTmp = (DWORD)wStandardVoltageConst * (dwMeterMaxCurrentConst/ 1000) * EventDemandOverRateConst; //单相Un*1.2Imax 1位小数，单位w，此处放大10倍，后面除以10
	#else
	...
	gEventPara.dwLimit[eEVENT_NEUTRAL_CURRENT_ERR_NO] = dwMeterBasicCurrentConst * EventNeutralCurrentErrLimitIConst / 10000;//20%,电流触发下限  double-long（单位：A，换算：-4），
	...
	//罗氏线圈变更事件
	#if( SEL_EVENT_ROGOWSKI_CHANGE == YES )
	gEventPara.wRogowskiADelay = EventRogowskiADelayConst;
	gEventPara.wRogowskiBDelay = EventRogowskiBDelayConst;
	#endif

	//终端抄表失败事件
	#if( SEL_EVENT_ROGOWSKI_COMM_FAIL == YES )
	gEventPara.bRetryNo = EventRetryNoConst;
	gEventPara.bCollectNo = EventCollectNoConst;
	#endif
...
void api_FactoryInitEvent( void )
{
	...
	#if( SEL_EVENT_DI_CHANGE == YES )
	api_FactoryInitDIChangePara();
	#endif

    #if(SEL_EVENT_FIND_UNKNOWN_METER == YES)
    api_FindUnknownMeterEventPara();
    #endif

    #if(SEL_TOPOLOGY == YES)
    api_TopoInitEventPara();
    #endif
	
	FactoryInitChargingPara();
	
	FactoryInitPrgRecordPara();

	FactoryInitRogowskiPara();
