...
//刷新表内温度
void FreshTemperature( BYTE chipNo )
{
	short Result;
	
	//温度状态
	Result = api_GetTemp(chipNo);
	
	SysWatchRamData.Temperature[chipNo] = (short)Result;
}
...
//备注:   
//-----------------------------------------------
void api_PowerUpSysWatch(BYTE chipNo)
{
	WakeUpTimer = 0;
	CheckBatteryPara();
...
WORD api_GetBatAndTempValue( BYTE DataType, BYTE Channel )
{
	...
	switch( Channel )
	{
		...
		case SYS_TEMPERATURE_AD:
			Result = SysWatchRamData.Temperature[0];
			break;
		default://!!!!!!
