...
//
//----------------------------------------------------------------------
#ifndef __RELAY_Local_API_H
#define __RELAY_Local_API_H
...
typedef enum
{
	...
	eST_RlyOffGd_L,				//本地拉闸大电流					内置表
	eST_AdvOffGd_L,				//本地预跳闸大电流					内置表
	eST_Error					//错误状态
}eRELAY_ST;
...
{
	eNO_OPERATE = 0,				//无操作
	eCMD_OPERATE,					//命令操作 记录事件记录
	ePOWERON_OPERATE,				//上电、初始化、定时、错误强驱  不记录事件记录
}eRELAY_OPERATE;
//-----------------------------------------------
//		本文件使用的结构体，共用体，枚举
//-----------------------------------------------
...
{
	eRELAY_ST	   Status;				//当前继电器状态
	eRELAY_CMD	   Command;				//当前继电器命令
	eLOCAL_STATUS  LastLocalStatus;		//本地状态
