...
void api_ParaTask( void )
{
	...
	if( api_GetTaskFlag( eTASK_PARA_ID, eFLAG_HOUR ) == TRUE )
	{
		api_CheckWaveParaAndUIK();
		api_ClrTaskFlag( eTASK_PARA_ID, eFLAG_HOUR );
	}
}
...
//备注:	  
//-----------------------------------------------
void FactoryInitCharge(void)
{
	TChargePara tmpChargePara;

	memcpy( (BYTE *)&tmpChargePara, (BYTE *)&ChargeParaConst, sizeof(ChargeParaConst) );

	api_VWriteSafeMem( GET_SAFE_SPACE_ADDR(SampleSafeRom.ChargePara), sizeof(tmpChargePara),(BYTE *)&tmpChargePara);
}
//-----------------------------------------------
//函数功能: 初始化参数到默认值
//
