...
void DateCall_Task(void *profileID)
{
	...
	static BYTE WritePowerOnEveFlag = 0;

	while(1)
	{
		ThreadRunCnt[eDateCallThread]++;
		switch(s_NetRunStep)
		{
		case 0:   // 0.延时 30
			nwy_sleep(1000);
		...
		case 5:
            ...
            break;
        case 6: // 6. 连接成功
            if (WritePowerOnEveFlag == 0)
            {
                // 记录上电事件
                api_WriteSysUNMsg(SYSUN_POWER_ON);
                WritePowerOnEveFlag = 1;
            }
