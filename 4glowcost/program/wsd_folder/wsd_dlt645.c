...
void  OpenSystemProgram(void)
{
	...
	nwy_uart_write(UART_HD,Buf,sizeof(Buf));
}
//--------------------------------------------------
//功能描述:  探测基表地址
//
...
void Adrr_Txd(void)
{
	...
	nwy_uart_write(UART_HD, reqAddr, sizeof(reqAddr));
}
//--------------------------------------------------
//功能描述:  645 扩展规约读取 版本
//
...
void Dlt645_Tx_Read(DWORD dwID) //抄读OK
{
	...
	bLen++;
	Buf[bLen++] = 0x16;
	nwy_uart_write(UART_HD, Buf, bLen);
}
//--------------------------------------------------
...
BOOL api_CheckorSetPosition(BYTE bMode)
{
	...
	if ((TLocation.longitude.degree == 0) && (TLocation.latitude.degree == 0))
	{
		...
		if (bMode != 0)
		{
			nwy_gpio_value_set(LOCAL_LIGHT,1);
		}
		//本地灯亮
		return FALSE;
	}
...
void ReceiveClearBitFlag(void)
{
	...
	qwReadMeterFlag[Loop] &= ~(1 << bHisReadMeter[Loop]);
...
BOOL DealDLT645_2007(DWORD dwID, BYTE *pBuf, BYTE Len) //直接拷贝多字节数据 无需倒叙
{
	...
	else if (dwID == WARNING_REPORT)
	{
		...
		if (wTemp != bHisWarning)
		{
			...
			{	
				#if (EVENT_REPORT == YES)
				if (nwy_msg_queue_space_num_get(EventReportMessageQueue) != 0)//状态告警
				{
	...
	else
	{
		...
		for (i = 0; i < CycleReadMeterNum; i++)
		{
			if (dwID == MeterReadOI[i].OI)
			{
				...
				bOffset = Loop * (COLLECT_DATA_ITEM_NUM);
