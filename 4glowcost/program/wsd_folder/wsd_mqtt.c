...
char Message[NWY_PAHO_MSG_LEN_MAX] = {0};//！！！ 缓存 不确定对不对应
#if (MQTT_USER == YES)
//断点续传相关
...
nwy_osi_thread_t nwy_paho_usertask_id = NULL;
...
//通用发布主题名
static char general_pub_topic[64] = "/devices/JLXF_pub";
#if (EVENT_REPORT == YES)
static char c_tail_eventReport[] = "/devices/JLXF/event/warning";
#endif
//通用订阅主题名
static char general_sub_topic[64] = "/devices/JLXF_sub";
//发布主题尾
...
static BOOL pub_reportEventdata(Eventmessage EventData)
{
	...
	char Addru[30] = {0};
	char tTopic[NWY_PAHO_TOPIC_LEN_MAX+1] = {0};//！！！ 不確定 用 局部变量合不合适 记得有方会使用这个地址

	memcpy(Addru, LtuAddr, sizeof(LtuAddr));
	memset(tTopic, 0, sizeof(tTopic));
	strncpy(tTopic, &c_tail_eventReport[0], strlen(c_tail_eventReport));
	...
	strncpy(Message, c_message, strlen(c_message));
	nwy_ext_echo("\r\nmqttpub topic = %s, msg = %s", tTopic,Message);
	...
	pubmsg.retained = 10;
	pubmsg.dup = 0;
	int rc = MQTTPublish(&paho_user_client, tTopic, &pubmsg);
	if (rc)
	{
...
// 备注:
//--------------------------------------------------
char tTopic[NWY_PAHO_TOPIC_LEN_MAX+1] = {0};//！！！ 记得有方会使用这个地址 先用全局测试
BOOL RemessageF(const char *type, char *topic_p, char *fmt, ...)
{
	nwy_ext_echo("test log\r\n");
	va_list args;
	va_start(args, fmt);
	const char *fs;
	MQTTMessage pubmsg = {0};
	...
	// 准备主题
	memset(tTopic, 0, sizeof(tTopic));
	strncpy(tTopic, topic_p, strlen(topic_p));
	...
	va_end(args);
	cJSON_AddNumberToObject(pJsonRoot, "timestamp", (double)getmktimems(&tTime));
	cJSON_AddStringToObject(pJsonRoot, "meterVersion", MeterVersion);
	cJSON_AddStringToObject(pJsonRoot, "meterAddr", LtuAddr);
	c_message = cJSON_PrintUnformatted(pJsonRoot);
	// 发布部分
	memset(Message, 0, sizeof(Message));
	nwy_ext_echo("str len of c_message: %d\r\n", strlen(c_message));
	strncpy(Message, c_message, strlen(c_message));
	nwy_ext_echo("\r\nmqttpub param topic = %s, msg = %s", tTopic, Message);
	memset(&pubmsg, 0, sizeof(pubmsg));
	pubmsg.payload = (void *)Message;
	pubmsg.payloadlen = strlen(Message);
	pubmsg.qos = 0;
	pubmsg.retained = 10;
	pubmsg.dup = 0;
	int rc = MQTTPublish(&paho_mqtt_client, tTopic, &pubmsg);
	if (rc)
	{
		free(c_message);
		cJSON_Delete(pJsonRoot);
...
void pubTranData(tTranData *data)
{
	...
	// 准备主题
	memset(tTopic, 0, sizeof(tTopic));
	strncpy(tTopic, TransitRespondTopic, strlen(TransitRespondTopic));


	// 发布部分
	memset(Message, 0, sizeof(Message));
	memcpy(Message, data->buf, data->len);
	memset(&pubmsg, 0, sizeof(pubmsg));
	pubmsg.payload = (void *)Message;
...
void StartUpgradeRemessage(FtpUpdatePara *ftpupdatepara)
{
	...
	RemessageF("CMD_DEVICE_UPGRADE", &general_pub_topic[0], "ds", "code", 200.0l, "msg", "SUCCESS");
			...
			// nwy_ext_echo("\r\n put queue ftp password is %s",ftpupdatepara->ftp_param.password);
			// nwy_ext_echo("\r\n put queue ftp password is %p",ftpupdatepara->ftp_param.password);
			// nwy_ext_echo("\r\n put queue ftpupdatepara password is %s",ftpupdatepara->PassWord);
			// nwy_ext_echo("\r\n put queue ftpupdatepara password is %p",ftpupdatepara->PassWord);
			
	// dump_hex(ftpupdatepara, sizeof(FtpUpdatePara));
	ret = nwy_msg_queue_send(FtpUpdateMessageQueue,sizeof(char*), &ftpupdatepara, 10);
	if ( ret== NWY_SUCCESS)
	{
		nwy_ext_echo("\r\n put message success");
	}
...
void MQTT_Services(void)
{
	...
	// 接收主站的MQTT帧
	if (nwy_msg_queue_recv(MqttResMessageQueue,sizeof(char*),&msg,NWY_OSA_NO_SUSPEND) == NWY_SUCCESS) // 等待一秒
	{
		...
		{ // 升级
			pFtpupdatepara = (FtpUpdatePara *)malloc(sizeof(FtpUpdatePara));
			if (pFtpupdatepara == NULL)
			{
				nwy_ext_echo("\r\n malloc FtpUpdatePara failed");
				RemessageF("CMD_DEVICE_UPGRADE", &general_pub_topic[0], "ds", "code", 200.0l, "msg", "malloc failed");
				return;
			}
			memset(pFtpupdatepara, 0, sizeof(FtpUpdatePara));

			cJSON_GetObjectValue(pRoot, "ftp_name", ecJSON_String,pFtpupdatepara->filename, sizeof(pFtpupdatepara->filename));

			cJSON_GetObjectValue(pRoot, "ftp_ip", ecJSON_String,pFtpupdatepara->host, sizeof(pFtpupdatepara->host));
			pFtpupdatepara->ftp_param.host = pFtpupdatepara->host;

			cJSON_GetObjectValue(pRoot, "ftp_port", ecJSON_Int, &GetTemp, sizeof(GetTemp));
			pFtpupdatepara->ftp_param.port = GetTemp;

			cJSON_GetObjectValue(pRoot, "ftp_user", ecJSON_String,pFtpupdatepara->username, sizeof(pFtpupdatepara->username));

			cJSON_GetObjectValue(pRoot, "ftp_pass", ecJSON_String,pFtpupdatepara->PassWord, sizeof(pFtpupdatepara->PassWord));
			pFtpupdatepara->ftp_param.username = pFtpupdatepara->username;
			pFtpupdatepara->ftp_param.password = pFtpupdatepara->PassWord;

			nwy_ext_echo("\r\nmqtt ftp password:%s", pFtpupdatepara->ftp_param.password);
			nwy_ext_echo("\r\nmqtt ftpupdatepara password:%s", pFtpupdatepara->PassWord);
			...
			if (strcmp(reqbuf, "Meter_upgrade_698") == 0)
			{
				if (tIap_698.dwIapFlag == 0)
				{
					...
					StartUpgradeRemessage(pFtpupdatepara);
				}
		...
		else if (strstr(pType->valuestring, "CMD_DEVICE_SINGAL"))
		{
			...
			nwy_nw_csq_get(0,&csq_val);
			double temp = csq_val.csq_rssi_level;
	...
	if (nwy_msg_queue_recv(TranDataToMqttServerMsgQue, sizeof(char *), &UploadTranDataPtr, NWY_OSA_NO_SUSPEND) == NWY_SUCCESS)
	{
	...
	if (nwy_msg_queue_recv(UpgradeResultMessageQueue,sizeof(BYTE), &UpgradeReply, NWY_OSA_NO_SUSPEND) == NWY_SUCCESS)
	{
		switch (UpgradeReply)
		{
		case eUpgradeExeSuc:
...
void TransmessageArrived(MessageData *md)
{
	...
	if (QueueWrite(&MessageQueueInfoArr[eQUEUE_TRAN_TO_UART], &TouartTranData, sizeof(tTranData), NWY_OSA_NO_SUSPEND) != TRUE)
	{
		nwy_ext_echo("mqtt to uart failed");
	}
}
...
void messageArrived(MessageData *md)
{
	...
	// else
	{
		...
		nwy_msg_queue_space_num_get(MqttResMessageQueue,&FreeQueue);
		if (FreeQueue == 0)
		{
			nwy_ext_echo("NEED_SPACE");
			return;
		}
		msg = (char *)malloc(md->message->payloadlen + 1);//加上字符串结束符
		if (msg == NULL)
		{
			nwy_ext_echo("NEED_RAM");
			return;
		}
		memset(msg, 0, md->message->payloadlen + 1);//加上字符串结束符
		memcpy(msg, md->message->payload, md->message->payloadlen);
		if (nwy_msg_queue_send(MqttResMessageQueue, sizeof(char*),&msg, NWY_OSA_NO_SUSPEND) != NWY_SUCCESS)
		{
			free(msg);
		}
	}
...
nwy_osi_thread_t nwy_paho_yeild_task_init(void)
{
	if (nwy_paho_task_id == NULL)
	{
		...
		nwy_thread_create(&task_id,"neo_paho_yeild_task",NWY_OSI_PRIORITY_NORMAL,nwy_paho_cycle, NULL,4, 1024 * 8,NULL );
...
void MQTT_connet(void)
{
	...
	/*code*/
	// 连接服务器
	memcpy(host_name, ServerIP, sizeof(Serverpara[1].ServerIP));
	Port = ServerPort;
	...
	if (NULL == (g_nwy_paho_writebuf = (unsigned char *)malloc(NWY_PAHO_PLAYOAD_LEN_MAX)))//NWY_PAHO_PLAYOAD_LEN_MAX 更新SDK后  宏定义需修改 否则上传数据量过多时异常！！！
	{
	...
	if (NULL == (g_nwy_paho_readbuf = (unsigned char *)malloc(NWY_PAHO_PLAYOAD_LEN_MAX)))
	{
		nwy_ext_echo("\r\nmalloc buffer g_nwy_paho_readbuf error");
		return;
	}
	memset(g_nwy_paho_readbuf, 0, NWY_PAHO_PLAYOAD_LEN_MAX);
	memset(g_nwy_paho_writebuf, 0, NWY_PAHO_PLAYOAD_LEN_MAX);
	...
	paho_mqtt_at_param.cleansession = 0;
	paho_mqtt_at_param.keepalive = 60;
	paho_network->cid = 0;
	int rc = NetworkConnect(paho_network, host_name, Port);
	if (rc < 0)
	{
	...
	MQTTClientInit(&paho_mqtt_client, paho_network, 10000, g_nwy_paho_writebuf, NWY_PAHO_PLAYOAD_LEN_MAX, g_nwy_paho_readbuf, NWY_PAHO_PLAYOAD_LEN_MAX);
	...
	nwy_ext_echo("\r\nConnecting MQTT");
	rc = MQTTConnect(&paho_mqtt_client, &data);
	if (rc)
	{
	...
	if (MQTTIsConnected(&paho_mqtt_client))
	{
		...
		for (int i = 0; i <sizeof(device_sub_topics_tail)/sizeof(device_sub_topics_tail[0]); i++)
		{
			...
			strncpy(tTopic, Sub_topic, strlen(Sub_topic));
			nwy_ext_echo("\r\n topic_sub:[%s][%s]\r\n", tTopic, Sub_topic);

			rc = MQTTSubscribe(&paho_mqtt_client, (char *)tTopic, bQos, messageArrived);
			if (rc == SUCCESS)
			{
				nwy_ext_echo("\r\n-------------------MQTT Sub ok \r\n ");
			}
		...
		//订阅general_sub_topic
		MQTTSubscribe(&paho_mqtt_client, general_sub_topic, bQos, messageArrived);
		...
		MQTTSubscribe(&paho_mqtt_client, TransitSingleTopic, bQos, TransmessageArrived);
		rc = MQTTSubscribe(&paho_mqtt_client, TransitCommonTopic, bQos, TransmessageArrived);
		if (rc == SUCCESS)
		{
...
void WSD_MQTT_Task(void *param)
{
	...
	nwy_dm_imei_get(0,imei,sizeof(imei));
	memcpy(MqttID, imei, sizeof(imei));
	DWORD Connectfre = 0;
		...
		if (0 != nwy_ext_check_data_connect())
		{
			if (MQTTIsConnected(&paho_mqtt_client) != 1)
			{
				...
				if (MQTTIsConnected(&paho_mqtt_client) != 1) //没有连接成功，再进行延时，连接成功不再进行延时
				{
					...
					{
						Connectfre++;
						nwy_thread_sleep(6000 * Connectfre);
					}
					else
					{
						nwy_thread_sleep(3600000 * 4); // 4小时重试一次连接 每次连接消耗560字节
					}
				}
				else
				{
...
void PowerDownSaveRamHeader(void)
{
	...
	if (nwy_file_exist(COLLECTED_DATA_FILE_PATH))
	{
		fd = nwy_file_open(COLLECTED_DATA_FILE_PATH, NWY_RB_PLUS_MODE);
		if (fd >= 0)
		{
			nwy_file_seek(fd, 0, NWY_SEEK_SET);
			nwy_ext_echo("\r\nheader crc16 is %04X", RamHeader.crc16);
			nwy_file_write(fd, &RamHeader, sizeof(RamHeader));
			nwy_file_close(fd);
		}
		else
		{
...
int CreateNewFile(void)
{
	...
	fd = nwy_file_open(COLLECTED_DATA_FILE_PATH, NWY_WB_PLUS_MODE);
	...
	nwy_file_write(fd, &RamHeader, sizeof(SaveDataInfo));
	if (fd < 0)
	{
...
BYTE CheckRamCrc(int fd)
{
	...
	if (RamHeader.crc16 != CalCRC16((unsigned char *)&RamHeader, sizeof(RamHeader) - sizeof(RamHeader.crc16)))
	{
		...
		nwy_file_seek(fd, 0, NWY_SEEK_SET);
		if (nwy_file_read(fd, &RamHeader, sizeof(SaveDataInfo)) == sizeof(SaveDataInfo))
		{
		...
		else
		{
			...
			nwy_file_close(fd);
			nwy_file_remove(COLLECTED_DATA_FILE_PATH);
			return FALSE;
		}
	}
	return TRUE;
...
BYTE WriteCollectedDataToFile(CollectionDatas *CollectData)
{
	...
	if (fd >= 0)
	{
		...
		nwy_file_seek(fd, (sizeof(SaveDataInfo) + RamHeader.WriteOffset * sizeof(CollectionDatas)), NWY_SEEK_SET);
		if (nwy_file_write(fd, CollectData, sizeof(CollectionDatas)) != sizeof(CollectionDatas))
		{
			nwy_ext_echo("\r\nwrite data error");
			nwy_file_close(fd);
			return FALSE;
...
BYTE ReadOldestRecord(CollectionDatas *CollectData)
{
	...
	if (fd >= 0)
	{
		...
		// 定位到数据区中对应的最旧记录位置
		nwy_file_seek(fd, (sizeof(SaveDataInfo) + RamHeader.ReadOffset * sizeof(CollectionDatas)), NWY_SEEK_SET);
		ReadBytes = nwy_file_read(fd, CollectData, sizeof(CollectionDatas));
...
static BOOL pub_cycledatatoUser(CollectionDatas *pCollectData) //大数据用指针传参
{
	...
	pubmsg.dup = 0;
	int rc = MQTTPublish(&paho_user_client, paho_mqtt_user_param.topic, &pubmsg);
	if (rc)
	{
		free(c_message);
...
void MQTT_UserServices(void)
{
	...
	#if(CYCLE_REPORT_PROTOCAL == PROTOCOL_MQTT)
	if (nwy_msg_queue_recv(CollectMessageQueue, sizeof(char *), &CollectDataPtr, NWY_OSA_NO_SUSPEND) == NWY_SUCCESS) // 组MQTT帧进行发送
	{
		memcpy(&CollectData, CollectDataPtr, sizeof(CollectionDatas));
		#if (CYCLE_REPORT == YES)
	...
	#endif
	#if (EVENT_REPORT == YES)
	if (nwy_msg_queue_recv(EventReportMessageQueue, sizeof(Eventmessage),&EventData, NWY_OSA_NO_SUSPEND) == NWY_SUCCESS) //事件上报
	{
...
void UsermessageArrived(MessageData *md)
{
	...
	if (!nwy_msg_queue_space_num_get(MqttResUserMessageQueue))
	{
		nwy_ext_echo("NEED_SPACE");
		return;
	}
	...
	memcpy(msg, md->message->payload, md->message->payloadlen);
	if (nwy_msg_queue_send(MqttResUserMessageQueue, sizeof(char*),&msg, NWY_OSA_NO_SUSPEND) != NWY_SUCCESS)
	{
		free(msg);
	}
}
...
void nwy_paho_usercycle(void *para)
{
	while (1)
	{
		...
		nwy_thread_suspend(nwy_paho_usertask_id);
	}
	nwy_thread_sleep(200);
}
nwy_osi_thread_t *nwy_paho_yeild_usertask_init(void)
{
	if (nwy_paho_usertask_id == NULL)
	{
		nwy_osi_thread_t task_id;
		nwy_thread_create(task_id,"neo_paho_yeild_task",NWY_OSI_PRIORITY_NORMAL, nwy_paho_usercycle, NULL, 4, 1024 * 8,NULL );
		nwy_paho_usertask_id = task_id;
	}
	else
		nwy_thread_resume(nwy_paho_usertask_id);
...
void MQTT_connetuser(void)
{
	...
	if (NULL == (g_nwy_user_writebuf = (unsigned char *)malloc(NWY_PAHO_PLAYOAD_LEN_MAX)))
	{
	...
	if (NULL == (g_nwy_user_readbuf = (unsigned char *)malloc(NWY_PAHO_PLAYOAD_LEN_MAX)))
	{
		nwy_ext_echo("\r\nmalloc buffer g_nwy_user_readbuf error");
		return;
	}
	memset(g_nwy_user_readbuf, 0, NWY_PAHO_PLAYOAD_LEN_MAX);
	memset(g_nwy_user_writebuf, 0, NWY_PAHO_PLAYOAD_LEN_MAX);
	...
	MQTTClientInit(&paho_user_client, paho_user_network, 10000, g_nwy_user_writebuf, NWY_PAHO_PLAYOAD_LEN_MAX, g_nwy_user_readbuf, NWY_PAHO_PLAYOAD_LEN_MAX);
	...
	nwy_ext_echo("\r\nConnecting MQTT");
	rc = MQTTConnect(&paho_user_client, &datatemp);
	if (rc)
	{
		nwy_ext_echo("\r\nFailed to create client, return code %d", rc);
	...
	else
	{
		...
		nwy_osi_thread_t *task_id = nwy_paho_yeild_usertask_init();
		if (task_id == NULL)
		{
			nwy_ext_echo("\r\npaho yeid task create failed ");
			MQTTDisconnect(&paho_user_client);
		...
		else
		{
			...
			if (nwy_msg_queue_space_num_get(EventReportMessageQueue) != 0)//上电清除所有告警标志
			{
				nwy_msg_queue_send(EventReportMessageQueue, sizeof(EventData),&EventData, NWY_OSA_NO_SUSPEND);
			}
			#endif
		}
	}
...
void USER_MQTT_Task(void *param)
{
	...
	DWORD Connectfre = 0;
	nwy_timer_para_t OneTimeTImerPara =	//考虑统一
	{
		.expired_time = 0,
		.type = NWY_TIMER_ONE_TIME,
		.cb = nwy_User_MQTT_Reconnect_timer_cb,
		.cb_para = NULL,
				...
				if (MQTTIsConnected(&paho_user_client) != 1)
				{
					...
					if (MQTTIsConnected(&paho_user_client) != 1) //没有连接成功，再进行延时，连接成功不再进行延时
					{
						if (Connectfre < 20)
						{
							...
							OneTimeTImerPara.expired_time = 6000 * Connectfre;
							nwy_sdk_timer_start(User_MQTT_Reconnect_timer, &OneTimeTImerPara);
						}
						else
						{
							UserMqttReconnectTimerFlag = 0;
							OneTimeTImerPara.expired_time = 3600000 * 4;
							nwy_sdk_timer_start(User_MQTT_Reconnect_timer, &OneTimeTImerPara); // 4小时重试一次连接 每次连接消耗560字节
						}
					}
					else
					{
