...
#define BAUD_RATE_FACTORY				9600
#define BYTE_DANCE_TIMEOUT_FACTORY 		200//字节间超时 ms		115200 间隔 默认50ms,9600:200
//-----------------------------------------------
//		本文件使用的结构体，共用体，枚举
//-----------------------------------------------
//-----------------------------------------------
...
static nwy_uartdcb_t dcb_factory = {
    .baudrate = 9600,
    .databits = 8,
    .stopbits = 1,
    .parity = PB_EVEN,
...
//-----------------------------------------------
//打印log
static nwy_osi_mutex_t fact_mutex = NULL;
void nwy_ext_factory(char *fmt, ...)
{
	...
	int i, size;
	if(NULL == fact_mutex)
		nwy_sdk_mutex_create(fact_mutex);
	if(NULL == fact_mutex)
		return;
	nwy_sdk_mutex_lock(fact_mutex, 0);
	va_start(a, fmt);
	...
	nwy_sdk_mutex_unlock(fact_mutex);
...
void SendDlt645(TSerial * p)
{
	...
	if (WhFlag&BIT1)
	{
		nwy_uart_write(UartWh,(BYTE*) &p->ProBuf[0],p->SendLength);
		WhFlag &= ~(BIT1);
		nwy_ext_echo("\r\nuart  send");
	}
	#endif
	...
	if (WhFlag & BIT2)
	{
		...
		if (QueueWrite(&MessageQueueInfoArr[eQUEUE_MODULE_TO_TCP], &FactoryTranData, sizeof(tTranData), NWY_OSA_NO_SUSPEND) == TRUE)
		{
...
void Deal_BLETran698(TSerial *p)
{
	...
	//串口收到回复报文还发送到串口？
	if (QueueWrite(&MessageQueueInfoArr[eQUEUE_TRAN_TO_UART], &FactoryTranData, sizeof(tTranData), NWY_OSA_NO_SUSPEND) == TRUE)
	{
...
WORD ReadFactoryExtPro(BYTE *pBuf)
{
	...
	WORD wReturnLen,tw,wLen,ServerConnectFlag = 0,buf2[100] = {0};
	...
	case 0xff:
		...
		{
			nwy_dm_imei_get(0,tempimei,sizeof(tempimei));
			lib_ExchangeData(p, (BYTE*)&tempimei, sizeof(tempimei));
			wReturnLen = sizeof(tempimei);
		}
		else if (pBuf[0] == 0x02)
		{
			for (i = CycleReadMeterNum; i < (CycleReadMeterNum + POWERON_READRPARA_NUM); i++) //置周期上报标志
			{
		...
		{
			if (api_GetMqttConStatus(0) == TRUE)
			{
		...
		{
			nwy_dm_inner_version_get(buf2, 100);
			nwy_ext_echo("\r\nble is %s", buf2);
			lib_ExchangeData(p, (BYTE *)&buf2[0], 100);//！！！ 读出来有杂报文
			wReturnLen = 100;
		}
		else if (pBuf[0] == 0x08) //读取信号强度 cqs
		{
			nwy_nw_csq_get(0,&csq_val);
			p[0] = csq_val.csq_rssi_level;
			wReturnLen = 1;
		}
...
WORD WriteFactoryExtPro(BYTE *pBuf)
{
	...
	switch (pBuf[1])
	{
		...
		if (((pBuf[0] >= 0x01) && (pBuf[0] <= 0x04)) || ((pBuf[0] >= 0x11) && (pBuf[0] <= 0x14)))
			{
				bTemp = (pBuf[0]>>4);
...
void api_ReceData_WHTask( void )
{
	...
	while (ReceNum != ParaSerial.RXRPoint)
	{
		...
		nwy_sdk_timer_start(Ble_Recv_ByteTimeout_timer, &OneTimeTImerPara);
		#endif
		...
		{
			nwy_sdk_timer_stop(Ble_Recv_ByteTimeout_timer);
			break;
		}
	}
...
BOOL  api_DeleteSysParaFile( void )
{
	// if (nwy_file_exist(filepara_name) == TRUE)
	// {
	// 	nwy_ext_echo("\r\n exit fle");
	// 	if(nwy_file_remove(filepara_name) == NWY_SUCESS)
	// 	{
	// 		nwy_ext_echo("\r\n dele secc");
	// 		return TRUE;
	// 	}
	// 	else
	// 	{
	// 		nwy_ext_echo("\r\ndelte fail is%d",nwy_file_remove(filepara_name));
	// 		return FALSE;
	// 	}
	// }		
	// else
...
BOOL  api_OperateFileSystem( BYTE Operation,int addr,BYTE*Buf,WORD bLen)//此函数测试没问题
{
	...
	fd = nwy_file_open(filepara_name, NWY_RB_PLUS_MODE);
	if (fd >= 0)
	{
...
//备注:  
//--------------------------------------------------
void  api_PowerOnCreatParaTable( void )// 读写文件待测试  api未细看
{
	int fd = -1, addr = 0;
	BYTE flag = FALSE;

	flag = nwy_file_exist(filepara_name);
	if (flag == FALSE)
	{
		fd = nwy_file_open(filepara_name, NWY_WB_PLUS_MODE);
	if(fd < 0)
	{
			nwy_ext_echo("\r\n open  para file fail,event_name:%s, ret:%d", filepara_name, fd);
			return;
		}
		else
    	{
...
void  api_ReadSystemFiletoRam( eREAD_METER_BIT eBit )
{
	...
	fd = nwy_file_open(filepara_name, NWY_RDONLY );
	if (fd >=0)
	{
		if(nwy_file_read(fd,(char*)&TempServerpara[0].ServerPort,2*sizeof(ServerParaRam_t)) ==(2*sizeof(ServerParaRam_t)))
		{
...
void  InitUartWH( void )
{
	...
	UartWh = nwy_uart_open(dev_name_list[2],BAUD_RATE_FACTORY,0);
	nwy_uart_dcb_set(UartWh, &dcb_factory);
	nwy_uart_rx_frame_timeout_set(UartWh, BYTE_DANCE_TIMEOUT_FACTORY);
	nwy_uart_rx_cb_register(UartWh, nwy_uart_wh_handle);
}
#endif
//--------------------------------------------------
//功能描述:  
...
void  Factory_Task( void *param )
{
	...
	while (1)
	{
		api_ReceData_WHTask();
		#if (TCP_COMM_MODULE == YES)
		if (nwy_msg_queue_recv(TcpDataToModuleMsgQue, sizeof(char *), &FactoryTranDataPtr, NWY_OSA_NO_SUSPEND) == NWY_SUCCESS)
		{
			memcpy(&FactoryTranData, FactoryTranDataPtr, sizeof(tTranData));
		...
		#endif
		nwy_thread_event_wait(factorypara_thread, &event, 50);//可以删掉 等待事件
	}
}
